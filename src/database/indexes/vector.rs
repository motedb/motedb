...
impl MoteDB {
    ...
    pub fn create_vector_index(
        ...
        if let Ok(schema) = self.table_registry.get_table(table_name) {
            if let Some(col_def) = schema.columns.iter().find(|c| c.name == column_name) {
                ...
                if let Some(col_sst) = self.columnar_sstables.get(table_name) {
                    match col_sst.read_vectors(col_position) {
                        Ok(vectors) => {
                            ...
                            debug_log!(
                                "[create_vector_index] Columnar path: {} vectors in {:?}",
                                vectors_to_index.len(),
                                start_time.elapsed()
                            );
                        }
                        Err(e) => {
                            let _ = e;
                        } // columnar read failed, fall through
                    }
                }
                ...
                if !vectors_to_index.is_empty() {
                    debug_log!(
                        "[create_vector_index] 扫描完成：{} 个向量，耗时 {:?}",
                        vectors_to_index.len(),
                        scan_time
                    );
                    ...
                    debug_log!(
                        "[create_vector_index] 批量建索引完成！耗时 {:?}",
                        build_time.elapsed()
                    );
                } else {
                    debug_log!(
                        "[create_vector_index] 未找到任何向量数据（扫描耗时 {:?}）",
                        scan_time
                    );
                }
            }
    ...
    pub fn update_vector(&self, row_id: RowId, index_name: &str, vector: &[f32]) -> Result<()> {
        let index_ref = self.vector_indexes.get(index_name).ok_or_else(|| {
    ...
    pub fn find_vector_index_name(&self, table_name: &str, column_name: &str) -> Result<String> {
        self.table_registry
            .find_vector_index(table_name, column_name)
    }
    ...
    pub fn vector_search(
        ...
        debug_log!(
            "[vector_search] DiskANN index搜索完成，结果数: {}",
            index_results.len()
        );
        ...
        let table_prefix = self.table_registry.get_table_id(table_name).unwrap_or(0) as u64;
        let mut memtable_results = Vec::new();
        self.lsm_engine
            .scan_memtable_incremental_with(|composite_key, row_bytes| {
        ...
        if !memtable_results.is_empty() {
            debug_log!(
                "[vector_search] 🔍 Memtable扫描到{}个向量",
                memtable_results.len()
            );
            debug_log!(
                "[vector_search] 🔍 Memtable前5个: {:?}",
                &memtable_results
                    .iter()
                    .take(5)
                    .map(|(id, dist)| (id, format!("{:.4}", dist)))
                    .collect::<Vec<_>>()
            );
        } else {
