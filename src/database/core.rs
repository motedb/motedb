...
pub struct MoteDB {
    ...
    /// Decoded VECTOR-column cache budget per table (from DBConfig, MB).
    pub(crate) vector_cache_budget_mb: Option<usize>,
...
impl MoteDB {
    ...
    pub fn create_with_config<P: AsRef<Path>>(path: P, config: DBConfig) -> Result<Self> {
        ...
        let mut db = Self {
            ...
            vector_cache_budget_mb: config.vector_cache_budget_mb,
    ...
    pub(crate) fn clone_for_callback(&self) -> Self {
        Self {
            ...
            vector_cache_budget_mb: self.vector_cache_budget_mb,
                            ...
                            if let Ok(schema) = db.table_registry.get_table(&table_name) {
                                    ...
                                    )
                                {
                                    ...
                                    store.set_vector_cache_budget(
                                        db.vector_cache_budget_mb
                                            .map(|mb| mb.saturating_mul(1024 * 1024))
                                            .unwrap_or(
                                                crate::storage::col_segment::DEFAULT_VECTOR_COL_CACHE_BUDGET_BYTES,
                                            ),
                                    );
