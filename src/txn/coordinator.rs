...
impl TransactionCoordinator {
    ...
    pub fn commit(&self, txn_id: TransactionId) -> Result<Timestamp> {
        ...
        // Mark as committed
        ctx.state
            .store(TransactionState::Committed as u8, Ordering::Release);
    ...
    pub fn rollback(&self, txn_id: TransactionId) -> Result<()> {
        ...
        // Mark as aborted
        ctx.state
            .store(TransactionState::Aborted as u8, Ordering::Release);
    ...
    pub fn create_savepoint(&self, txn_id: TransactionId, name: String) -> Result<()> {
        ...
        debug_log!(
            "[Savepoint] Created delta savepoint '{}' for txn {} (mem: 0 bytes)",
            name,
            txn_id
        );
    ...
    pub fn rollback_to_savepoint(&self, txn_id: TransactionId, name: &str) -> Result<()> {
        ...
        debug_log!(
            "[Savepoint] Rolled back to '{}' in txn {} (undid {} ops)",
            name,
            txn_id,
            undo_count
        );
    ...
    pub fn release_savepoint(&self, txn_id: TransactionId, name: &str) -> Result<()> {
        ...
        debug_log!(
            "[Savepoint] Released savepoint '{}' in txn {}",
            name,
            txn_id
        );
    ...
    pub fn get_context(&self, txn_id: TransactionId) -> Result<Arc<TransactionContext>> {
        self.active_txns
            ...
            .ok_or_else(|| StorageError::Transaction(format!("Transaction {} not found", txn_id)))
    }
    ...
    fn create_snapshot_internal(
        ...
        let active_txns: HashSet<TransactionId> = self
            .active_txns
            .iter()
        ...
        for row_id in read_set.iter() {
            if let Some(chain) = self.version_store.versions.get(row_id) {
                ...
                if let Some(version) = head.as_ref() {
                    if version.begin_ts > ctx.snapshot.timestamp && version.txn_id != ctx.txn_id {
                        return Err(StorageError::Transaction(format!(
                            "Read-write conflict on row {} in txn {}",
                            row_id, ctx.txn_id
                        )));
                    }
                }
