...
impl DiskANNIndex {
    ...
    pub fn create(
        ...
        let graph = Arc::new(DiskGraph::create(data_dir, config.max_degree, graph_cache)?);
    ...
    pub fn load(data_dir: impl AsRef<Path>, config: VamanaConfig) -> Result<Self> {
        let data_dir = data_dir.as_ref();
        ...
        // Load SQ8 vectors
        let sq8_vectors = Arc::new(SQ8Vectors::load(data_dir, quantizer.clone(), vector_cache)?);
    ...
    pub fn batch_insert(&self, vectors: &[(RowId, Vec<f32>)]) -> Result<usize> {
        ...
        self.batch_build_graph(&ids)?;
        debug_log!(
            "[DiskANN] Graph built in {:?}",
            _graph_build_start.elapsed()
        );
    ...
    fn batch_build_graph(&self, ids: &[RowId]) -> Result<()> {
        ...
        if show_progress {
            debug_log!(
                "[DiskANN] 🔥 Progressive Batch Build: {} nodes in {} batches",
                new_count,
                num_batches
            );
        ...
        let medoid_vec = match self.vectors.get(medoid_id) {
            ...
            None => {
                return Err(StorageError::InvalidData(
                    "Failed to get medoid vector".into(),
                ))
            }
        };

        let mut nodes_with_dist: Vec<_> = shuffled
            .iter()
    ...
    pub fn reorder_for_ssd(&self) -> Result<()> {
        ...
        debug_log!(
            "[DiskANN] ✅ SSD optimization completed in {:?}",
            _start.elapsed()
        );
    ...
    fn incremental_insert_into_graph(&self, new_id: RowId, medoid_id: RowId) -> Result<()> {
        ...
        let ef_construction = if graph_size < 1000 {
            300
