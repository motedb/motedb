...
impl FreshVamanaGraph {
    ...
    pub fn batch_insert(&self, vectors: &[(RowId, Vec<f32>)]) -> Result<()> {
        ...
        if self.nodes.len() + vectors.len() > self.config.max_nodes {
            return Err(StorageError::ResourceExhausted(format!(
                "Batch insert would exceed max_nodes: {} + {} > {}",
                self.nodes.len(),
                vectors.len(),
                self.config.max_nodes
            )));
    ...
    fn batch_build_graph_simple(&self, node_ids: &[RowId], max_degree: usize) -> Result<()> {
        for &node_id in node_ids {
            if let Some(node_ref) = self.nodes.get(&node_id) {
                ...
                // 批量计算距离（自动使用 SIMD）
                let mut distances: Vec<_> = node_ids
                    .iter()
    ...
    fn batch_build_graph_parallel(&self, node_ids: &[RowId], max_degree: usize) -> Result<()> {
        ...
        let vectors: Vec<_> = node_ids
            .par_iter()
            .filter_map(|&id| self.nodes.get(&id).map(|node| (id, node.vector.clone())))
            .collect();

        debug_log!("[FreshGraph] 预加载 {} 个向量", vectors.len());

        // 🚀 Phase 2: 并行计算每个节点的邻居（自动SIMD优化）
        let neighbors_list: Vec<_> = vectors
            .par_iter()
            .map(|(node_id, vector)| {
    ...
    fn brute_force_knn(&self, query: &[f32], k: usize) -> Vec<RowId> {
        let mut candidates: Vec<(RowId, f32)> = self
            .nodes
            .iter()
            .map(|entry| {
    ...
    fn graph_search(&self, query: &[f32], k: usize, ef: usize) -> Result<Vec<Candidate>> {
        ...
        let mut results: Vec<Candidate> = global_candidates
            .into_sorted_vec()
            .into_iter()
    ...
    fn graph_search_from_point(
        ...
        // 🆕 过滤已删除节点
        let results: Vec<Candidate> = visited
            .into_sorted_vec()
            .into_iter()
