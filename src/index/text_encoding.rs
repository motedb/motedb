...
///   ...
/// ```ignore
pub fn encode_posting_list(doc_ids: &[u64], positions: Option<&Vec<Vec<u32>>>) -> Result<Vec<u8>> {
    let mut bytes = Vec::new();
...
pub fn encode_segmented_posting_list(
    ...
    while offset < num_docs {
        ...
        let segment_positions = positions
            .as_ref()
            .map(|pos_lists| pos_lists[offset..end].to_vec());

        let encoded = encode_posting_list(segment_doc_ids, segment_positions.as_ref())?;
...
mod tests {
    ...
    fn test_varint_encoding() {
        // Test various values
        let test_cases = vec![0u64, 127, 128, 16383, 16384, u64::MAX];
    ...
    fn test_posting_list_with_positions() {
        ...
        let positions = vec![vec![0, 5, 10], vec![2, 8], vec![1, 3, 7, 15]];
