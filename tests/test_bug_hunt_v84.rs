...
fn test_single_row_aggregates() {
    ...
    assert_eq!(q(&db, "SELECT COUNT(*), SUM(v), MIN(v), MAX(v), AVG(v) FROM t"),
        vec![vec![Value::Integer(1), Value::Integer(42), Value::Integer(42), Value::Integer(42), Value::Integer(42)]]);
}
...
fn test_having_on_group_key() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'b',5),(4,'c',100)").unwrap();
...
fn test_tautology_or() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,20),(3,NULL)").unwrap();
...
fn test_subquery_null_comparison() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("CREATE TABLE empty(id INT PRIMARY KEY, x INT)").unwrap();
...
fn test_count_all_null_col() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,NULL),(2,NULL),(3,NULL)").unwrap();
...
fn test_aggregate_null_in_middle() {
    ...
    assert_eq!(q(&db, "SELECT COUNT(v) FROM t"), vec![vec![Value::Integer(2)]]);
    assert_eq!(q(&db, "SELECT MIN(v) FROM t"), vec![vec![Value::Integer(10)]]);
...
fn test_where_expr_both_sides() {
    ...
    db.execute("INSERT INTO t VALUES (1,10,5),(2,20,10)").unwrap();
...
fn test_nested_coalesce_case() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,NULL),(2,5),(3,0)").unwrap();
    let r = q(&db, "SELECT COALESCE(CASE WHEN v IS NULL THEN -1 ELSE v END, -99) FROM t ORDER BY id");
    assert_eq!(r, vec![
...
fn test_order_by_multi_with_null() {
    ...
    assert!(pos1 < pos3 && pos1 < pos4 && pos2 < pos3 && pos2 < pos4,
        "a=1 rows (id1,id2) must come before a=2 rows (id3,id4); got {:?}", ids);
}
...
fn test_distinct_order_by_other() {
    ...
    db.execute("INSERT INTO t VALUES (3,'a'),(1,'b'),(2,'a')").unwrap();
...
fn test_having_different_col() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a',10,1),(2,'a',20,2),(3,'b',5,10)").unwrap();
    // SELECT cat, SUM(v) HAVING SUM(w) > 5.
    // 'a': SUM(w)=3 (not >5). 'b': SUM(w)=10 (>5).
    let r = q(&db, "SELECT cat, SUM(v) FROM t GROUP BY cat HAVING SUM(w) > 5");
...
fn test_text_numeric_lexicographic() {
    ...
    db.execute("INSERT INTO t VALUES (1,'10'),(2,'9'),(3,'100')").unwrap();
    ...
    assert_eq!(vals, vec!["10".to_string(), "100".to_string(), "9".to_string()],
