...
fn test_groupby_multiple_keys() {
    ...
    db.execute("INSERT INTO t VALUES (1,1,10),(1,1,20),(1,2,30),(2,1,40)").unwrap();
    let r = q(&db, "SELECT a, b, SUM(v) FROM t GROUP BY a, b ORDER BY a, b");
    assert_eq!(
        r,
        vec![
...
fn test_rank_order_by_partition() {
    ...
    db.execute("INSERT INTO s VALUES (1,1,10),(2,1,20),(3,2,30)").unwrap();
    let r = q(
        &db,
...
fn test_count_distinct_text() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a'),(2,'b'),(3,'a'),(4,'c')").unwrap();
...
fn test_count_distinct_null_excluded() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1),(2,1),(3,NULL),(4,2)").unwrap();
...
fn test_update_with_case() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT, bucket TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,5,''),(2,15,''),(3,25,'')").unwrap();
    db.execute("UPDATE t SET bucket = CASE WHEN v < 10 THEN 'low' WHEN v < 20 THEN 'mid' ELSE 'high' END").unwrap();
...
fn test_derived_table_where() {
    ...
    let r = q(&db, "SELECT v FROM (SELECT v FROM t WHERE v > 15) sub ORDER BY v");
