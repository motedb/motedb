...
fn test_scalar_subquery_empty() {
    ...
    db.execute("CREATE TABLE empty(id INT PRIMARY KEY, v INT)")
        .unwrap();
...
fn test_sum_of_expression() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,1),(2,20,2),(3,30,3)")
        .unwrap();
    // SUM(a + b): 11 + 22 + 33 = 66.
...
fn test_join_with_where_both() {
    ...
    db.execute("CREATE TABLE a(id INT PRIMARY KEY, x INT)")
        .unwrap();
    db.execute("CREATE TABLE b(id INT PRIMARY KEY, y INT)")
        .unwrap();
    db.execute("INSERT INTO a VALUES (1,10),(2,20)").unwrap();
    db.execute("INSERT INTO b VALUES (1,100),(2,200)").unwrap();
    let r = q(
        &db,
        "SELECT a.x, b.y FROM a JOIN b ON a.id = b.id WHERE a.x > 15",
    );
...
fn test_order_by_col_not_in_select() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,3),(2,20,1),(3,30,2)")
        .unwrap();
    // SELECT a, ORDER BY b.
...
fn test_group_by_all_filtered() {
    ...
    let r = q(
        &db,
        "SELECT cat, COUNT(*) FROM t WHERE v > 100 GROUP BY cat",
    );
...
fn test_where_cross_column() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,20),(2,30,30),(3,40,10)")
        .unwrap();
...
fn test_correlated_subquery_gt() {
    ...
    let mut r = q(
        &db,
        "SELECT id FROM t t1 WHERE v > (SELECT AVG(v) FROM t t2 WHERE t2.cat = t1.cat)",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
...
fn test_in_subquery_pk() {
    ...
    db.execute("INSERT INTO child VALUES (10,1),(20,2),(30,99)")
        .unwrap();
    // Children whose pid exists in parent.
    let mut r = q(
        &db,
        "SELECT id FROM child WHERE pid IN (SELECT id FROM parent)",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
