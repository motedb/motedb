...
use motedb::{Database, DBConfig, QueryResult, types::Value};
...
fn assert_count(db: &Database, sql: &str, expected: usize) {
    ...
    assert_eq!(actual, expected, "SQL '{}' returned {} rows, expected {}", sql, actual, expected);
}
...
fn test_vector_crud_lifecycle() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE vecs (id INT PRIMARY KEY AUTO_INCREMENT, emb VECTOR(4), label TEXT)").unwrap();

    // CREATE
    db.execute("INSERT INTO vecs (emb, label) VALUES ([1.0, 0.0, 0.0, 0.0], 'x')").unwrap();
    db.execute("INSERT INTO vecs (emb, label) VALUES ([0.0, 1.0, 0.0, 0.0], 'y')").unwrap();
    db.execute("INSERT INTO vecs (emb, label) VALUES ([0.0, 0.0, 1.0, 0.0], 'z')").unwrap();
    ...
    let r = db.execute("SELECT id FROM vecs ORDER BY emb <-> [1.0, 0.0, 0.0, 0.0] LIMIT 1").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // UPDATE
    db.execute("UPDATE vecs SET label = 'x-axis' WHERE id = 1").unwrap();
...
fn test_spatial_crud_lifecycle() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE pts (id INT PRIMARY KEY AUTO_INCREMENT, loc GEOMETRY, name TEXT)").unwrap();

    // CREATE
    db.execute("INSERT INTO pts (loc, name) VALUES (POINT(1.0, 2.0), 'home')").unwrap();
    db.execute("INSERT INTO pts (loc, name) VALUES (POINT(3.0, 4.0), 'office')").unwrap();
    db.execute("INSERT INTO pts (loc, name) VALUES (POINT(5.0, 6.0), 'park')").unwrap();
    assert_eq!(count_rows(&db, "SELECT COUNT(*) FROM pts"), 1); // COUNT returns 1 row

    // READ via distance
    let r = db.execute("SELECT id FROM pts ORDER BY ST_DISTANCE(loc, 1.0, 2.0) LIMIT 1").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // UPDATE
    db.execute("UPDATE pts SET name = 'house' WHERE id = 1").unwrap();
...
fn test_text_crud_lifecycle() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE docs (id INT PRIMARY KEY AUTO_INCREMENT, title TEXT, body TEXT)").unwrap();

    // CREATE
    db.execute("INSERT INTO docs (title, body) VALUES ('Intro', 'database for edge computing')").unwrap();
    db.execute("INSERT INTO docs (title, body) VALUES ('Guide', 'machine learning on embedded devices')").unwrap();
    db.execute("INSERT INTO docs (title, body) VALUES ('Ref', 'sensor data and real-time processing')").unwrap();
    ...
    // UPDATE
    db.execute("UPDATE docs SET title = 'Introduction' WHERE id = 1").unwrap();
...
fn test_mixed_vector_plus_spatial() {
    ...
    for i in 0..50 {
        ...
        let sql = format!("INSERT INTO items (emb, loc, cat) VALUES ({}, POINT({:.1}, 40.0), '{}')", v, lat, cat);
        db.execute(&sql).unwrap();
    }

    // Vector KNN + category filter
    let r = db.execute("SELECT id FROM items WHERE cat = 'A' ORDER BY emb <-> [0.0, 0.5, 0.3, 0.2] LIMIT 5").unwrap();
    ...
    let r = db.execute("SELECT id FROM items WHERE cat = 'B' ORDER BY ST_DISTANCE(loc, 35.0, 40.0) LIMIT 3").unwrap();
    if let QueryResult::Select { rows, .. } = r.materialize().unwrap() {
...
fn test_mixed_vector_plus_text_plus_scalar() {
    ...
    for i in 0..30 {
        let v: Vec<String> = (0..8).map(|d| format!("{:.3}", ((i + d) as f64 * 0.1).sin())).collect();
        ...
        let sql = format!(
            ...
            v.join(","), name, price, stock
        );
        db.execute(&sql).unwrap();
    }

    // Vector KNN + price range + in_stock filter
    let qv: Vec<String> = (0..8).map(|d| format!("{:.3}", (d as f64 * 0.1).sin())).collect();
    let sql = format!(
...
fn test_mixed_all_types_single_table() {
    ...
    // Insert mixed data
    for i in 0..20 {
        let v = format!("[{:.2},{:.2},{:.2},{:.2}]", (i as f64).sin(), (i as f64 * 2.0).sin(), 0.5, 0.5);
    ...
    let r = db.execute("SELECT id FROM sensors WHERE val > 30 ORDER BY ST_DISTANCE(loc, 42.0, 35.0) LIMIT 5").unwrap();
    if let QueryResult::Select { rows, .. } = r.materialize().unwrap() {
        assert!(!rows.is_empty(), "should find sensors with val > 30");
    }

    // Aggregate: count by region
    let r = db.execute("SELECT region, COUNT(*) FROM sensors GROUP BY region").unwrap().materialize().unwrap();
    if let QueryResult::Select { rows, .. } = r {
        assert_eq!(rows.len(), 2, "should have 2 regions");
    }

    // Aggregate: average val by active status
    let r = db.execute("SELECT active, AVG(val) FROM sensors GROUP BY active").unwrap().materialize().unwrap();
    if let QueryResult::Select { rows, .. } = r {
...
fn test_empty_table_queries() {
    ...
    let r = db.execute("SELECT id FROM empty ORDER BY emb <-> [1.0, 0.0, 0.0, 0.0] LIMIT 5").unwrap();
...
fn test_single_row_table() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE solo (id INT PRIMARY KEY AUTO_INCREMENT, emb VECTOR(2), loc GEOMETRY)").unwrap();
    db.execute("INSERT INTO solo (emb, loc) VALUES ([1.0, 0.0], POINT(0.0, 0.0))").unwrap();

    assert_count(&db, "SELECT * FROM solo", 1);
    assert_count(&db, "SELECT * FROM solo WHERE id = 1", 1);

    // KNN should return the single row
    let r = db.execute("SELECT id FROM solo ORDER BY emb <-> [0.9, 0.1] LIMIT 1").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if rows.len() == 1));
...
fn test_large_vector_dim() {
    let (db, _dir) = db();
    let dim = 256;
    db.execute(&format!("CREATE TABLE big_vec (id INT PRIMARY KEY AUTO_INCREMENT, emb VECTOR({}))", dim)).unwrap();

    let v: Vec<String> = (0..dim).map(|i| format!("{:.4}", (i as f64 * 0.01).sin())).collect();
    let sql = format!("INSERT INTO big_vec (emb) VALUES ([{}])", v.join(","));
    db.execute(&sql).unwrap();

    assert_count(&db, "SELECT * FROM big_vec", 1);

    let qv: Vec<String> = (0..dim).map(|i| format!("{:.4}", (i as f64 * 0.01).cos())).collect();
    let sql = format!("SELECT id FROM big_vec ORDER BY emb <-> [{}] LIMIT 1", qv.join(","));
...
fn test_null_and_missing_values() {
    ...
    db.execute("INSERT INTO nullable (emb, txt, val) VALUES ([0.0, 0.0], '', 0.0)").unwrap();
    db.execute("INSERT INTO nullable (emb, txt, val) VALUES ([1.0, 1.0], 'data', 42.5)").unwrap();
...
fn test_batch_insert_mixed_types() {
    ...
    db.execute("INSERT INTO batch (emb, loc, cat) VALUES ([1.0, 0.0, 0.0, 0.0], POINT(1.0, 1.0), 'A')").unwrap();
    db.execute("INSERT INTO batch (emb, loc, cat) VALUES ([0.0, 1.0, 0.0, 0.0], POINT(2.0, 2.0), 'B')").unwrap();
    db.execute("INSERT INTO batch (emb, loc, cat) VALUES ([0.0, 0.0, 1.0, 0.0], POINT(3.0, 3.0), 'A')").unwrap();
...
fn test_create_query_with_indexes() {
    ...
    let r = db.execute("SELECT id FROM idx_test WHERE body LIKE 'doc%' LIMIT 5").unwrap();
    assert!(r.materialize().unwrap().row_count() <= 5);

    // Vector distance query
    let r = db.execute("SELECT id FROM idx_test ORDER BY emb <-> [0.0, 0.5, 0.5, 0.5] LIMIT 3").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));
}
...
fn test_index_after_bulk_insert() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE bulk (id INT PRIMARY KEY AUTO_INCREMENT, emb VECTOR(8), region TEXT)").unwrap();

    // Individual INSERTs for VECTOR data
    for i in 0..100 {
        let v: Vec<String> = (0..8).map(|d| format!("{:.3}", ((i + d) as f64 * 0.1).sin())).collect();
        let region = if i % 3 == 0 { "US" } else { "EU" };
        db.execute(&format!("INSERT INTO bulk (emb, region) VALUES ([{}],'{}')", v.join(","), region)).unwrap();
    }
    ...
    // Query with index
    assert!(count_rows(&db, "SELECT * FROM bulk WHERE region = 'US'") >= 1, "should find US rows"); // ceil(1000/3)
}
...
fn test_memory_does_not_grow_unbounded() {
    ...
    for batch_num in 0..5 {
        ...
        for i in start..end {
            ...
            let v: Vec<String> = (0..16).map(|d| format!("{:.3}", ((i + d) as f64 * 0.01).sin())).collect();
            let region = if i % 3 == 0 { "US" } else { "EU" };
            sql.push_str(&format!("([{}],POINT({:.1},{:.1}),'cat_{}',{:.1})", v.join(","), 30.0 + (i % 90) as f64, 40.0 + (i % 90) as f64, i % 10, (i as f64 * 1.7) % 1000.0));
        }
        ...
        eprintln!("  After {}K rows: RSS = {:.0} MB", (batch_num + 1) * 10, rss);
    }
    ...
    eprintln!("  RSS: first={:.0}MB last={:.0}MB ratio={:.1}x", first, last, last / first);
    assert!(last < first * 3.0, "RSS grew more than 3x: {:.0} → {:.0} MB", first, last);
}

#[test]
fn test_latency_growth_sublinear() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE perf (id INT PRIMARY KEY AUTO_INCREMENT, cat TEXT, val FLOAT)").unwrap();
    ...
    for start in (0..10_000).step_by(5000) {
        ...
        for i in start..(start + 5000).min(10_000) {
            if !sql.is_empty() { sql.push(','); }
            sql.push_str(&format!("('cat_{}',{:.1})", i % 10, (i as f64 * 1.7) % 1000.0));
        }
        db.execute(&format!("INSERT INTO perf (cat, val) VALUES {}", sql)).unwrap();
    }
    ...
    eprintln!("  Latency ratio (30K/10K): {:.1}x (data ratio: 3.0x)", ratio);
    assert!(ratio < 5.0, "Latency grew more than 5x for 3x data: {:.1}x", ratio);
}
...
fn test_transaction_multimodal_insert() {
    ...
    db.execute("INSERT INTO txn_mm (emb, loc, txt) VALUES ([1.0, 0.0, 0.0, 0.0], POINT(1.0, 1.0), 'base')").unwrap();
...
fn test_transaction_multimodal_rollback() {
    let (db, _dir) = db();
    db.execute("CREATE TABLE rb_mm (id INT PRIMARY KEY AUTO_INCREMENT, emb VECTOR(4), txt TEXT)").unwrap();

    db.execute("INSERT INTO rb_mm (emb, txt) VALUES ([1.0, 0.0, 0.0, 0.0], 'committed')").unwrap();
    assert_count(&db, "SELECT * FROM rb_mm", 1);

    db.execute("BEGIN").unwrap();
    db.execute("INSERT INTO rb_mm (emb, txt) VALUES ([0.0, 1.0, 0.0, 0.0], 'rolled_back')").unwrap();
...
fn test_robot_sensor_scenario() {
    ...
    let (db, _dir) = db();
    db.execute("CREATE TABLE robot_log (\
    ...
    for t in 0..50 {
        ...
        let v: Vec<String> = (0..16).map(|d| format!("{:.3}", ((t + d) as f64 * 0.05).sin())).collect();
        let event = match t % 4 {
            0 => "obstacle_detected",
            1 => "path_planning",
    ...
    let qv: Vec<String> = (0..16).map(|d| format!("{:.3}", (d as f64 * 0.05).sin())).collect();
    let sql = format!(
    ...
    let r = db.execute("SELECT COUNT(*) FROM robot_log WHERE battery < 50").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // Aggregate: events per zone
    let r = db.execute("SELECT zone, COUNT(*) FROM robot_log GROUP BY zone").unwrap().materialize().unwrap();
    if let QueryResult::Select { rows, .. } = r {
...
fn test_ar_glasses_scenario() {
    ...
    let (db, _dir) = db();
    db.execute("CREATE TABLE anchors (\
        id INT PRIMARY KEY AUTO_INCREMENT, \
        position GEOMETRY, \
        descriptor VECTOR(8), \
        label TEXT, \
    ...
    let qv: Vec<String> = (0..8).map(|d| format!("{:.3}", (d as f64 * 0.1).sin())).collect();
    let sql = format!("SELECT id, label FROM anchors ORDER BY descriptor <-> [{}] LIMIT 3", qv.join(","));
    let r = db.execute(&sql).unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // Count by label
    let r = db.execute("SELECT label, COUNT(*) FROM anchors GROUP BY label").unwrap().materialize().unwrap();
    if let QueryResult::Select { rows, .. } = r {
...
fn test_iot_drone_scenario() {
    ...
    let (db, _dir) = db();
    db.execute("CREATE TABLE telemetry (\
        id INT PRIMARY KEY AUTO_INCREMENT, \
        gps GEOMETRY, \
        sensor_vec VECTOR(6), \
        status TEXT, \
    ...
    for t in 0..40 {
        ...
        let v: Vec<String> = (0..6).map(|d| format!("{:.2}", ((t + d) as f64 * 0.1).sin() * 50.0 + 500.0)).collect();
        let status = if t < 10 { "takeoff" } else if t < 30 { "cruising" } else { "landing" };
        let alt = if t < 10 { t as f64 * 10.0 } else if t < 30 { 100.0 } else { (40 - t) as f64 * 10.0 };
        let batt = 100 - t * 2;
    ...
    let r = db.execute("SELECT id FROM telemetry ORDER BY ST_DISTANCE(gps, 37.02, -122.02) LIMIT 3").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // Cruising phase with low battery
    let r = db.execute("SELECT COUNT(*) FROM telemetry WHERE status = 'cruising' AND battery < 70").unwrap();
    assert!(matches!(&r.materialize().unwrap(), QueryResult::Select { rows, .. } if !rows.is_empty()));

    // Similar sensor patterns during takeoff
    let qv: Vec<String> = (0..6).map(|d| format!("{:.2}", (d as f64 * 0.1).sin() * 50.0 + 500.0)).collect();
    let sql = format!(
    ...
    let r = db.execute("SELECT status, COUNT(*) FROM telemetry GROUP BY status").unwrap().materialize().unwrap();
    if let QueryResult::Select { rows, .. } = r {
