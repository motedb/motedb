...
fn test_int_where_eq_bool() {
    ...
    db.execute("INSERT INTO t VALUES (1,0),(2,1),(3,5)").unwrap();
    // v = TRUE (1) → id2.
...
fn test_join_on_bool() {
    let (db, _d) = db();
    db.execute("CREATE TABLE a(id INT PRIMARY KEY, flag BOOLEAN)").unwrap();
    db.execute("CREATE TABLE b(id INT PRIMARY KEY, flag BOOLEAN)").unwrap();
    db.execute("INSERT INTO a VALUES (1,TRUE)").unwrap();
    db.execute("INSERT INTO b VALUES (1,TRUE),(2,FALSE)").unwrap();
...
fn test_group_by_bool() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, flag BOOLEAN, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,TRUE,10),(2,FALSE,20),(3,TRUE,30)").unwrap();
    let mut r = q(&db, "SELECT flag, COUNT(*) FROM t GROUP BY flag");
    r.sort_by_key(|row| match row[0] { Value::Bool(b) => b, _ => false });
...
fn test_float_col_int_literal_where() {
    ...
    db.execute("INSERT INTO t VALUES (1,10.0),(2,20.5),(3,30.0)").unwrap();
...
fn test_comparison_in_arithmetic() {
    ...
    let _ = db.execute("SELECT (v > 5) FROM t").and_then(|s| s.materialize());
}
...
fn test_string_case_sensitive_comparison() {
    ...
    db.execute("INSERT INTO t VALUES (1,'Hello'),(2,'hello')").unwrap();
...
fn test_order_by_string_case() {
    ...
    assert_eq!(vals, vec!["Banana".to_string(), "apple".to_string(), "cherry".to_string()]);
}
