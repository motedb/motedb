...
fn test_count_distinct_text() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a'),(2,'b'),(3,'a'),(4,'c'),(5,'b')")
        .unwrap();
    assert_eq!(
        q(&db, "SELECT COUNT(DISTINCT s) FROM t"),
...
fn test_count_distinct_text_grouped() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'x','a'),(2,'x','b'),(3,'x','a'),(4,'y','c')")
        .unwrap();
...
fn test_sum_distinct_grouped() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'a',10),(4,'b',5)")
        .unwrap();
...
fn test_min_max_distinct_grouped() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'a',10),(4,'b',5),(5,'b',15)")
        .unwrap();
    let mut r = q(
        &db,
        "SELECT cat, MIN(DISTINCT v), MAX(DISTINCT v) FROM t GROUP BY cat",
    );
    r.sort_by_key(|row| match &row[0] {
        Value::Text(s) => s.as_str().to_string(),
        _ => "zzz".into(),
...
fn test_multiple_distinct_aggregates() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT, w INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,100),(2,10,200),(3,20,100),(4,20,200)")
        .unwrap();
...
fn test_having_non_agg_column() {
    ...
    let r = q(
        &db,
        "SELECT cat, SUM(v) FROM t GROUP BY cat HAVING cat != 'b'",
    );
...
fn test_group_having_all_excluded() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20)")
        .unwrap();
...
fn test_aggregate_uniform_values() {
    ...
    assert_eq!(
        q(
            &db,
            "SELECT SUM(v), MIN(v), MAX(v), AVG(v), COUNT(DISTINCT v) FROM t"
        ),
        vec![vec![
            Value::Integer(15),
            Value::Integer(5),
...
fn test_in_subquery_filtered() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE id IN (SELECT id FROM t WHERE cat = 'a')",
    ));
    assert_eq!(r, vec![1, 2]);
}
...
fn test_sum_expr_grouped() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, qty INT, price INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',2,5),(2,'a',3,10),(3,'b',1,20)")
        .unwrap();
