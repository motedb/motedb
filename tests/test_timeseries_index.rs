...
fn test_timestamp_sort_binary_search() {
    ...
    let results = db
        .columnar_store()
        .query_time_range("sort_test", 0, 2_000_000, &[])
        .unwrap();
    ...
    let narrow = db
        .columnar_store()
        .query_time_range("sort_test", 1_400_000, 1_600_000, &[])
        .unwrap();
...
fn test_zone_map_segment_pruning() {
    ...
    let conditions = vec![ColumnCondition::Equals {
        column_idx: 4, // zone column
        value: Value::Integer(0),
    }];
    let results = db
        .columnar_store()
        .query_with_conditions("zone_test", 0, 500_000, &conditions, &[])
        .unwrap();
    ...
    let range_conditions = vec![ColumnCondition::Range {
    ...
    let range_results = db
        .columnar_store()
        .query_with_conditions("zone_test", 0, 500_000, &range_conditions, &[])
        .unwrap();
    ...
    for (_rid, row) in &range_results {
        if let Some(Value::Float(temp)) = row.get("temperature") {
            assert!(
                *temp >= 25.0 && *temp <= 35.0,
                "Temperature should be in [25, 35], got {}",
                temp
            );
        }
    }
...
fn test_bloom_filter_text_pruning() {
    ...
    let results = db
        .columnar_store()
        .query_with_conditions("bloom_test", 0, 500_000, &conditions, &[])
        .unwrap();
    ...
    let no_conditions = vec![ColumnCondition::Equals {
        column_idx: 3,
        value: Value::text("nonexistent".to_string()),
    }];
    let no_results = db
        .columnar_store()
        .query_with_conditions("bloom_test", 0, 500_000, &no_conditions, &[])
        .unwrap();
    assert!(
        no_results.is_empty(),
        "Should return empty for nonexistent label"
    );
}
...
fn test_segment_manager_binary_search_pruning() {
    ...
    let narrow = db
        .columnar_store()
        .query_time_range("pruning_test", 2_030_000, 2_050_000, &[])
        .unwrap();
    ...
    let boundary = db
        .columnar_store()
        .query_time_range("pruning_test", 1_090_000, 2_010_000, &[])
        .unwrap();
    ...
    let full = db
        .columnar_store()
        .query_time_range("pruning_test", 0, 10_000_000, &[])
        .unwrap();
...
fn test_combined_time_and_column_pruning() {
    ...
    let results = db
        .columnar_store()
        .query_with_conditions("combined_test", 0, 10_000_000, &conditions, &[])
        .unwrap();
    ...
    let narrow_conditions = vec![ColumnCondition::Equals {
        column_idx: 4,
        value: Value::Integer(2),
    }];
    let narrow_results = db
        .columnar_store()
        .query_with_conditions("combined_test", 0, 1_500_000, &narrow_conditions, &[])
        .unwrap();
    assert!(
        narrow_results.is_empty(),
...
fn test_10k_rows_multi_segment() {
    ...
    let full = db
        .columnar_store()
        .query_time_range("large_test", 0, 20_000_000, &[])
        .unwrap();
    assert_eq!(
        full.len(),
        5_000,
        "Full query should return all 5K rows, got {}",
        full.len()
    );
    ...
    let label_results = db
        .columnar_store()
        .query_with_conditions("large_test", 0, 20_000_000, &label_cond, &[])
        .unwrap();
    // label_3 appears in batches 3, 13, 23, ..., 43 → 5 batches × 100 rows = 500
    assert_eq!(
        label_results.len(),
        500,
        "label_3 should match 500 rows, got {}",
        label_results.len()
    );
...
fn test_buffer_query_with_conditions() {
    ...
    let results = db
        .columnar_store()
        .query_with_conditions("buf_test", 0, 200_000, &conditions, &[])
        .unwrap();
    assert_eq!(
        results.len(),
        25,
        "Should return 25 rows with zone=0 from buffer"
    );
...
fn test_segment_v2_metadata() {
    ...
    let results = db
        .columnar_store()
        .query_time_range("meta_test", 0, 2_000_000, &[])
        .unwrap();
...
fn test_column_projection_with_conditions() {
    ...
    for (_rid, row) in &results {
        ...
        assert!(
            !row.contains_key("temperature"),
            "Should NOT have temperature column"
        );
        assert!(
            !row.contains_key("humidity"),
            "Should NOT have humidity column"
        );
...
fn test_empty_result_out_of_range() {
    ...
    let results = db
        .columnar_store()
        .query_time_range("empty_test", 100_000_000, 200_000_000, &[])
        .unwrap();
    ...
    let cond_results = db
        .columnar_store()
        .query_with_conditions("empty_test", 0, 2_000_000, &conditions, &[])
        .unwrap();
    assert!(
        cond_results.is_empty(),
        "Condition for nonexistent zone should be empty"
    );
}
...
fn test_data_integrity_roundtrip() {
    ...
    db.columnar_store()
        .ingest("integrity_test", rows.clone())
        .unwrap();
    db.flush().unwrap();

    let results = db
        .columnar_store()
        .query_time_range("integrity_test", 0, 2_000_000, &[])
        .unwrap();
    ...
    for (i, (_rid, sql_row)) in results.iter().enumerate() {
        ...
        if let Some(Value::Float(hum)) = sql_row.get("humidity") {
            assert!(
                (hum - expected_humidity).abs() < 0.001,
...
fn test_multiple_range_conditions() {
    ...
    let results = db
        .columnar_store()
        .query_with_conditions("multi_cond_test", 0, 2_000_000, &conditions, &[])
        .unwrap();
...
fn test_merge_preserves_indexing() {
    ...
    db.execute("CREATE TABLE merge_idx (ts TIMESTAMP, val FLOAT, tag TEXT) TIMESERIES(ts)")
        .unwrap();
    ...
    let all = db
        .columnar_store()
        .query_time_range("merge_idx", 0, 1_000_000, &[])
        .unwrap();
    assert_eq!(
        all.len(),
        300,
        "All 300 rows should be queryable after merge, got {}",
        all.len()
    );
    ...
    let tagged = db
        .columnar_store()
        .query_with_conditions("merge_idx", 0, 1_000_000, &conditions, &[])
        .unwrap();
    // tag_1 appears in batch 1, 4 → 100 rows
    assert_eq!(
        tagged.len(),
        100,
        "tag_1 should match 100 rows, got {}",
        tagged.len()
    );
...
fn test_crash_recovery_with_indexing() {
    ...
    // Phase 1: Write data
    {
        ...
        db.execute("CREATE TABLE recover_test (ts TIMESTAMP, val FLOAT, tag TEXT) TIMESERIES(ts)")
            .unwrap();
    ...
    // Phase 2: Reopen and verify
    {
        ...
        let results = db
            .columnar_store()
            .query_time_range("recover_test", 0, 2_000_000, &[])
            .unwrap();
        assert_eq!(
            results.len(),
            200,
            "Should recover all 200 rows, got {}",
            results.len()
        );
    }
}
