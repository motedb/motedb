...
fn test_sum_case_conditional() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'b',20),(3,'a',30)").unwrap();
    // SUM(CASE WHEN cat='a' THEN v ELSE 0 END) = 10+0+30 = 40.
    let r = q(&db, "SELECT SUM(CASE WHEN cat = 'a' THEN v ELSE 0 END) FROM t");
...
fn test_having_two_aggregates_and() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'b',5),(4,'b',100)").unwrap();
    // HAVING SUM(v) > 20 AND COUNT(*) >= 2.
    // a: sum=30, count=2 → both true. b: sum=105, count=2 → both true.
    let mut r = q(&db, "SELECT cat FROM t GROUP BY cat HAVING SUM(v) > 20 AND COUNT(*) >= 2");
...
fn test_having_two_aggregates_or() {
    ...
    let r = q(&db, "SELECT cat FROM t GROUP BY cat HAVING SUM(v) > 100 OR COUNT(*) > 1");
...
fn test_subquery_distinct_count() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'b',5),(4,'b',15),(5,'a',30)").unwrap();
...
fn test_group_agg_with_null_values() {
    ...
    let mut r = q(&db, "SELECT cat, SUM(v), COUNT(v), COUNT(*) FROM t GROUP BY cat");
    r.sort_by_key(|row| match &row[0] { Value::Text(s) => s.as_str().to_string(), _ => "zzz".into() });
    // a: SUM=40, COUNT(v)=2, COUNT(*)=3. b: SUM=NULL, COUNT(v)=0, COUNT(*)=1.
    assert_eq!(r, vec![
        vec![Value::Text("a".into()), Value::Integer(40), Value::Integer(2), Value::Integer(3)],
        vec![Value::Text("b".into()), Value::Null, Value::Integer(0), Value::Integer(1)],
    ]);
}
...
fn test_rapid_update_select_loop() {
    ...
    for i in 1..=20 {
        ...
        assert_eq!(r, vec![vec![Value::Integer(i)]], "iter {}: v should be {}", i, i);
    }
}
...
fn test_delete_insert_cycle() {
    ...
    for cycle in 0..3 {
        db.execute(&format!("INSERT INTO t VALUES ({}, {})", cycle + 100, cycle)).unwrap();
    }
...
fn test_multiple_indexes() {
    ...
    db.execute("INSERT INTO t VALUES (1,10,'x'),(2,20,'y'),(3,10,'z')").unwrap();
...
fn test_correlated_select_list_diff_tables() {
    ...
    let mut r = q(&db, "SELECT name, (SELECT COUNT(*) FROM emp WHERE emp.dept_id = dept.id) FROM dept");
    r.sort_by_key(|row| match &row[0] { Value::Text(s) => s.as_str().to_string(), _ => "zzz".into() });
...
fn test_order_by_subquery_result() {
    let (db, _d) = db();
    db.execute("CREATE TABLE dept(id INT PRIMARY KEY, name TEXT)").unwrap();
    db.execute("CREATE TABLE emp(id INT PRIMARY KEY, dept_id INT)").unwrap();
    db.execute("INSERT INTO dept VALUES (1,'a'),(2,'b'),(3,'c')").unwrap();
    db.execute("INSERT INTO emp VALUES (10,1),(11,1),(12,1),(13,2)").unwrap();
    ...
    let names: Vec<String> = r.iter().filter_map(|row| match &row[0] { Value::Text(s) => Some(s.as_str().to_string()), _ => None }).collect();
...
fn test_count_star_vs_count_one_grouped() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',NULL),(3,'b',5)").unwrap();
    let mut r1 = q(&db, "SELECT cat, COUNT(*) FROM t GROUP BY cat");
    r1.sort_by_key(|row| match &row[0] { Value::Text(s) => s.as_str().to_string(), _ => "zzz".into() });
    let mut r2 = q(&db, "SELECT cat, COUNT(1) FROM t GROUP BY cat");
    r2.sort_by_key(|row| match &row[0] { Value::Text(s) => s.as_str().to_string(), _ => "zzz".into() });
