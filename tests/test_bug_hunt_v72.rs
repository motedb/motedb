...
fn test_batch_insert_partial_failure_atomicity() {
    ...
    match res {
        ...
        Err(_) => {
            ...
            assert_eq!(
                r, vec![vec![Value::Integer(0)]],
                "failed batch INSERT should be atomic (0 rows), got {:?}", r
            );
        }
    }
}
...
fn test_batch_insert_duplicate_pk_in_batch() {
    ...
    let r = q(&db, "SELECT COUNT(*) FROM t");
    assert_eq!(r, vec![vec![Value::Integer(0)]], "failed batch should leave 0 rows");
}
...
fn test_drop_table_then_recreate() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY)").unwrap();
    let r = q(&db, "SELECT COUNT(*) FROM t");
    assert_eq!(r, vec![vec![Value::Integer(0)]], "recreated table should be empty");
}
...
fn test_union_column_count_mismatch() {
    ...
    assert!(res.is_err(), "UNION with different column counts should error");
}
...
fn test_union_all_column_count_ok() {
    ...
    let r = q(&db, "SELECT x, y FROM a UNION ALL SELECT x, y FROM b ORDER BY x");
    assert_eq!(
        r,
        vec![vec![Value::Integer(1), Value::Integer(2)], vec![Value::Integer(3), Value::Integer(4)]]
    );
}
...
fn test_large_in_list() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY)").unwrap();
    db.execute("INSERT INTO t VALUES (1),(2),(3),(50),(100)").unwrap();
    // IN with 20 values including some matches.
    let list: Vec<String> = (1..=20).map(|i| i.to_string()).collect();
    let sql = format!("SELECT id FROM t WHERE id IN ({}) ORDER BY id", list.join(","));
    let r = q(&db, &sql);
...
fn test_update_pk_to_same_value_with_other_col() {
    ...
    db.execute("INSERT INTO t VALUES (1,10)").unwrap();
    // SET id = 1 (same), v = 20.
    db.execute("UPDATE t SET id = 1, v = 20 WHERE id = 1").unwrap();
    let r = q(&db, "SELECT id, v FROM t");
...
fn test_delete_insert_same_pk_cycle() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    for cycle in 0..5 {
        db.execute(&format!("INSERT INTO t VALUES (1,{})", cycle * 10)).unwrap();
        let r = q(&db, "SELECT v FROM t WHERE id = 1");
...
fn test_count_distinct_empty_string_vs_null() {
    ...
    db.execute("INSERT INTO t VALUES (1,''),(2,''),(3,NULL),(4,'x')").unwrap();
...
fn test_groupby_empty_string() {
    ...
    db.execute("INSERT INTO t VALUES ('',10),('',20),('x',30)").unwrap();
    let r = q(&db, "SELECT s, SUM(v) FROM t GROUP BY s ORDER BY s");
    assert_eq!(
        r,
        vec![vec![Value::text("".into()), Value::Integer(30)], vec![Value::text("x".into()), Value::Integer(30)]]
    );
}
...
fn test_select_case_on_aggregate() {
    ...
    let r = q(&db, "SELECT CASE WHEN SUM(v) > 25 THEN 'big' ELSE 'small' END FROM t");
...
fn test_aggregate_single_row() {
    ...
    assert_eq!(
        r,
        vec![vec![Value::Integer(1), Value::Integer(42), Value::Float(42.0), Value::Integer(42), Value::Integer(42)]]
    );
}
