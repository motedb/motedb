...
fn bench_report_v020() {
    ...
    // ─── 1. INSERT THROUGHPUT ───
    {
        ...
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Rows inserted      │  {:>12}                            │", n);
        println!("│  Total time         │  {:>12}                           │", format_duration(elapsed));
        println!("│  Throughput         │  {:>12}                         │", format_throughput(n, elapsed));
        println!("│  Avg latency        │  {:>12}                           │", format_latency(elapsed / n as u128));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 2. POINT QUERY (PK) ───
    {
        ...
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Queries            │  {:>12}                            │", queries);
        println!("│  Total time         │  {:>12}                           │", format_duration(elapsed));
        println!("│  QPS                │  {:>12}                         │", format_throughput(queries, elapsed));
        println!("│  Avg latency        │  {:>12}                           │", format_latency(elapsed / queries as u128));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 3. COLUMN INDEX QUERY ───
    {
        ...
        let t = Instant::now();
        for i in 0..queries {
            let _ = exec(&db, &format!("SELECT * FROM t WHERE age = {}", 30 + (i as i64 % 20)));
        }
        let elapsed = t.elapsed().as_micros();
        println!("┌─────────────────────────────────────────────────────────────────┐");
    ...
    // ─── 4. COLUMN INDEX RANGE QUERY ───
    {
        ...
        for i in 0..queries {
            let v = i as i64 % 40;
            let _ = exec(&db, &format!("SELECT * FROM t WHERE val >= {} AND val <= {}", v, v + 10));
        }
        let elapsed = t.elapsed().as_micros();
        println!("┌─────────────────────────────────────────────────────────────────┐");
    ...
    // ─── 5. FULL TABLE SCAN ───
    {
        ...
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Rows scanned       │  {:>12}                            │", rows);
        println!("│  Total time         │  {:>12}                           │", format_duration(elapsed));
        println!("│  Throughput         │  {:>12} rows/s                    │", format_throughput(rows, elapsed));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 6. MIXED CRUD ───
    {
        ...
        for i in 0..ops {
            match i % 10 {
                0..=4 => { let _ = exec(&db, &format!("SELECT * FROM t WHERE id = {}", i)); }
                5..=6 => { let _ = exec(&db, &format!("UPDATE t SET val = {} WHERE id = {}", i * 100, i)); }
                7..=8 => { let _ = exec(&db, &format!("SELECT * FROM t WHERE val = {}", i * 100)); }
                9 => {
                    let rid = n + i;
        ...
        println!("│  Total time         │  {:>12}                           │", format_duration(elapsed));
        println!("│  Throughput         │  {:>12} ops/s                    │", format_throughput(ops, elapsed));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 7. VECTOR SEARCH ───
    {
        ...
        let dim: usize = 64;
        let vec_n: usize = n / 5;
        db.execute(&format!("CREATE TABLE items (id INT, emb VECTOR({}))", dim)).unwrap();

        for i in 0..vec_n {
            let v: Vec<String> = (0..dim).map(|j| format!("{:.4}", ((i * dim + j) as f32 * 0.01).sin())).collect();
            db.execute(&format!("INSERT INTO items (id, emb) VALUES ({}, [{}])", i, v.join(", "))).unwrap();
        }
        ...
        for i in 0..queries {
            ...
            let v: Vec<String> = (0..dim).map(|j| format!("{:.4}", ((offset * dim + j) as f32 * 0.01).sin())).collect();
            let _ = exec(&db, &format!("SELECT id FROM items ORDER BY emb <-> [{}] LIMIT 10", v.join(", ")));
        }
        let elapsed = t.elapsed().as_micros();
        println!("┌─────────────────────────────────────────────────────────────────┐");
        println!("│  7. VECTOR SEARCH — ORDER BY emb <-> [q] LIMIT 10 (dim={})    │", dim);
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Vectors indexed    │  {:>12}                            │", vec_n);
        println!("│  Queries            │  {:>12}                            │", queries);
        println!("│  Total time         │  {:>12}                           │", format_duration(elapsed));
        println!("│  QPS                │  {:>12}                         │", format_throughput(queries, elapsed));
        println!("│  Avg latency        │  {:>12}                           │", format_latency(elapsed / queries.max(1) as u128));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 8. FULL-TEXT SEARCH ───
    {
        ...
        for i in 0..queries {
            ...
            let _ = exec(&db, &format!("SELECT id FROM docs WHERE MATCH(content, '{}') ORDER BY id", term));
        }
        let elapsed = t.elapsed().as_micros();
        println!("┌─────────────────────────────────────────────────────────────────┐");
    ...
    // ─── 9. SPATIAL QUERY ───
    {
        ...
        db.execute("CREATE TABLE points (id INT, location GEOMETRY)").unwrap();

        let sp_n: usize = n / 2;
        for i in 0..sp_n {
            let x = (i as f64 * 0.001) % 100.0;
            let y = (i as f64 * 0.0017) % 100.0;
            db.execute(&format!("INSERT INTO points (id, location) VALUES ({}, POINT({:.4}, {:.4}))", i, x, y)).unwrap();
        }
    ...
    // ─── 10. WAL RECOVERY ───
    {
        ...
        println!("┌─────────────────────────────────────────────────────────────────┐");
        println!("│  10. WAL RECOVERY — REOPEN AFTER {} ROW INSERT             │", n);
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Rows verified      │  {:>12}                            │", count);
        println!("│  Recovery time      │  {:>12}                           │", format_duration(elapsed));
        println!("│  Throughput         │  {:>12} rows/s                    │", format_throughput(n, elapsed));
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
    ...
    // ─── 11. PREPARED STATEMENT vs RAW SQL ───
    {
        ...
        println!("├─────────────────────┬───────────────────────────────────────────┤");
        println!("│  Queries            │  {:>12}                            │", queries);
        println!("│  Raw SQL            │  {:>12}  ({:>10})           │", format_duration(elapsed_raw), format_throughput(queries, elapsed_raw));
        println!("│  Prepared stmt      │  {:>12}  ({:>10})           │", format_duration(elapsed_prep), format_throughput(queries, elapsed_prep));
        println!("│  Speedup            │  {:>12.1}x                            │", speedup);
        println!("└─────────────────────┴───────────────────────────────────────────┘\n");
        db.close().unwrap();
    }
