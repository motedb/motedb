...
fn test_where_complex_and_or() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT, c TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,5,'x'),(2,20,5,'y'),(3,10,15,'x'),(4,30,5,'z')")
        .unwrap();
    // (a > 5 AND b = 5) OR c = 'x'
    // id1: a10>5,b5 ✓ ; id2: a20>5,b5 ✓ ; id3: c='x' ✓ ; id4: a30>5,b5 ✓
    let r = q(
        &db,
        "SELECT id FROM t WHERE (a > 5 AND b = 5) OR c = 'x' ORDER BY id",
    );
    assert_eq!(
        r.iter()
            .map(|row| match &row[0] {
...
fn test_where_function_and_comparison() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'alice',30),(2,'bob',15),(3,'carol',40)")
        .unwrap();
    // LENGTH(s) > 3 AND v > 20
    let r = q(
        &db,
        "SELECT id FROM t WHERE LENGTH(s) > 3 AND v > 20 ORDER BY id",
    );
...
fn test_where_nested_parens() {
    ...
    let r = q(
        &db,
        "SELECT id FROM t WHERE ((a = 1 AND b = 1) OR (a = 2 AND b = 2)) ORDER BY id",
    );
    assert_eq!(
        r.iter()
            .map(|row| match &row[0] {
...
fn test_where_not_with_parens() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,1,1),(2,1,2),(3,2,1)")
        .unwrap();
    // NOT (a = 1 AND b = 1) → ids 2, 3
    let r = q(
        &db,
        "SELECT id FROM t WHERE NOT (a = 1 AND b = 1) ORDER BY id",
    );
    assert_eq!(
        r.iter()
            .map(|row| match &row[0] {
...
fn test_order_by_mixed_types_consistency() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'banana'),(2,'apple'),(3,'cherry'),(4,'apple')")
        .unwrap();
...
fn test_persist_mixed_ops_reopen() {
    ...
    let path = dir.path().to_path_buf();
    {
        ...
        db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT, s TEXT)")
            .unwrap();
        db.execute("INSERT INTO t VALUES (1,10,'a'),(2,20,'b'),(3,30,'c')")
            .unwrap();
...
fn test_join_filter_aggregate() {
    let (db, _d) = db();
    db.execute("CREATE TABLE orders(id INT PRIMARY KEY, cust INT, amt INT, status TEXT)")
        .unwrap();
    db.execute("CREATE TABLE cust(id INT PRIMARY KEY, name TEXT)")
        .unwrap();
    db.execute("INSERT INTO orders VALUES (1,1,100,'ok'),(2,1,200,'pending'),(3,2,50,'ok')")
        .unwrap();
    db.execute("INSERT INTO cust VALUES (1,'Alice'),(2,'Bob')")
        .unwrap();
...
fn test_groupby_count_orderby_count() {
    ...
    db.execute("INSERT INTO t VALUES (1),(1),(1),(2),(2),(3)")
        .unwrap();
    let r = q(
        &db,
        "SELECT g, COUNT(*) AS c FROM t GROUP BY g ORDER BY c DESC, g ASC",
    );
...
fn test_aggregate_empty_where_result() {
    ...
    let r = q(
        &db,
        "SELECT COUNT(*), SUM(v), AVG(v), MIN(v), MAX(v) FROM t WHERE v > 100",
    );
    assert_eq!(
        r,
        vec![vec![
