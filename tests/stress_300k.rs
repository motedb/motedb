...
fn select_int(db: &Database, sql: &str) -> Option<i64> {
    match exec(db, sql) {
        QueryResult::Select { rows, .. } => rows.first().and_then(|row| {
...
fn count_filtered(db: &Database, table: &str, filter: &str) -> i64 {
    select_int(
        db,
        &format!("SELECT COUNT(*) AS cnt FROM {} WHERE {}", table, filter),
    )
    .unwrap_or(0)
}
...
fn print_elapsed(name: &str, ops: usize, ms: u64) {
    ...
    println!(
        "{:<60} | {:>6} ops | {:>8.1} ms | {:>7.1} µs/op | {:>10.0} ops/s",
        name, ops, ms, us, tps
    );
}
...
fn test_300k_insert_integrity() {
    ...
    exec(
        &db,
        "CREATE TABLE t1 (id INTEGER PRIMARY KEY, name TEXT, score FLOAT, age INTEGER, city TEXT)",
    );

    // Use 50K for CI reliability, 300K for local stress testing
    let n = if std::env::var("CI").is_ok() {
        50_000
    } else {
        N
    };
    ...
    assert_eq!(
        count_filtered(&db, "t1", &format!("id > {}", near_end)),
        1000
    );
    assert_eq!(
        count_filtered(&db, "t1", &format!("id >= {}", near_end + 1)),
        1000
    );
    assert_eq!(
        count_filtered(&db, "t1", &format!("id > {}", near_end + 1)),
    ...
    println!(
        "  ✓ INSERT integrity: COUNT={}, first/mid/last/WHERE verified",
        cnt
    );
}
...
fn test_300k_update_correctness() {
    ...
    exec(
        &db,
        "CREATE TABLE t2 (id INTEGER PRIMARY KEY, val TEXT, counter INTEGER)",
    );
    ...
    for i in 1..=M as i64 {
        exec(
            &db,
            &format!("INSERT INTO t2 VALUES ({}, 'original', 0)", i),
        );
    }
    ...
    for i in (2..=M as i64).step_by(2) {
        exec(
            &db,
            &format!(
                "UPDATE t2 SET val = 'updated', counter = {} WHERE id = {}",
                i * 10,
                i
            ),
        );
    ...
    print_elapsed(
        &format!("UPDATE {} rows (even ids)", update_count),
        update_count,
        t.elapsed().as_millis() as u64,
    );
...
fn test_300k_delete_correctness() {
    ...
    print_elapsed(
        &format!("DELETE {} rows (every 3rd)", delete_count),
        delete_count,
        t.elapsed().as_millis() as u64,
    );
    ...
    assert_eq!(
        after,
        (M - delete_count) as i64,
        "after DELETE: expected {}, got {}",
        M - delete_count,
        after
    );
    ...
    for i in (3..=300i64).step_by(3) {
        assert!(
            select_row(&db, &format!("SELECT * FROM t3 WHERE id = {}", i)).is_none(),
    ...
    println!(
        "  ✓ DELETE correctness: {} deleted, {} remaining",
        delete_count, after
    );
}
...
fn test_300k_pk_lookup_consistency() {
    ...
    exec(
        &db,
        "CREATE TABLE t4 (id INTEGER PRIMARY KEY, payload TEXT, value INTEGER)",
    );
    ...
    for i in 1..=M as i64 {
        exec(
            &db,
            &format!("INSERT INTO t4 VALUES ({}, 'p_{}', {})", i, i, i * 7),
        );
    }
    print_elapsed(
        &format!("INSERT {} rows", M),
        M,
        t.elapsed().as_millis() as u64,
    );
    ...
    println!(
        "  ✓ PK lookup: {} sampled IDs match before/after flush",
        samples.len()
    );
}
...
fn test_300k_where_filter_correctness() {
    ...
    exec(
        &db,
        "CREATE TABLE t5 (id INTEGER PRIMARY KEY, category INTEGER, amount INTEGER)",
    );
    ...
    for i in 1..=M as i64 {
        exec(
            &db,
            &format!("INSERT INTO t5 VALUES ({}, {}, {})", i, i % 10, i * 3),
        );
    }
...
fn test_300k_mixed_crud_correctness() {
    ...
    exec(
        &db,
        "CREATE TABLE t6 (id INTEGER PRIMARY KEY, status TEXT, version INTEGER)",
    );
    ...
    for i in 1..=half as i64 {
        exec(
            &db,
            &format!(
                "UPDATE t6 SET status = 'inactive', version = 2 WHERE id = {}",
                i
            ),
        );
    }
    print_elapsed(
        &format!("UPDATE {} rows", half),
        half,
        t.elapsed().as_millis() as u64,
    );
    assert_eq!(
    ...
    let r_q = select_row(
        &db,
        &format!("SELECT * FROM t6 WHERE id = {}", quarter as i64 + 1),
    )
    .unwrap();
    assert_eq!(r_q[1], Value::text("inactive".into()));

    println!(
        "  ✓ Mixed CRUD: insert={}, update={}, delete={}, final={}",
        M, half, quarter, expected
    );
}
...
fn test_300k_flush_consistency() {
    ...
    exec(
        &db,
        "CREATE TABLE t7 (id INTEGER PRIMARY KEY, data TEXT, num INTEGER)",
    );
    ...
    for i in 1..=M as i64 {
        exec(
            &db,
            &format!("INSERT INTO t7 VALUES ({}, 'd_{}', {})", i, i, i),
        );
    }
    // UPDATE 每 7 行
    ...
    let samples_before: Vec<(i64, Option<Vec<Value>>)> = sample_ids
        .iter()
        .map(|&id| {
            (
                id,
                select_row(&db, &format!("SELECT * FROM t7 WHERE id = {}", id)),
            )
        })
        .collect();
    ...
    assert_eq!(
        ...
        "COUNT mismatch before/after flush"
    );
    ...
    println!(
        "  ✓ Flush consistency: count={}, {} samples verified",
        count_before,
        sample_ids.len()
    );
}
...
fn test_300k_restart_durability() {
    ...
    // Phase 1: Write (scoped so db is fully dropped before reopen)
    {
        ...
        exec(
            &db,
            "CREATE TABLE t8 (id INTEGER PRIMARY KEY, val TEXT, ts INTEGER)",
        );
        ...
        print_elapsed(
            &format!("INSERT {} rows (pre-restart)", M),
            M,
            t.elapsed().as_millis() as u64,
        );
        ...
        for i in (1..=100i64).step_by(3) {
            exec(
                &db,
                &format!("UPDATE t8 SET val = 'modified' WHERE id = {}", i),
            );
        }
        ...
        assert_eq!(
            ...
            "id=1 should be modified before close"
        );
        // id=2 没改
        ...
        assert_eq!(
            r2_before[1],
            Value::text("v_2".into()),
            "id=2 should be v_2 before close"
        );
    ...
    assert_eq!(
        r1[1],
        Value::text("modified".into()),
        "id=1 should retain 'modified' after restart"
    );
    ...
    assert_eq!(
        r2[1],
        Value::text("v_2".into()),
        "id=2 should be v_2 after restart"
    );
    ...
    println!(
        "  ✓ Restart durability: count={}, id=1 modified verified, 6 random rows verified",
        count_after
    );
}
