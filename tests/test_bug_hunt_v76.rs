...
fn test_min_ignores_null() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,5),(2,NULL),(3,10)")
        .unwrap();
...
fn test_self_join() {
    ...
    db.execute("INSERT INTO emp VALUES (1,'alice',NULL),(2,'bob',1),(3,'carol',1)")
        .unwrap();
    let r = q(
        &db,
...
fn test_having_sum_filter() {
    ...
    let mut r = q(
        &db,
        "SELECT cat, SUM(amt) FROM t GROUP BY cat HAVING SUM(amt) > 10",
    );
    r.sort_by_key(|row| match &row[0] {
        Value::Text(s) => s.as_str().to_string(),
        _ => "zzz".into(),
...
fn test_nested_subquery() {
    ...
    let mut r = q(
        &db,
        "SELECT id FROM t WHERE id IN (SELECT id FROM t WHERE v > (SELECT MIN(v) FROM t))",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
...
fn test_semi_join_via_in_subquery() {
    ...
    db.execute("CREATE TABLE orders(id INT PRIMARY KEY, cust_id INT)")
        .unwrap();
    db.execute("CREATE TABLE customers(id INT PRIMARY KEY)")
        .unwrap();
    ...
    let mut r = q(
        &db,
        "SELECT id FROM customers WHERE id IN (SELECT cust_id FROM orders)",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
...
fn test_offset_beyond_rows() {
    ...
    assert_eq!(
        r,
        Vec::<Vec<Value>>::new(),
        "OFFSET beyond row count must return empty"
    );
}
...
fn test_and_or_precedence() {
    ...
    db.execute("INSERT INTO t VALUES (1,1,1),(2,2,3),(3,2,1),(4,1,3)")
        .unwrap();
