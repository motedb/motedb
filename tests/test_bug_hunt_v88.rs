...
fn test_timestamp_insert_select() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, '2024-01-15T10:30:00')")
        .unwrap();
...
fn test_timestamp_where_equality() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, '2024-01-15T10:30:00'),(2, '2024-06-01T08:00:00')")
        .unwrap();
...
fn test_timestamp_in_list() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE ts IN ('2024-01-15T10:30:00', '2024-06-01T08:00:00')",
    ));
    assert_eq!(r, vec![1, 2]);
}
...
fn test_extract_month() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, '2024-03-15T10:30:00')")
        .unwrap();
...
fn test_extract_day_hour() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, '2024-03-15T10:30:45')")
        .unwrap();
...
fn test_aggregate_zero_rows_filtered() {
    ...
    assert_eq!(
        q(&db, "SELECT COUNT(*) FROM t WHERE v > 100"),
        vec![vec![Value::Integer(0)]]
    );
    assert_eq!(
        q(&db, "SELECT SUM(v) FROM t WHERE v > 100"),
        vec![vec![Value::Null]]
    );
    assert_eq!(
        q(&db, "SELECT MIN(v) FROM t WHERE v > 100"),
        vec![vec![Value::Null]]
    );
    assert_eq!(
        q(&db, "SELECT MAX(v) FROM t WHERE v > 100"),
        vec![vec![Value::Null]]
    );
    assert_eq!(
        q(&db, "SELECT AVG(v) FROM t WHERE v > 100"),
        vec![vec![Value::Null]]
    );
}
...
fn test_group_by_order_by_key() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'c',1),(2,'a',2),(3,'b',3)")
        .unwrap();
    let r = q(
        &db,
...
fn test_count_distinct_subquery_text() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'x'),(2,'y'),(3,'x'),(4,'z'),(5,'y')")
        .unwrap();
    let r = q(
        &db,
        "SELECT COUNT(*) FROM (SELECT DISTINCT s FROM t) AS sub",
    );
    assert_eq!(r, vec![vec![Value::Integer(3)]]);
}
...
fn test_between_with_negative_bounds() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,-50),(2,-10),(3,0),(4,10)")
        .unwrap();
...
fn test_update_multi_col_expressions() {
    ...
    db.execute("UPDATE t SET a = a + 100, b = b * 2 WHERE id = 1")
        .unwrap();
