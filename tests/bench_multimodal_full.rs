...
fn bench_vector_knn_50k() {
    ...
    timed(&format!("INSERT {} vectors ({}-dim)", n, dim), || {
        ...
        for start in (0..n).step_by(batch) {
            ...
            for i in start..end {
                if i > start { sql.push(','); }
                let cat = match i % 4 { 0 => "electronics", 1 => "clothing", 2 => "books", _ => "food" };
                // Generate pseudo-random vector
                let vec: Vec<String> = (0..dim).map(|d| format!("{:.4}", ((i * 7 + d * 13) as f64).sin() * 0.5 + 0.5)).collect();
                sql.push_str(&format!("([{}],'{}')", vec.join(","), cat));
            }
            match db.execute(&sql) { Ok(_) => {}, Err(e) => panic!("batch INSERT failed: {:?}", e) }
        }
    });
    ...
    let query_vec: Vec<String> = (0..dim).map(|d| format!("{:.4}", (d as f64 * 0.1).sin() * 0.5 + 0.5)).collect();
    let knn_sql = format!(
    ...
    timed("ORDER BY distance LIMIT 10 (x5)", || {
        for _ in 0..5 {
            ...
            assert!(matches!(&r, motedb::QueryResult::Select { rows, .. } if !rows.is_empty()), "should find nearest vectors");
        }
    });
        ...
        for _ in 0..5 {
            ...
            if let motedb::QueryResult::Select { rows, .. } = r {
                for row in &rows {
                    ...
                    assert_eq!(row.iter().filter(|v| matches!(v, motedb::types::Value::Text(_))).count(), 0);
                }
            }
        }
    });
...
fn bench_spatial_50k() {
    ...
    timed(&format!("INSERT {} spatial points", n), || {
        ...
        for start in (0..n).step_by(batch) {
            ...
            for i in start..end {
                ...
                sql.push_str(&format!("(POINT({:.1},{:.1}),'city_{}',{})", lon, lat, i % 1000, pop));
            }
            match db.execute(&sql) { Ok(_) => {}, Err(e) => panic!("batch INSERT failed: {:?}", e) }
        }
    });
    ...
    timed("ST_DISTANCE ORDER BY LIMIT 10 (x10)", || {
        for _ in 0..10 {
            ...
            assert!(matches!(&r, motedb::QueryResult::Select { rows, .. } if !rows.is_empty()), "should find nearest places");
        }
    });
...
fn bench_text_fts_50k() {
    ...
    timed(&format!("INSERT {} documents", n), || {
        ...
        for start in (0..n).step_by(batch) {
            ...
            for i in start..end {
                ...
                let cat = match i % 3 { 0 => "tech", 1 => "science", _ => "news" };
                sql.push_str(&format!("('{} guide {}','{} and {} for edge computing','{}')", w1, i, w1, w2, cat));
            }
            match db.execute(&sql) { Ok(_) => {}, Err(e) => panic!("batch INSERT failed: {:?}", e) }
        }
    });
    ...
    timed("FTS 'database' top-10 (x10)", || {
        for _ in 0..10 {
            let r = db.execute("SELECT id FROM docs WHERE MATCH(body, 'database') LIMIT 10")
                .unwrap().materialize().unwrap();
        }
    });
    ...
    timed("LIKE prefix scan 'robot%' (x5)", || {
        for _ in 0..5 {
            let r = db.execute("SELECT id FROM docs WHERE title LIKE 'robot%' LIMIT 10")
                .unwrap().materialize().unwrap();
        }
    });
...
fn bench_mixed_multimodal_20k() {
    ...
    timed(&format!("INSERT {} multimodal items", n), || {
        ...
        for start in (0..n).step_by(batch) {
            ...
            for i in start..end {
                if i > start { sql.push(','); }
                let vec: Vec<String> = (0..64).map(|d| format!("{:.3}", ((i + d) as f64 * 0.1).sin())).collect();
                ...
                sql.push_str(&format!(
                    "([{}],POINT({:.1},{:.1}),'item info {}',{},'{}')",
                    vec.join(","), lon, lat, i, price, region
                ));
            }
            match db.execute(&sql) { Ok(_) => {}, Err(e) => panic!("batch INSERT failed: {:?}", e) }
        }
    ...
    // Create all indexes
    timed("CREATE multimodal indexes (vector + spatial + text + column)", || {
    ...
    let qvec: Vec<String> = (0..64).map(|d| format!("{:.3}", (d as f64 * 0.1).sin())).collect();
    ...
    timed("GROUP BY region COUNT + AVG(price)", || {
        let _r = db.execute("SELECT region, COUNT(*), AVG(price) FROM items GROUP BY region")
            .unwrap().materialize().unwrap();
    });
...
fn test_vector_insert_and_knn() {
    let (db, _dir) = edge_db();
    db.execute("CREATE TABLE vecs (id INT PRIMARY KEY AUTO_INCREMENT, embedding VECTOR(4), label TEXT)").unwrap();

    db.execute("INSERT INTO vecs (embedding, label) VALUES ([1.0, 0.0, 0.0, 0.0], 'x-axis')").unwrap();
    db.execute("INSERT INTO vecs (embedding, label) VALUES ([0.0, 1.0, 0.0, 0.0], 'y-axis')").unwrap();
    db.execute("INSERT INTO vecs (embedding, label) VALUES ([0.0, 0.0, 1.0, 0.0], 'z-axis')").unwrap();
    db.execute("INSERT INTO vecs (embedding, label) VALUES ([1.0, 1.0, 0.0, 0.0], 'xy-plane')").unwrap();

    // Query nearest to [1,0,0,0] — should be 'x-axis' (distance 0) then 'xy-plane' (distance ~0.3)
    let r = db.execute("SELECT label FROM vecs ORDER BY embedding <-> [1.0, 0.0, 0.0, 0.0] LIMIT 2")
        .unwrap().materialize().unwrap();
    if let motedb::QueryResult::Select { rows, .. } = r {
...
fn test_spatial_insert_and_distance() {
    ...
    let r = db.execute("SELECT id FROM pts ORDER BY ST_DISTANCE(loc, 0, 0) LIMIT 2")
        .unwrap().materialize().unwrap();
    if let motedb::QueryResult::Select { rows, .. } = r {
...
fn test_multimodal_table_all_types() {
    ...
    let r = db.execute("SELECT COUNT(*) FROM mm").unwrap().materialize().unwrap();
    if let motedb::QueryResult::Select { rows, .. } = r {
        assert_eq!(rows.len(), 1, "COUNT should return 1 row");
    }

    // WHERE filter on scalar column + text
    let r = db.execute("SELECT id FROM mm WHERE cat = 'A'").unwrap().materialize().unwrap();
    if let motedb::QueryResult::Select { rows, .. } = r {
        assert_eq!(rows.len(), 1, "should find 1 row with cat='A'");
    }

    // LIKE on text
    let r = db.execute("SELECT id FROM mm WHERE txt LIKE 'hello%'").unwrap().materialize().unwrap();
    if let motedb::QueryResult::Select { rows, .. } = r {
...
fn test_p99_multimodal_queries() {
    ...
    for start in (0..n).step_by(batch) {
        ...
        for i in start..end {
            if i > start { sql.push(','); }
            let vec: Vec<String> = (0..32).map(|d| format!("{:.2}", ((i + d) as f64 * 0.1).sin())).collect();
            let lat = 30.0 + (i as f64 * 0.01) % 30.0;
            let lon = -120.0 + (i as f64 * 0.01) % 60.0;
            let region = if i % 3 == 0 { "US" } else { "EU" };
            sql.push_str(&format!("([{}],POINT({:.1},{:.1}),'sensor_{}','{}')", vec.join(","), lon, lat, i, region));
        }
        match db.execute(&sql) { Ok(_) => {}, Err(e) => panic!("batch INSERT failed: {:?}", e) }
    }
    ...
    let qvec: Vec<String> = (0..32).map(|d| format!("{:.2}", (d as f64 * 0.1).sin())).collect();

    let queries: Vec<(&str, String)> = vec![
        ("PK point", "SELECT * FROM sensors WHERE id = 5000".to_string()),
        ("WHERE region", "SELECT * FROM sensors WHERE region = 'US' LIMIT 10".to_string()),
        ("COUNT WHERE", "SELECT COUNT(*) FROM sensors WHERE region = 'US'".to_string()),
        ("GROUP BY", "SELECT region, COUNT(*) FROM sensors GROUP BY region".to_string()),
        ("Vector KNN", format!("SELECT id FROM sensors ORDER BY emb <-> [{}] LIMIT 5", qvec.join(","))),
        ("Spatial dist", "SELECT id FROM sensors ORDER BY ST_DISTANCE(loc, -100.0, 40.0) LIMIT 5".to_string()),
    ];
