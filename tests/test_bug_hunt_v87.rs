...
fn test_cast_float_to_int_truncates() {
    ...
    assert_eq!(
        ...
        "CAST 3.7 AS INT truncates to 3"
    );
}
...
fn test_order_by_tie_secondary() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (3,1,1),(1,1,3),(2,1,2)")
        .unwrap();
...
fn test_where_col_vs_col_arith() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT, c INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,3,4,7),(2,1,2,5),(3,2,3,4)")
        .unwrap();
...
fn test_group_sum_expression() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, qty INT, price INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',2,10),(2,'a',3,5),(3,'b',1,20)")
        .unwrap();
...
fn test_where_empty_string() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,''),(2,'x'),(3,'')")
        .unwrap();
...
fn test_order_by_pk() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (3,30),(1,10),(2,20)")
        .unwrap();
...
fn test_having_count_star() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a'),(2,'a'),(3,'b'),(4,'c'),(5,'c'),(6,'c')")
        .unwrap();
    // Groups with COUNT(*) >= 3.
...
fn test_multi_join_with_where() {
    ...
    db.execute("CREATE TABLE b(id INT PRIMARY KEY, aid INT, y INT)")
        .unwrap();
    db.execute("CREATE TABLE c(id INT PRIMARY KEY, bid INT, z INT)")
        .unwrap();
    ...
    let r = q(
        &db,
        "SELECT a.x, b.y, c.z FROM a JOIN b ON a.id = b.aid JOIN c ON b.id = c.bid WHERE a.x > 50",
    );
    assert_eq!(
        r,
        vec![vec![
...
fn test_self_join_hierarchy() {
    ...
    db.execute("CREATE TABLE emp(id INT PRIMARY KEY, name TEXT, mgr_id INT)")
        .unwrap();
    db.execute("INSERT INTO emp VALUES (1,'ceo',NULL),(2,'vp1',1),(3,'vp2',1),(4,'eng',2)")
        .unwrap();
    ...
    let ceo_row = r
        .iter()
        .find(|row| matches!(&row[0], Value::Text(s) if s.as_str() == "ceo"))
        .unwrap();
