...
fn test_point3d_literal_insert() {
    ...
    exec(&db, "CREATE TABLE lidar (id INTEGER PRIMARY KEY, pt GEOMETRY)");
...
fn test_point3d_and_point_2d_coexist() {
    ...
    exec(&db, "CREATE TABLE mixed (id INTEGER PRIMARY KEY, pt GEOMETRY)");
...
fn test_create_octree_index_basic() {
    ...
    exec(&db, "CREATE TABLE cloud (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    for i in 1..=20i64 {
        let x = i as f64;
        let y = (i + 10) as f64;
        let z = (i + 20) as f64;
        exec(&db, &format!("INSERT INTO cloud VALUES ({}, POINT3D({}, {}, {}))", i, x, y, z));
    }

    exec(&db, "CREATE OCTREE INDEX cloud_pt ON cloud(pt)");

    // KNN query
    let r = rows(&db, "SELECT id FROM cloud WHERE ST_KNN_3D(pt, 1.0, 11.0, 21.0, 3)");
...
fn test_octree_index_with_prefix_syntax() {
    ...
    exec(&db, "CREATE TABLE pts (id INTEGER PRIMARY KEY, location GEOMETRY)");
    ...
    let r = rows(&db, "SELECT id FROM pts WHERE ST_KNN_3D(location, 0.0, 0.0, 0.0, 1)");
    assert_eq!(r.len(), 1);
}
...
fn test_st_within_3d() {
    ...
    exec(&db, "CREATE TABLE voxels (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    for i in 0..10i64 {
        let v = i as f64;
        exec(&db, &format!("INSERT INTO voxels VALUES ({}, POINT3D({}, {}, {}))", i + 1, v, v, v));
    }

    exec(&db, "CREATE OCTREE INDEX voxels_pt ON voxels(pt)");

    // Points within [0,0,0] to [4,4,4] — should match id 1..5
    let r = rows(&db, "SELECT id FROM voxels WHERE ST_WITHIN_3D(pt, 0.0, 0.0, 0.0, 4.0, 4.0, 4.0)");
    assert_eq!(r.len(), 5, "Should find 5 points in range [0,4]");

    // Narrower range [0,0,0] to [2,2,2] — should match id 1..3
    let r2 = rows(&db, "SELECT id FROM voxels WHERE ST_WITHIN_3D(pt, 0.0, 0.0, 0.0, 2.0, 2.0, 2.0)");
...
fn test_st_knn_3d_accuracy() {
    ...
    exec(&db, "CREATE TABLE scan (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    ...
    exec(&db, "INSERT INTO scan VALUES (4, POINT3D(10.0, 10.0, 10.0))");
    ...
    let r = rows(&db, "SELECT id FROM scan WHERE ST_KNN_3D(pt, 0.0, 0.0, 0.0, 3)");
    assert_eq!(r.len(), 3);
...
fn test_st_radius_3d() {
    ...
    exec(&db, "CREATE TABLE radar (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    ...
    let r = rows(&db, "SELECT id FROM radar WHERE ST_RADIUS_3D(pt, 0.0, 0.0, 0.0, 1.1)");
    assert_eq!(r.len(), 3, "Radius 1.1 should find 3 points");

    // Radius 2.0 from origin — should match 1, 2, 3, 5
    let r2 = rows(&db, "SELECT id FROM radar WHERE ST_RADIUS_3D(pt, 0.0, 0.0, 0.0, 2.0)");
...
fn test_order_by_st_distance_3d() {
    ...
    exec(&db, "CREATE TABLE points3d (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    exec(&db, "INSERT INTO points3d VALUES (1, POINT3D(0.0, 0.0, 0.0))");
    exec(&db, "INSERT INTO points3d VALUES (2, POINT3D(1.0, 1.0, 1.0))");
    exec(&db, "INSERT INTO points3d VALUES (3, POINT3D(5.0, 5.0, 5.0))");
    exec(&db, "INSERT INTO points3d VALUES (4, POINT3D(10.0, 10.0, 10.0))");

    exec(&db, "CREATE OCTREE INDEX points3d_pt ON points3d(pt)");

    let r = rows(&db, "SELECT id FROM points3d ORDER BY ST_DISTANCE_3D(pt, 0.0, 0.0, 0.0) LIMIT 2");
    assert_eq!(r.len(), 2);
...
fn test_insert_delete_sync() {
    ...
    exec(&db, "CREATE TABLE dyn (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    ...
    // KNN before delete
    let r = rows(&db, "SELECT id FROM dyn WHERE ST_KNN_3D(pt, 0.0, 0.0, 0.0, 2)");
    assert_eq!(r.len(), 2);
    ...
    let r2 = rows(&db, "SELECT id FROM dyn WHERE ST_KNN_3D(pt, 0.0, 0.0, 0.0, 2)");
    assert_eq!(r2.len(), 1);
    let id = match &r2[0][0] {
        Value::Integer(i) => *i,
        _ => panic!("Expected integer"),
...
fn test_update_sync() {
    ...
    exec(&db, "CREATE TABLE movable (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    exec(&db, "CREATE OCTREE INDEX movable_pt ON movable(pt)");

    exec(&db, "INSERT INTO movable VALUES (1, POINT3D(0.0, 0.0, 0.0))");

    // Before update — closest to (0,0,0)
    let r1 = rows(&db, "SELECT id FROM movable WHERE ST_KNN_3D(pt, 0.0, 0.0, 0.0, 1)");
    assert_eq!(r1.len(), 1);

    // Move the point far away
    exec(&db, "UPDATE movable SET pt = POINT3D(100.0, 100.0, 100.0) WHERE id = 1");

    // After update — closest to (0,0,0) should still be id=1 but at distance 100*sqrt(3)
    let r2 = rows(&db, "SELECT id FROM movable WHERE ST_KNN_3D(pt, 0.0, 0.0, 0.0, 1)");
    assert_eq!(r2.len(), 1);

    // Radius search should NOT find it near origin
    let r3 = rows(&db, "SELECT id FROM movable WHERE ST_RADIUS_3D(pt, 0.0, 0.0, 0.0, 10.0)");
    assert_eq!(r3.len(), 0, "Moved point should not be near origin");

    // But should find it near (100,100,100)
    let r4 = rows(&db, "SELECT id FROM movable WHERE ST_RADIUS_3D(pt, 100.0, 100.0, 100.0, 1.0)");
...
fn test_flush_and_reopen() {
    ...
    {
        let db = Database::create(dir.path()).expect("create db");
        exec(&db, "CREATE TABLE persistent (id INTEGER PRIMARY KEY, pt GEOMETRY)");
        for i in 1..=10i64 {
            let v = i as f64;
            exec(&db, &format!("INSERT INTO persistent VALUES ({}, POINT3D({}, {}, {}))", i, v, v, v));
        }
...
fn test_2d_3d_coexistence() {
    ...
    exec(&db, "CREATE TABLE geo (id INTEGER PRIMARY KEY, location GEOMETRY, cloud GEOMETRY)");
    ...
    // 2D query
    let r2d = rows(&db, "SELECT id FROM geo WHERE ST_KNN(location, 3.0, 3.0, 3)");
    assert_eq!(r2d.len(), 3, "2D KNN should work");

    // 3D query
    let r3d = rows(&db, "SELECT id FROM geo WHERE ST_KNN_3D(cloud, 6.0, 6.0, 6.0, 3)");
...
fn test_drop_octree_index() {
    ...
    exec(&db, "CREATE TABLE droptest (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    exec(&db, "INSERT INTO droptest VALUES (1, POINT3D(1.0, 2.0, 3.0))");
    exec(&db, "CREATE OCTREE INDEX droptest_pt ON droptest(pt)");

    // Verify index works
    let r = rows(&db, "SELECT id FROM droptest WHERE ST_KNN_3D(pt, 1.0, 2.0, 3.0, 1)");
    assert_eq!(r.len(), 1);
    ...
    let r2 = rows(&db, "SELECT id FROM droptest WHERE ST_KNN_3D(pt, 1.0, 2.0, 3.0, 1)");
...
fn test_ioctree_1000_points() {
    ...
    exec(&db, "CREATE TABLE large_cloud (id INTEGER PRIMARY KEY, pt GEOMETRY)");
    ...
    let r = rows(&db, "SELECT id FROM large_cloud WHERE ST_KNN_3D(pt, 5.0, 5.0, 5.0, 5)");
    assert_eq!(r.len(), 5);

    // Range — points in [0,0,0]-[3,3,3]
    let r2 = rows(&db, "SELECT id FROM large_cloud WHERE ST_WITHIN_3D(pt, 0.0, 0.0, 0.0, 3.0, 3.0, 3.0)");
    ...
    let r3 = rows(&db, "SELECT id FROM large_cloud WHERE ST_RADIUS_3D(pt, 5.0, 5.0, 5.0, 1.74)");
