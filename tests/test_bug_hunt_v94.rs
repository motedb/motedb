...
fn test_order_by_case_column() {
    ...
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY CASE WHEN v > 15 THEN 0 ELSE 1 END ASC, id ASC",
    );
    let ids: Vec<i64> = r
        .iter()
...
fn test_order_by_case_multi_col() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,5,0),(2,0,5),(3,5,5)")
        .unwrap();
    // ORDER BY CASE WHEN a > b THEN a WHEN b > a THEN b ELSE 0 END.
    // id1: a=5>b=0 → a=5. id2: b=5>a=0 → b=5. id3: a=b → 0. ASC: id3(0),id1(5),id2(5).
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY CASE WHEN a > b THEN a WHEN b > a THEN b ELSE 0 END ASC, id ASC",
    );
    let ids: Vec<i64> = r
        .iter()
        .filter_map(|row| match row[0] {
...
fn test_case_returns_column() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,20),(2,30,40)")
        .unwrap();
    // CASE WHEN a > b THEN a ELSE b → returns the larger.
    let r = q(
        &db,
        "SELECT CASE WHEN a > b THEN a ELSE b END FROM t ORDER BY id",
    );
...
fn test_simple_case_column() {
    ...
    let r = q(
        &db,
        "SELECT CASE cat WHEN 'a' THEN 1 WHEN 'b' THEN 2 ELSE 0 END FROM t ORDER BY id",
    );
    assert_eq!(
        r,
        vec![
...
fn test_sum_of_case_grouped() {
    ...
    let mut r = q(
        &db,
        "SELECT cat, SUM(CASE WHEN v > 10 THEN v ELSE 0 END) FROM t GROUP BY cat",
    );
...
fn test_case_with_null() {
    ...
    let r = q(
        &db,
        "SELECT CASE WHEN v IS NULL THEN 'null' ELSE 'val' END FROM t ORDER BY id",
    );
    assert_eq!(
        r,
        vec![
...
fn test_coalesce_two_columns() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,NULL,5),(2,10,NULL),(3,NULL,NULL)")
        .unwrap();
...
fn test_order_by_case_desc() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,5),(2,25),(3,15)")
        .unwrap();
    // CASE WHEN v > 20 THEN 0 ELSE 1 DESC.
    // v>20: id2(25)→0; id1,id3→1. DESC: 1s first (id1,id3), then id2(0).
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY CASE WHEN v > 20 THEN 0 ELSE 1 END DESC, id ASC",
    );
    let ids: Vec<i64> = r
        .iter()
        .filter_map(|row| match row[0] {
...
fn test_case_in_arithmetic() {
    ...
    let r = q(
        &db,
        "SELECT v + CASE WHEN v > 10 THEN 100 ELSE 0 END FROM t ORDER BY id",
    );
...
fn test_multiple_case_select() {
    ...
    assert_eq!(
        r,
        vec![vec![
            Value::Text("pos_a".into()),
            Value::Text("big_b".into())
        ]]
    );
}
...
fn test_case_all_false_no_else() {
    ...
    let r = q(
        &db,
        "SELECT CASE WHEN v > 100 THEN 'a' WHEN v > 200 THEN 'b' END FROM t",
    );
...
fn test_case_text_equality() {
    ...
    let r = q(
        &db,
        "SELECT CASE WHEN s = 'foo' THEN 'F' ELSE 'O' END FROM t ORDER BY id",
    );
    assert_eq!(
        r,
        vec![vec![Value::Text("F".into())], vec![Value::Text("O".into())]]
    );
}
...
fn test_case_arithmetic_in_when() {
    ...
    let r = q(
        &db,
        "SELECT CASE WHEN a + b > 10 THEN 'big' ELSE 'small' END FROM t ORDER BY id",
    );
    assert_eq!(
        r,
        vec![
