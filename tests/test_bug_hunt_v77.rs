...
fn test_order_by_expression() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,1,9),(2,5,5),(3,3,3)")
        .unwrap();
...
fn test_three_table_join() {
    ...
    let r = q(
        &db,
        "SELECT c.name FROM a JOIN b ON a.bid = b.id JOIN c ON b.cid = c.id",
    );
...
fn test_left_join_null_fill() {
    ...
    let r = q(
        &db,
        "SELECT a.id, b.v FROM a LEFT JOIN b ON a.bid = b.id ORDER BY a.id",
    );
    assert_eq!(
        r,
        vec![
...
fn test_distinct_multi_column() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,1,1),(2,1,1),(3,1,2),(4,2,1)")
        .unwrap();
...
fn test_aggregate_with_where() {
    ...
    let r = q(
        &db,
        "SELECT SUM(v), COUNT(*), MIN(v), MAX(v) FROM t WHERE v > 15",
    );
