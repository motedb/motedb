...
fn test_min_max_distinct() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,10),(3,30),(4,30)")
        .unwrap();
    // MIN/MAX DISTINCT same as MIN/MAX (dedup doesn't change extremes).
    assert_eq!(
        q(&db, "SELECT MIN(DISTINCT v) FROM t"),
        vec![vec![Value::Integer(10)]]
    );
    assert_eq!(
        q(&db, "SELECT MAX(DISTINCT v) FROM t"),
...
fn test_coalesce_columns() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,NULL,20),(2,10,NULL),(3,NULL,NULL)")
        .unwrap();
...
fn test_in_subquery_with_null() {
    ...
    db.execute("INSERT INTO t VALUES (1,10),(2,20),(3,30)")
        .unwrap();
    db.execute("CREATE TABLE s(id INT PRIMARY KEY, x INT)")
        .unwrap();
...
fn test_order_by_aggregate_alias() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'b',50),(4,'b',5)")
        .unwrap();
    let r = q(
        &db,
        "SELECT cat, SUM(v) AS total FROM t GROUP BY cat ORDER BY total ASC",
    );
...
fn test_group_by_multi_agg_with_count() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',NULL),(3,'a',30),(4,'b',5)")
        .unwrap();
    let mut r = q(
        &db,
        "SELECT cat, COUNT(*), COUNT(v), SUM(v) FROM t GROUP BY cat",
    );
    r.sort_by_key(|row| match &row[0] {
        Value::Text(s) => s.as_str().to_string(),
        _ => "zzz".into(),
...
fn test_where_or_different_cols() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,0),(2,0,20),(3,30,30),(4,0,0)")
        .unwrap();
...
fn test_like_suffix() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'report.pdf'),(2,'data.csv'),(3,'image.pdf')")
        .unwrap();
...
fn test_update_where_and() {
    ...
    db.execute("INSERT INTO t VALUES (1,10,100),(2,10,200),(3,20,100)")
        .unwrap();
    db.execute("UPDATE t SET a = 99 WHERE a = 10 AND b = 200")
        .unwrap();
...
fn test_delete_complex_where() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,5),(2,20,15),(3,30,25)")
        .unwrap();
