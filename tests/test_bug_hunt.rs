...
fn test_null_and_or_logic() {
    ...
    assert!(
        rows.len() >= 2,
        "Rows 1 and 2 should match: v=1 OR w=1, got {} rows",
        rows.len()
    );
}
...
fn test_not_null_is_false() {
    ...
    assert_eq!(
        rows.len(),
        1,
        "NOT NULL should be false, only v=0 row should match"
    );
}
...
fn test_update_multi_row_where() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, status TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'active', 10)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (2, 'inactive', 20)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (3, 'active', 30)")
        .unwrap();
...
fn test_sum_all_null_returns_null() {
    ...
    assert_eq!(
        row.unwrap()[0],
        Value::Null,
        "SUM(all NULLs) should be NULL, not 0"
    );
}
...
fn test_sum_mixed_null_values() {
    ...
    assert_eq!(
        row.unwrap()[0],
        Value::Integer(30),
        "SUM should skip NULLs: 10+20=30"
    );
}
...
fn test_count_star_vs_count_column_with_nulls() {
    ...
    assert_eq!(
        row[0],
        Value::Integer(2),
        "COUNT(v) should skip NULLs: 2 non-null values"
    );
}
...
fn test_concat_with_null() {
    ...
    db.execute("INSERT INTO t VALUES (1, 'hello', 'world')")
        .unwrap();
    db.execute("INSERT INTO t VALUES (2, 'hello', NULL)")
        .unwrap();
    ...
    match concat_null {
        ...
        Value::Text(s) => {
            assert_ne!(
                s.as_str(),
                "helloNULL",
                "CONCAT('hello', NULL) should not be 'helloNULL'"
            );
        }
...
fn test_like_on_null_column() {
    ...
    assert_eq!(
        rows.len(),
        0,
        "NULL NOT LIKE '%' should not include NULL row"
    );
}
...
fn test_update_no_matching_rows() {
    ...
    let result = db
        .execute("UPDATE t SET v = 99 WHERE id = 999")
        .unwrap()
        .materialize()
        .unwrap();
...
fn test_count_distinct() {
    ...
    assert_eq!(
        row[0],
        Value::Integer(3),
        "COUNT(DISTINCT) should be 3 unique values: 10, 20, 30"
    );
}
...
fn test_prepared_select_with_param() {
    ...
    let r1 = db
        .execute_prepared("SELECT * FROM t WHERE id = ?", vec![Value::Integer(1)])
        .unwrap()
        .materialize()
        .unwrap();
    let r2 = db
    ...
    if let (QueryResult::Select { rows: rows1, .. }, QueryResult::Select { rows: rows2, .. }) =
        (r1, r2)
    {
...
fn test_where_between() {
    ...
    assert_eq!(
        rows.len(),
        5,
        "BETWEEN 30 AND 70 should match v=30,40,50,60,70"
    );
}
...
fn test_multiple_tables_independent() {
    ...
    assert_eq!(
        orders.len(),
        3,
        "Orders should still exist after dropping users"
    );
}
...
fn test_table_with_same_key_different_tables() {
    ...
    db.execute("CREATE TABLE t1 (id INT PRIMARY KEY, v TEXT)")
        .unwrap();
    db.execute("CREATE TABLE t2 (id INT PRIMARY KEY, v TEXT)")
        .unwrap();
