...
fn test_order_by_nulls_asc() {
    ...
    db.execute("INSERT INTO t VALUES (1,10),(2,NULL),(3,5),(4,NULL)").unwrap();
    ...
    let ids: Vec<i64> = r.iter().map(|row| {
        if let Value::Integer(i) = row[0] { i } else { -999 }
    }).collect();
    ...
    assert!(pos3 < pos1, "v=5 (id=3) must sort before v=10 (id=1); got {:?}", ids);
}
...
fn test_order_by_nulls_desc() {
    ...
    let pos3 = ids.iter().position(|&i| i == 3).unwrap();
    let pos1 = ids.iter().position(|&i| i == 1).unwrap();
    assert!(pos1 < pos3, "v=10 (id=1) must sort before v=5 (id=3) DESC; got {:?}", ids);
}
...
fn test_not_in_with_null() {
    ...
    assert_eq!(r, Vec::<Vec<Value>>::new(), "NOT IN with NULL must yield no rows (3-valued logic); got {:?}", r);
}
...
fn test_division_by_zero() {
    ...
    let res = db.execute("SELECT 1 / 0 FROM t").and_then(|s| s.materialize());
...
fn test_count_star_vs_count_col_with_null() {
    ...
    assert_eq!(r2, vec![vec![Value::Integer(2)]], "COUNT(v) must ignore the NULL row");
}
...
fn test_sum_all_null() {
    ...
    assert_eq!(r, vec![vec![Value::Null]], "SUM of all-NULL must be NULL; got {:?}", r);
}
...
fn test_sum_empty() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    let r = q(&db, "SELECT SUM(v) FROM t");
    assert_eq!(r, vec![vec![Value::Null]], "SUM over empty set must be NULL; got {:?}", r);
}
...
fn test_distinct_with_null() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,NULL),(3,10),(4,NULL),(5,20)").unwrap();
    ...
    assert_eq!(r, vec![vec![Value::Null], vec![Value::Integer(10)], vec![Value::Integer(20)]]);
}
...
fn test_like_underscore_wildcard() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'cat'),(2,'cot'),(3,'cart'),(4,'ct')").unwrap();
    ...
    let mut got: Vec<&str> = r.iter().filter_map(|row| match &row[0] {
        Value::Text(s) => Some(s.as_str()),
        _ => None,
    }).collect();
    got.sort();
    assert_eq!(got, vec!["cat", "cot"],
        "'c_t' must match only single-char-middle (cat, cot); got {:?}", r);
}
...
fn test_negative_in_where() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,-5),(2,0),(3,5)").unwrap();
...
fn test_between_inclusive() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,15),(3,20)").unwrap();
    let r = q(&db, "SELECT id FROM t WHERE v BETWEEN 10 AND 20");
    let mut ids: Vec<i64> = r.iter().filter_map(|row| match row[0] {
...
fn test_between_excludes_outside() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,5),(2,10),(3,25)").unwrap();
    let r = q(&db, "SELECT id FROM t WHERE v BETWEEN 10 AND 20");
    assert_eq!(r, vec![vec![Value::Integer(2)]]);
}
