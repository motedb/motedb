...
fn avg_float_precision() {
    ...
    for i in 1..=10 {
        exec(
            &db,
            &format!("INSERT INTO t VALUES ({}, {:.3})", i, i as f64 * 1.5),
        );
    }
...
fn checkpoint_reopen_all_rows_intact() {
    ...
    let path = dir.path().to_path_buf();
    {
        ...
        for i in 1..=200 {
            exec(
                &db,
                &format!("INSERT INTO t VALUES ({}, {}, 'row{}')", i, i * 3, i),
            );
        }
...
fn group_by_many_groups_correct() {
    ...
    for i in 1..=100 {
        exec(
            &db,
            &format!("INSERT INTO t VALUES ({}, {}, {})", i, i % 10, i),
        );
    }
...
fn mixed_types_all_correct() {
    ...
    exec(
        &db,
        "CREATE TABLE t (id INT PRIMARY KEY, i INT, f FLOAT, s TEXT)",
    );
...
fn where_range_on_float() {
    ...
    let r = rows(
        &db,
        "SELECT id FROM t WHERE v BETWEEN 3.0 AND 6.0 ORDER BY id",
    );
    assert_eq!(r.len(), 3);
}
...
fn where_range_on_text() {
    ...
    let r = rows(
        &db,
        "SELECT id FROM t WHERE s BETWEEN 'b' AND 'e' ORDER BY id",
    );
...
fn pk_with_gaps_correct() {
    ...
    for i in (1..=20).step_by(2) {
        assert_eq!(
            scalar_i64(&db, &format!("SELECT COUNT(*) FROM t WHERE id = {}", i)),
            0
        );
    }
}
