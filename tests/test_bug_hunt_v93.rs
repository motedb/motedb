...
fn test_coalesce_with_subquery() {
    ...
    let r = q(
        &db,
        "SELECT COALESCE(v, (SELECT MAX(v) FROM t)) FROM t ORDER BY id",
    );
...
fn test_subquery_plus_case() {
    ...
    let r = q(
        &db,
        "SELECT (SELECT MAX(v) FROM t) + (CASE WHEN 1 = 1 THEN 5 ELSE 0 END) FROM t",
    );
...
fn test_in_subquery_consistency() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v IN (SELECT v FROM t WHERE v > 15)",
    ));
    assert_eq!(r, vec![2, 3]);
}
...
fn test_two_in_subqueries_and() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v IN (SELECT x FROM s1) AND v IN (SELECT y FROM s2)",
    ));
    assert_eq!(r, vec![1]);
}
...
fn test_in_subquery_plus_where() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v IN (SELECT x FROM s) AND cat = 'a'",
    ));
    assert_eq!(r, vec![1, 2]);
}
...
fn test_arithmetic_on_subquery_where() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v > (SELECT SUM(v) FROM t) / 4",
    ));
    assert_eq!(r, vec![3, 4]);
}
...
fn test_having_subquery_count() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'a',10),(2,'a',20),(3,'b',5),(4,'b',15),(5,'b',25)")
        .unwrap();
    // Groups where COUNT(*) > (SELECT COUNT(*) FROM t) / 2.
    // Total=5, /2=2.5. a:count=2 (not >2.5). b:count=3 (>2.5).
    let r = q(
        &db,
        "SELECT cat FROM t GROUP BY cat HAVING COUNT(*) > (SELECT COUNT(*) FROM t) / 2",
    );
...
fn test_subquery_order_by_desc() {
    ...
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY (SELECT MAX(v) FROM t) - v DESC",
    );
    let ids: Vec<i64> = r
        .iter()
...
fn test_in_empty_subquery() {
    ...
    let r = q(
        &db,
        "SELECT id FROM t WHERE v IN (SELECT x FROM s WHERE x > 100)",
    );
    assert_eq!(r, Vec::<Vec<Value>>::new());
}
...
fn test_not_in_empty_subquery() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v NOT IN (SELECT x FROM s WHERE x > 100)",
    ));
    assert_eq!(r, vec![1, 2]);
}
...
fn test_case_subquery_in_order_by() {
    ...
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY CASE WHEN v = (SELECT MAX(v) FROM t) THEN 0 ELSE 1 END ASC",
    );
    let ids: Vec<i64> = r
        .iter()
        .filter_map(|row| match row[0] {
