...
fn setup_db(dir: &Path) -> Database {
    ...
    db.execute(
        "CREATE TABLE users (id INT PRIMARY KEY AUTO_INCREMENT, name TEXT, age INT, score FLOAT)",
    )
    .unwrap();
    db
}
...
fn test_parameterized_select_basic() {
    ...
    assert_eq!(all.len(), 1);
    let target_id = match all[0][0] {
        Value::Integer(id) => id,
        _ => panic!("no id"),
    };

    let rows = query_rows_prepared(
        &db,
...
fn test_parameterized_unbound_error() {
    ...
    assert!(
        result.is_err(),
        "execute_prepared with unbound ? should error"
    );
...
fn test_table_qualified_column_select() {
    ...
    let row0 = query_rows(&db, "SELECT * FROM users")
        .into_iter()
        .next()
        .unwrap();
    let id = match row0[0] {
    ...
    let rows = query_rows(
        &db,
        &format!("SELECT users.name, users.age FROM users WHERE id = {}", id),
    );
    assert_eq!(rows.len(), 1);
...
fn test_column_index_scan_correctness() {
    ...
    db.execute(
        "CREATE TABLE items (id INT PRIMARY KEY AUTO_INCREMENT, category TEXT, price FLOAT)",
    )
    .unwrap();
...
fn test_edge_update_reselect() {
    ...
    db.execute(&format!(
        "UPDATE users SET age = 30, score = 95.0 WHERE id = {}",
        id
    ))
    .unwrap();
...
fn test_edge_delete_verify() {
    ...
    let id5 = match row5[0][0] {
        Value::Integer(i) => i,
        _ => panic!(),
    };

    db.execute(&format!("DELETE FROM users WHERE id = {}", id5))
        .unwrap();
...
fn test_edge_string_with_keywords() {
    let dir = TempDir::new().unwrap();
    let db = setup_db(dir.path());
    db.execute("INSERT INTO users VALUES (null, 'from where select', 25, 90.0)")
        .unwrap();
...
fn test_comprehensive_summary() {
    ...
    println!(
        "  INSERT:   {:.0} µs/op, {:.0} ops/s",
        insert_us,
        1_000_000.0 / insert_us
    );

    // PK SELECT (raw)
    let pk_iters = if std::env::var("CI").is_ok() {
        200
    } else {
        1000
    };
    ...
    for i in 0..pk_iters {
        let _ = query_rows(
            &db,
            &format!("SELECT * FROM users WHERE id = {}", i % n + 1),
        );
    }
    ...
    println!(
        "  PK SELECT (prep): {:.0} µs/op  ({:.2}x vs raw)",
        pk_prep_us,
        pk_us / pk_prep_us
    );
    ...
    println!(
        "  Full scan {} rows (WHERE age>40): {:.1} ms ({:.1} µs/row)",
        n,
        scan_time.as_secs_f64() * 1000.0,
        scan_time.as_micros() as f64 / n as f64
    );

    // UPDATE
    let update_n = if std::env::var("CI").is_ok() {
        100
    } else {
        500
    };
    ...
    println!(
        "  UPDATE:   {:.0} µs/op",
        start.elapsed().as_micros() as f64 / update_n as f64
    );
    ...
    println!(
        "  DELETE:   {:.0} µs/op",
        start.elapsed().as_micros() as f64 / delete_n as f64
    );
    ...
    let rows_p = query_rows_prepared(
        &db,
