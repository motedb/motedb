...
fn count_empty_table_is_zero() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE e (id INT PRIMARY KEY, v INT)").unwrap();
...
fn aggregates_with_where_matching_nothing() {
    ...
    for i in 1..=5 {
        db.insert_row("t", vec![Value::Integer(i), Value::Integer(i * 10)]).unwrap();
    }
    // WHERE matches nothing.
    let r = one(&db, "SELECT COUNT(*), SUM(v), MIN(v), MAX(v) FROM t WHERE id > 100");
    assert_eq!(r[0], Value::Integer(0));
...
fn avg_integer_division_yields_float() {
    ...
    db.insert_row("t", vec![Value::Integer(1), Value::Integer(1)]).unwrap();
    db.insert_row("t", vec![Value::Integer(2), Value::Integer(2)]).unwrap();
    db.insert_row("t", vec![Value::Integer(3), Value::Integer(3)]).unwrap();
...
fn limit_zero_returns_no_rows() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    for i in 1..=10 {
        db.insert_row("t", vec![Value::Integer(i), Value::Integer(i)]).unwrap();
    }
...
fn order_by_multi_column_mixed_direction() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, grp INT, v INT)").unwrap();
    let data = [
        (1, 1, 30),
        (2, 1, 10),
        (3, 1, 20),
    ...
    for (id, grp, v) in data {
        db.insert_row("t", vec![Value::Integer(id), Value::Integer(grp), Value::Integer(v)])
            .unwrap();
    }
...
fn order_by_tiebreaker_stable_secondary() {
    ...
    for (id, b) in [(1, 30), (2, 10), (3, 20), (4, 10)] {
        db.insert_row("t", vec![Value::Integer(id), Value::Integer(5), Value::Integer(b)])
            .unwrap();
    }
...
fn null_does_not_equal_null_in_where() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    db.insert_row("t", vec![Value::Integer(1), Value::Null]).unwrap();
    db.insert_row("t", vec![Value::Integer(2), Value::Null]).unwrap();
    db.insert_row("t", vec![Value::Integer(3), Value::Integer(7)]).unwrap();
...
fn is_not_null_filters_nulls() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    db.insert_row("t", vec![Value::Integer(1), Value::Null]).unwrap();
    db.insert_row("t", vec![Value::Integer(2), Value::Integer(7)]).unwrap();
    db.insert_row("t", vec![Value::Integer(3), Value::Null]).unwrap();
    db.insert_row("t", vec![Value::Integer(4), Value::Integer(9)]).unwrap();
...
fn integer_comparisons_exact() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    for i in -2..=2 {
        db.insert_row("t", vec![Value::Integer(i + 10), Value::Integer(i * 100)]).unwrap();
    }
    // v values: -200, -100, 0, 100, 200
    let r = rows(&db, "SELECT v FROM t WHERE v >= 0 ORDER BY v");
    let vals: Vec<i64> = r.iter().map(|row| match row[0] {
        Value::Integer(n) => n,
...
fn integer_values_returned_as_integer_not_float() {
    ...
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    db.insert_row("t", vec![Value::Integer(1), Value::Integer(42)]).unwrap();
...
fn distinct_multi_column() {
    ...
    for (i, (a, b)) in pairs.iter().enumerate() {
        db.insert_row(
            "t",
            vec![Value::Integer(i as i64 + 1), Value::Integer(*a), Value::Integer(*b)],
        )
        .unwrap();
    }
...
fn limit_offset_pagination() {
    ...
    let page = |offset: usize, limit: usize| -> Vec<i64> {
        rows(&db, &format!("SELECT id FROM t ORDER BY id LIMIT {} OFFSET {}", limit, offset))
            .into_iter()
            .map(|row| match row[0] {
                Value::Integer(n) => n,
                _ => panic!(),
...
fn count_star_vs_count_column_with_nulls() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    db.insert_row("t", vec![Value::Integer(1), Value::Integer(10)]).unwrap();
    db.insert_row("t", vec![Value::Integer(2), Value::Null]).unwrap();
    db.insert_row("t", vec![Value::Integer(3), Value::Integer(30)]).unwrap();
...
fn aggregates_with_negative_and_mixed() {
    ...
    for (i, v) in [10, -5, 7, -3, 0].iter().enumerate() {
        db.insert_row("t", vec![Value::Integer(i as i64 + 1), Value::Integer(*v)]).unwrap();
    }
...
fn drop_then_recreate_same_name() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT)").unwrap();
    db.insert_row("t", vec![Value::Integer(1), Value::Integer(100)]).unwrap();
    ...
    db.insert_row("t", vec![Value::Integer(1), Value::Text("hi".into())]).unwrap();
...
fn update_all_rows_and_multi_column() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, a INT, b INT)").unwrap();
    for i in 1..=4 {
        db.insert_row("t", vec![Value::Integer(i), Value::Integer(0), Value::Integer(0)])
            .unwrap();
    }
    ...
    db.execute("UPDATE t SET a = 1, b = 2 WHERE id = 2").unwrap();
...
fn delete_all_rows() {
    ...
    db.insert_row("t", vec![Value::Integer(1), Value::Integer(42)]).unwrap();
    assert_eq!(one(&db, "SELECT v FROM t WHERE id = 1")[0], Value::Integer(42));
}
