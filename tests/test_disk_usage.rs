...
fn disk_usage_measurement() {
    ...
    db.execute("CREATE TABLE scalar (id INT PRIMARY KEY, name TEXT, age INT, score FLOAT)").unwrap();
    for i in 0..10000 {
        db.execute_prepared(
            "INSERT INTO scalar VALUES (?, ?, ?, ?)",
            vec![Value::Integer(i), Value::text(format!("user_{}", i)), Value::Integer(20 + i % 50), Value::Float(50.0 + i as f64 % 100.0)],
        ).unwrap();
    }

    db.execute("CREATE INDEX idx_scalar_age ON scalar(age)").unwrap();
    db.checkpoint().unwrap();

    // Raw row size
    let row = vec![Value::Integer(42), Value::text("user_42".to_string()), Value::Integer(62), Value::Float(92.0)];
    let col_types = [motedb::types::ColumnType::Integer, motedb::types::ColumnType::Text, motedb::types::ColumnType::Integer, motedb::types::ColumnType::Float];
    ...
    println!("Compression ratio: {:.1}x", (encoded.len() as f64 * 10000.0) / mote_bytes as f64);
    ...
    println!("    TOTAL: {} KB", all_files.iter().map(|(_, s)| s).sum::<u64>() / 1024);
    ...
    println!("    Actual: {} bytes ({:.0}% overhead)", encoded.len(), (encoded.len() as f64 / 39.0 - 1.0) * 100.0);
    ...
    println!("  1 day: {:.0} MB", rows_per_day as f64 * imu_sst / 1024.0 / 1024.0);
    println!("  30 days: {:.0} MB", rows_per_day as f64 * imu_sst * 30.0 / 1024.0 / 1024.0);
