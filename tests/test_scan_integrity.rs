...
fn test_where_count_accuracy() {
    ...
    let count_completed = query_val(&db, "SELECT COUNT(*) AS c FROM t WHERE status = 'completed'");
    ...
    println!("COUNT(*) total={}, pending={}, completed={}", count_total, count_pending, count_completed);
    println!("Manual  total={}, pending={}, completed={}", manual_total, manual_pending, manual_completed);
    ...
    if count_pending != manual_pending {
        println!("MISMATCH: COUNT WHERE pending={} but manual scan pending={}", count_pending, manual_pending);
    }
    if count_completed != manual_completed {
        println!("MISMATCH: COUNT WHERE completed={} but manual scan completed={}", count_completed, manual_completed);
    }
