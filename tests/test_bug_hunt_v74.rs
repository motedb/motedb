...
fn test_table_name_case_sensitive_or_insensitive() {
    let (db, _d) = db();
    db.execute("CREATE TABLE MyTable(id INT PRIMARY KEY)").unwrap();
...
fn test_select_star_order_matches_schema() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(first INT, second INT, third INT)").unwrap();
    db.execute("INSERT INTO t VALUES (10,20,30)").unwrap();
    let r = q(&db, "SELECT * FROM t");
    // Order should match schema definition order.
    assert_eq!(r[0], vec![Value::Integer(10), Value::Integer(20), Value::Integer(30)]);
}
...
fn test_groupby_count_constant() {
    ...
    assert_eq!(
        r,
        vec![vec![Value::Integer(1), Value::Integer(2)], vec![Value::Integer(2), Value::Integer(1)]]
    );
}
...
fn test_negative_in_select() {
    ...
    assert_eq!(r, vec![vec![Value::Integer(-1), Value::Float(-2.5), Value::Integer(-100)]]);
}
...
fn test_negative_in_where() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,-10),(2,5),(3,-20)").unwrap();
...
fn test_negative_in_order_by() {
    ...
    assert_eq!(r, vec![vec![Value::Integer(-20)], vec![Value::Integer(-10)], vec![Value::Integer(5)]]);
}
...
fn test_string_ordering_numbers_as_text() {
    ...
    assert_eq!(
        r,
        vec![vec![Value::text("10".into())], vec![Value::text("100".into())], vec![Value::text("9".into())]]
    );
}
...
fn test_empty_string_equality() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,''),(2,'x'),(3,NULL)").unwrap();
...
fn test_order_by_two_cols_asc() {
    ...
    db.execute("INSERT INTO t VALUES (1,2),(1,1),(2,1),(1,3)").unwrap();
