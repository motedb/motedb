...
fn test_nested_subquery_in_select() {
    ...
    let r = q(
        &db,
        "SELECT (SELECT MAX(v) FROM t WHERE v < (SELECT MAX(v) FROM t)) FROM t",
    );
...
fn test_subquery_in_having() {
    ...
    let r = q(
        &db,
        "SELECT cat FROM t GROUP BY cat HAVING SUM(v) > (SELECT AVG(v) FROM t)",
    );
...
fn test_not_in_subquery_nested() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE id NOT IN (SELECT id FROM t WHERE v >= 30)",
    ));
    assert_eq!(r, vec![1, 2]);
}
...
fn test_subquery_max_comparison() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,50),(3,30),(4,50)")
        .unwrap();
...
fn test_two_subqueries_in_where() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v > (SELECT MIN(v) FROM t) AND v < (SELECT MAX(v) FROM t)",
    ));
    assert_eq!(r, vec![2, 3]);
}
...
fn test_subquery_in_arithmetic_where() {
    ...
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM t WHERE v > (SELECT MIN(v) FROM t) + 15",
    ));
    assert_eq!(r, vec![3, 4]);
}
...
fn test_correlated_subquery_in_select_list() {
    ...
    db.execute("CREATE TABLE emp(id INT PRIMARY KEY, dept_id INT, salary INT)")
        .unwrap();
    db.execute("INSERT INTO dept VALUES (1,1000),(2,500)")
        .unwrap();
    ...
    let mut r = q(
        &db,
        "SELECT id, (SELECT SUM(salary) FROM emp WHERE emp.dept_id = dept.id) FROM dept",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
...
fn test_cross_table_in_with_outer_where() {
    ...
    db.execute("CREATE TABLE orders(id INT PRIMARY KEY, cust_id INT, amt INT)")
        .unwrap();
    db.execute("CREATE TABLE customers(id INT PRIMARY KEY, region TEXT)")
        .unwrap();
    db.execute("INSERT INTO customers VALUES (1,'east'),(2,'west'),(3,'east')")
        .unwrap();
    db.execute("INSERT INTO orders VALUES (10,1,100),(11,2,200),(12,3,300)")
        .unwrap();
    // Customers in 'east' who have orders.
    let r = sorted_int(&q(
        &db,
        "SELECT id FROM customers WHERE region = 'east' AND id IN (SELECT cust_id FROM orders)",
    ));
    assert_eq!(r, vec![1, 3]);
}
...
fn test_union_of_aggregates() {
    ...
    let mut r = q(
        &db,
        "SELECT SUM(v) FROM t WHERE cat = 'a' UNION SELECT SUM(v) FROM t WHERE cat = 'b'",
    );
    r.sort_by_key(|row| match row[0] {
        Value::Integer(i) => i,
        _ => 999,
...
fn test_multiple_scalar_subqueries_select() {
    ...
    let r = q(
        &db,
        "SELECT (SELECT MIN(v) FROM t), (SELECT MAX(v) FROM t), (SELECT AVG(v) FROM t)",
    );
    assert_eq!(r.len(), 1);
...
fn test_subquery_in_order_by() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,30),(3,20)")
        .unwrap();
    ...
    let r = q(
        &db,
        "SELECT id FROM t ORDER BY ABS(v - (SELECT AVG(v) FROM t)) ASC, id ASC",
    );
    let ids: Vec<i64> = r
        .iter()
...
fn test_correlated_count_diff_tables() {
    let (db, _d) = db();
    db.execute("CREATE TABLE orders(id INT PRIMARY KEY, cust_id INT)")
        .unwrap();
...
fn test_aggregate_over_derived() {
    ...
    let r = q(
        &db,
        "SELECT SUM(sub.total) FROM (SELECT SUM(v) AS total FROM t GROUP BY cat) AS sub",
    );
