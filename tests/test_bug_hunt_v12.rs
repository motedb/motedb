...
fn join_three_tables() {
    ...
    let r = rows(
        &db,
        "SELECT a.id FROM a INNER JOIN b ON a.id = b.a_id INNER JOIN c ON b.id = c.b_id",
    );
...
fn aggregate_consistent_across_reopen() {
    ...
    let (count_before, sum_before, min_before, max_before);
    {
        ...
        exec(&db, "CREATE TABLE t (id INT PRIMARY KEY, v INT)");
        for i in 1..=100 {
            exec(&db, &format!("INSERT INTO t VALUES ({}, {})", i, i * 3));
        }
...
fn group_by_text_multi_agg_correct() {
    ...
    exec(
        &db,
        "CREATE TABLE t (id INT PRIMARY KEY, dept TEXT, salary INT)",
    );
...
fn where_in_subquery_correct() {
    ...
    let r = rows(
        &db,
        "SELECT v FROM a WHERE id IN (SELECT ref FROM b) ORDER BY v",
    );
    assert_eq!(r.len(), 2);
    match (&r[0][0], &r[1][0]) {
