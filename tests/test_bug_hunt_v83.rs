...
fn test_union_dedup() {
    ...
    let mut vals: Vec<i64> = r.iter().filter_map(|row| match row[0] { Value::Integer(i) => Some(i), _ => None }).collect();
    vals.sort();
...
fn test_union_all_no_dedup() {
    ...
    let mut vals: Vec<i64> = r.iter().filter_map(|row| match row[0] { Value::Integer(i) => Some(i), _ => None }).collect();
    vals.sort();
    assert_eq!(vals, vec![10, 10, 10, 10, 20, 20], "UNION ALL keeps all 6 rows");
}
...
fn test_group_having_order_combo() {
    ...
    let r = q(&db, "SELECT cat, SUM(v) FROM t GROUP BY cat HAVING SUM(v) > 10 ORDER BY SUM(v) DESC");
...
fn test_cross_table_correlated() {
    ...
    db.execute("INSERT INTO orders VALUES (10,1,100),(11,1,200),(12,2,50)").unwrap();
    // For each customer, their total order amount (correlated on cust_id).
    let mut r = q(&db, "SELECT name, (SELECT SUM(amt) FROM orders WHERE cust_id = customers.id) FROM customers");
...
fn test_nested_in_subquery() {
    ...
    let r = sorted_int(&q(&db, "SELECT id FROM a WHERE bid IN (SELECT id FROM b WHERE cid IN (SELECT id FROM c))"));
    assert_eq!(r, vec![1]);
}
...
fn test_string_special_chars() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a,b'),(2,'c;d'),(3,'e f')").unwrap();
...
fn test_count_group_single() {
    ...
    db.execute("INSERT INTO t VALUES (1,'x'),(2,'x'),(3,'x')").unwrap();
...
fn test_order_by_multi_asc() {
    ...
    db.execute("INSERT INTO t VALUES (1,2,1),(2,1,2),(3,1,1),(4,2,2)").unwrap();
...
fn test_min_max_float() {
    ...
    db.execute("INSERT INTO t VALUES (1,1.5),(2,2.5),(3,0.5)").unwrap();
...
fn test_sum_float() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v FLOAT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1.5),(2,2.5),(3,3.0)").unwrap();
...
fn test_where_col_equals_col() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,5,5),(2,5,6),(3,6,6)").unwrap();
