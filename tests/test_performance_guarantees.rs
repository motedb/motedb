...
fn test_pk_select_latency_stable() {
    for &target in &[1_000, 5_000, 10_000] {
        ...
        assert!(p99 < 5_000, "PK SELECT P99 {}us too slow at {} rows", p99, target);
    }
}
...
fn test_batch_insert_fast() {
    ...
    let (_, p99) = measure_p99_us(|| {
        ...
        for i in 0..100 {
            ...
            let id = 900000 + (i as u64) + std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos() as u64 % 100000;
...
fn test_all_queries_under_threshold_10k() {
    ...
    let queries = vec![
        ...
        ("WHERE + LIMIT", "SELECT * FROM bench WHERE tag = 'US' LIMIT 10"),
        ("COUNT WHERE", "SELECT COUNT(*) FROM bench WHERE tag = 'US'"),
        ("DISTINCT", "SELECT DISTINCT tag FROM bench"),
        ("ORDER BY + LIMIT", "SELECT * FROM bench ORDER BY val DESC LIMIT 10"),
    ];
    ...
    for (name, sql) in queries {
        ...
        assert!(
            p99 < 50_000,
            "{} P99 {}us exceeds 50ms at 10K rows", name, p99
        );
    }
}
