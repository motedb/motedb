...
fn bench_text_fts() {
    ...
    for i in 0..n {
        ...
        exec(
            &db,
            &format!("INSERT INTO docs VALUES ({}, '{}', '{}')", i, title, body),
        );
    }
    ...
    print_result(
        &format!("FTS 'machine learning' top-10 x {}", q),
        q,
        multi_ms,
    );
...
fn bench_vector_knn() {
    ...
    exec(
        &db,
        &format!(
            "CREATE TABLE vecs (id INT PRIMARY KEY, embedding VECTOR({}))",
            dim
        ),
    );

    println!(
        "\n  === Vector KNN Benchmark ({} vectors, dim={}) ===",
        n, dim
    );
    print_separator();
    ...
    for i in 0..n {
        let values: Vec<f32> = (0..dim)
            .map(|j| ((i as f32).sin() + j as f32 * 0.01).sin())
            .collect();
        let tensor_str = format!(
            "[{}]",
            values
                .iter()
                .map(|v| format!("{:.6}", v))
        ...
        exec(
            &db,
            &format!("INSERT INTO vecs VALUES ({}, {})", i, tensor_str),
        );
    }
    let seed_ms = seed_start.elapsed().as_millis() as u64;
    ...
    let sql_knn_ms = {
        ...
        for _ in 0..q {
            let _ = exec(
                &db,
                &format!(
                    "SELECT id FROM vecs ORDER BY embedding <-> {} LIMIT 10",
                    query_str
                ),
            );
        }
    ...
    print_result(
        &format!("SQL ORDER BY emb <-> q LIMIT 10 x {}", q),
        q,
        sql_knn_ms,
    );
    ...
    println!(
        "  -> Raw KNN: {:.1}µs/op, SQL KNN: {:.1}µs/op",
        knn_per, sql_per
    );
...
fn bench_spatial_queries() {
    ...
    exec(
        &db,
        "CREATE TABLE points (id INT PRIMARY KEY, name TEXT, location GEOMETRY)",
    );
    ...
    for i in 0..n {
        ...
        exec(
            &db,
            &format!(
                "INSERT INTO points VALUES ({}, 'point_{}', POINT({:.6}, {:.6}))",
                i, i, lat, lon
            ),
        );
    }
    ...
    exec(
        &db,
        "CREATE INDEX idx_loc ON points (location) USING OCTREE",
    );
    db.wait_for_indexes_ready();
    db.flush().unwrap();
    let idx_ms = idx_start.elapsed().as_millis() as u64;
    ...
    let dist_ms = {
        ...
        for _ in 0..q {
            let _ = exec(
                &db,
                "SELECT id FROM points ORDER BY ST_DISTANCE(location, 116.5, 40.0) LIMIT 10",
            );
        }
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("ST_DISTANCE ORDER BY LIMIT 10 x {}", q),
        q,
        dist_ms,
    );
    ...
    println!(
        "  -> ST_DISTANCE: {:.1}µs/op, WITHIN_RADIUS: {:.1}µs/op",
        dist_per, radius_per
    );
...
fn bench_multimodal_mixed() {
    ...
    exec(
        &db,
        "CREATE TABLE items (id INT PRIMARY KEY, title TEXT, embedding VECTOR(64))",
    );
    exec(
        &db,
        "CREATE TABLE locations (id INT PRIMARY KEY, name TEXT, coords GEOMETRY)",
    );
    ...
    // Seed
    let words = [
        "laptop", "phone", "tablet", "watch", "camera", "speaker", "monitor", "keyboard",
    ];
    for i in 0..n {
        ...
        exec(
            &db,
            &format!(
                "INSERT INTO locations VALUES ({}, 'loc_{}', POINT({:.4}, {:.4}))",
                i,
                i,
                116.0 + i as f64 * 0.01,
                40.0 + i as f64 * 0.01
    ...
    exec(
        &db,
        "CREATE INDEX idx_coords ON locations (coords) USING OCTREE",
    );
    db.wait_for_indexes_ready();
    db.flush().unwrap();
    ...
    let spatial_ms = {
        ...
        for _ in 0..q {
            let _ = exec(
                &db,
                "SELECT id FROM locations ORDER BY ST_DISTANCE(coords, 116.5, 40.0) LIMIT 5",
            );
        }
        start.elapsed().as_millis() as u64
    };
    ...
    println!(
        "  -> FTS: {:.0}µs/op, KNN: {:.0}µs/op, Spatial: {:.0}µs/op",
