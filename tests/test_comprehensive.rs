...
use motedb::{config::DBConfig, sql::QueryResult, types::Value, Database};
...
fn test_ddl_create_drop_create() {
    ...
    assert_eq!(
        qv(&db, "SELECT name FROM t WHERE id = 1"),
...
fn test_ddl_multiple_tables() {
    ...
    let db = setup_db(dir.path());
    for i in 0..5 {
        db.execute(&format!("CREATE TABLE t{} (id INT PRIMARY KEY, v INT)", i))
            .unwrap();
        db.execute(&format!("INSERT INTO t{} VALUES (1, {})", i, i * 10))
            .unwrap();
    }
    for i in 0..5 {
...
fn test_ddl_table_with_all_types() {
    let dir = TempDir::new().unwrap();
    let db = setup_db(dir.path());
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, i INT, f FLOAT, s TEXT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, -42, 3.14, 'hello', 1)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (2, 0, 0.0, '', 0)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (3, NULL, NULL, NULL, NULL)")
        .unwrap();
...
fn test_insert_auto_increment() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY AUTO_INCREMENT, v TEXT)")
        .unwrap();
    for c in ['a', 'b', 'c', 'd', 'e'] {
        db.execute(&format!("INSERT INTO t (v) VALUES ('{}')", c))
            .unwrap();
    }
    let rows = query_rows(&db, "SELECT * FROM t ORDER BY id");
...
fn test_insert_null_columns() {
    ...
    db.execute("INSERT INTO t VALUES (1, NULL, NULL, NULL)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (2, 10, NULL, 1.5)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (3, NULL, 'text', NULL)")
        .unwrap();
...
fn test_insert_named_columns() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t (id, b, a) VALUES (1, 20, 10)")
        .unwrap();
...
fn test_select_arithmetic() {
    ...
    assert_eq!(
        qv(&db, "SELECT a + b FROM t WHERE a > 0"),
        Value::Integer(13)
    );
    assert_eq!(
        qv(&db, "SELECT a - b FROM t WHERE a > 0"),
...
fn test_select_functions() {
    let dir = TempDir::new().unwrap();
    let db = setup_db(dir.path());
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, s TEXT, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'Hello World', -5)")
        .unwrap();
    assert_eq!(
        qv(&db, "SELECT UPPER(s) FROM t"),
        Value::text("HELLO WORLD".into())
    );
    assert_eq!(
        qv(&db, "SELECT LOWER(s) FROM t"),
        Value::text("hello world".into())
    );
...
fn test_select_concat() {
    ...
    db.execute("INSERT INTO t VALUES (1, 'hello', 'world')")
        .unwrap();
    assert_eq!(
        qv(&db, "SELECT CONCAT(a, ' ', b) FROM t"),
        Value::text("hello world".into())
    );
}
...
fn test_aggregates_basic() {
    ...
    match avg {
        Value::Float(f) => assert!((f - 30.0).abs() < 0.01),
        _ => panic!("AVG should be float"),
    }
}
...
fn test_count_distinct() {
    ...
    assert_eq!(
        qv(&db, "SELECT COUNT(DISTINCT v) FROM t"),
        Value::Integer(3)
    );
}
...
fn test_where_and_or() {
    ...
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE a = 1 AND b = 1").len(),
        1
    );
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE a = 1 OR b = 1").len(),
        3
    );
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE a = 0 AND b = 0").len(),
        1
    );
}
...
fn test_where_is_null_is_not_null() {
    ...
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE v IS NOT NULL").len(),
        2
    );
}
...
fn test_where_in_not_in() {
    ...
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE v IN (10, 30, 50)").len(),
        3
    );
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE v NOT IN (10, 30, 50)").len(),
        2
    );
}
...
fn test_where_like() {
    ...
    db.execute("INSERT INTO t VALUES (3, 'world hello')")
        .unwrap();
    db.execute("INSERT INTO t VALUES (4, 'HELLO')").unwrap();

    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE s LIKE 'hello%'").len(),
        2,
        "LIKE 'hello%'"
    );
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE s LIKE '%world%'").len(),
        2,
        "LIKE '%%world%%'"
    );
    ...
    assert_eq!(
        query_rows(&db, "SELECT * FROM t WHERE s NOT LIKE 'hello%'").len(),
        2,
        "NOT LIKE 'hello%'"
    );
}
...
fn test_where_parenthesized() {
    ...
    let rows = query_rows(
        &db,
        "SELECT * FROM t WHERE (a = 0 OR a = 1) AND b = 0 ORDER BY id",
    );
    assert_eq!(rows.len(), 2);
}
...
fn test_limit_offset() {
    ...
    assert_eq!(
        query_rows(&db, "SELECT * FROM t ORDER BY id LIMIT 3").len(),
        3
    );
...
fn test_delete_reinsert() {
    ...
    assert_eq!(
        qv(&db, "SELECT v FROM t WHERE id = 1"),
        Value::text("new".into())
    );
}
...
fn test_prepared_insert_select() {
    ...
    for i in 1..=5 {
        let r = db
            .execute_prepared("SELECT name FROM t WHERE id = ?", vec![Value::Integer(i)])
            .unwrap()
            .materialize()
            .unwrap();
...
fn test_reopen_full_crud() {
...

    {
        ...
        db.execute("UPDATE t SET v = 'updated' WHERE id = 2")
            .unwrap();
        db.execute("DELETE FROM t WHERE id = 3").unwrap();
        drop(db);
    }
...
fn test_stress_500_rows_crud() {
    ...
    for i in (10..=500).step_by(10) {
        db.execute(&format!("UPDATE t SET v = v + 1000 WHERE id = {}", i))
            .unwrap();
    }
...
fn test_perf_insert_throughput() {
    ...
    eprintln!(
        "INSERT (GroupCommit): {} rows in {:.1}ms ({:.0}µs/op, {:.0} ops/sec)",
        n,
    ...
    for i in 1..=n {
        db2.execute(&format!("INSERT INTO t VALUES ({}, {})", i, i))
            .unwrap();
    }
    let elapsed2 = start2.elapsed();
    let us_per2 = elapsed2.as_micros() as f64 / n as f64;
    eprintln!(
        "INSERT (NoSync):    {} rows in {:.1}ms ({:.0}µs/op, {:.0} ops/sec)",
        n,
        elapsed2.as_micros() as f64 / 1000.0,
        us_per2,
...
fn test_perf_select_throughput() {
    ...
    let us_per = elapsed.as_micros() as f64 / n as f64;
    eprintln!(
        "SELECT by PK:       {} queries in {:.1}ms ({:.0}µs/op, {:.0} ops/sec)",
        n,
        elapsed.as_micros() as f64 / 1000.0,
        us_per,
        n as f64 / elapsed.as_secs_f64()
...
fn test_perf_prepared_select() {
    ...
    for i in 1..=n {
        let _ = db
            .execute_prepared(sql, vec![Value::Integer(i)])
            .unwrap()
            .materialize()
            .unwrap();
    }
    ...
    let us_per = elapsed.as_micros() as f64 / n as f64;
    eprintln!(
        "Prepared SELECT: {} queries in {:.1}ms ({:.0}µs/op, {:.0} ops/sec)",
        n,
        elapsed.as_micros() as f64 / 1000.0,
        us_per,
        n as f64 / elapsed.as_secs_f64()
...
fn test_perf_update_throughput() {
    ...
    let us_per = elapsed.as_micros() as f64 / n as f64;
    eprintln!(
        "UPDATE by PK:       {} updates in {:.1}ms ({:.0}µs/op, {:.0} ops/sec)",
        n,
        elapsed.as_micros() as f64 / 1000.0,
        us_per,
        n as f64 / elapsed.as_secs_f64()
...
fn test_perf_full_scan() {
    ...
    let elapsed = start.elapsed();
    eprintln!(
        "Full scan COUNT(*) 1000 rows: {:.1}ms",
        elapsed.as_micros() as f64 / 1000.0
    );
    ...
    let elapsed = start.elapsed();
    eprintln!(
        "Filtered scan (v>900, ~100 rows): {:.1}ms, returned {} rows",
        elapsed.as_micros() as f64 / 1000.0,
        rows.len()
    );
    assert!(rows.len() > 90);
