...
fn ten_checkpoints_data_integrity() {
    ...
    for round in 0..10 {
        ...
        assert_eq!(
            ...
            expected_count,
            "after checkpoint {}, count mismatch",
            round + 1
        );
        assert_eq!(
            scalar_i64(&db, "SELECT SUM(v) FROM t"),
            expected_sum,
            "after checkpoint {}, sum mismatch",
            round + 1
        );
    }
...
fn where_like_and_in_combo() {
    ...
    exec(
        &db,
        "CREATE TABLE t (id INT PRIMARY KEY, name TEXT, cat INT)",
    );
    ...
    let r = rows(
        &db,
        "SELECT id FROM t WHERE name LIKE 'apple%' AND cat IN (1, 2) ORDER BY id",
    );
    assert_eq!(r.len(), 2);
}
...
fn where_is_null_and_is_not_null() {
    ...
    let r = rows(
        &db,
        "SELECT id FROM t WHERE a IS NULL OR b IS NULL ORDER BY id",
    );
    assert_eq!(r.len(), 3);
}
...
fn nested_or_and_with_parens() {
    ...
    let r = rows(
        &db,
        "SELECT id FROM t WHERE (a = 10 OR a = 30) AND (b = 100 OR b = 300) ORDER BY id",
    );
    assert_eq!(r.len(), 2);
}
...
fn count_stable_after_many_updates() {
    ...
    for cycle in 0..5 {
        ...
        assert_eq!(
            scalar_i64(&db, "SELECT COUNT(*) FROM t"),
            100,
            "count stable after UPDATE cycle {}",
            cycle
        );
    }
}
...
fn everything_combined_query() {
    ...
    for i in 1..=50 {
        ...
        exec(
            &db,
            &format!("INSERT INTO t VALUES ({}, '{}', {}, {})", i, cat, i, i * 2),
        );
    }
...
fn multi_table_count_independent() {
    ...
    for i in 1..=20 {
        exec(&db, &format!("INSERT INTO b VALUES ({})", i));
    }
    for i in 1..=10 {
        exec(&db, &format!("INSERT INTO c VALUES ({})", i));
    }
