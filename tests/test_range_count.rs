...
fn count(db: &Database, sql: &str) -> i64 {
    ...
    match r {
        QueryResult::Select { rows, .. } => {
            rows.first().and_then(|row| row.first()).and_then(|v| {
...
fn test_range_count_after_flush() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, cat INTEGER)").unwrap();
    ...
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE id > 195"), 5, "id > 195");
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE id >= 195"), 6, "id >= 195");
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE id < 5"), 4, "id < 5");
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE id <= 5"), 5, "id <= 5");
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE id != 1"), 199, "id != 1");

    // Range predicates on non-PK column
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE cat > 7"), 40, "cat > 7"); // cat 8,9 → 20 each
    assert_eq!(count(&db, "SELECT COUNT(*) FROM t WHERE cat >= 8"), 40, "cat >= 8");
