...
fn test_txn_insert_commit_visible() {
    ...
    db.insert_row_with_txn("t", tx, vec![Value::Integer(1), Value::Integer(100)]).unwrap();
    db.insert_row_with_txn("t", tx, vec![Value::Integer(2), Value::Integer(200)]).unwrap();
...
fn test_txn_rollback_data_gone() {
    ...
    let tx = db.begin_transaction().unwrap();
    db.insert_row_with_txn("t", tx, vec![Value::Integer(2), Value::Integer(20)]).unwrap();
...
fn test_sequential_transactions_correctness() {
    ...
    for i in 1..=5 {
        db.insert_row_with_txn("t", tx1, vec![Value::Integer(i), Value::Integer(i * 10)]).unwrap();
    }
    ...
    for i in 1..=5 {
        ...
        assert_eq!(r[0], Value::Integer(i * 10), "Row {} should have v={}", i, i * 10);
    }
}
...
fn test_savepoint_rollback_partial() {
    ...
    let tx = db.begin_transaction().unwrap();
    db.insert_row_with_txn("t", tx, vec![Value::Integer(1), Value::Integer(10)]).unwrap();
    ...
    assert_eq!(r[0], Value::Integer(10), "Row before savepoint should survive");
}
...
fn test_multi_row_insert_exact() {
    ...
    db.execute("INSERT INTO t VALUES (1, 'Alice', 90.5), (2, 'Bob', 85.0), (3, 'Charlie', 92.3)").unwrap();
    ...
    match &rs[0][2] { Value::Float(f) => assert!((f - 90.5).abs() < 0.01), v => panic!("{:?}", v) }
...
fn test_named_column_insert_exact() {
    ...
    db.execute("INSERT INTO t (id, b, a) VALUES (1, 'hello', 42)").unwrap();
...
fn test_having_filters_groups_exact() {
    ...
    db.execute("INSERT INTO orders VALUES (1, 'Alice', 50)").unwrap();
    db.execute("INSERT INTO orders VALUES (2, 'Alice', 30)").unwrap();
    db.execute("INSERT INTO orders VALUES (3, 'Alice', 20)").unwrap();
    db.execute("INSERT INTO orders VALUES (4, 'Bob', 10)").unwrap();
    db.execute("INSERT INTO orders VALUES (5, 'Bob', 15)").unwrap();
    db.execute("INSERT INTO orders VALUES (6, 'Charlie', 80)").unwrap();
    db.execute("INSERT INTO orders VALUES (7, 'Charlie', 70)").unwrap();
    db.execute("INSERT INTO orders VALUES (8, 'Charlie', 60)").unwrap();
...
fn test_where_in_subquery_exact() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE orders (id INT PRIMARY KEY, cid INT, amount INT)").unwrap();
    db.execute("CREATE TABLE vip (id INT PRIMARY KEY)").unwrap();

    db.execute("INSERT INTO orders VALUES (1, 10, 100)").unwrap();
    db.execute("INSERT INTO orders VALUES (2, 20, 200)").unwrap();
    db.execute("INSERT INTO orders VALUES (3, 10, 50)").unwrap();
    db.execute("INSERT INTO orders VALUES (4, 30, 300)").unwrap();
...
fn test_scalar_subquery_in_select() {
    ...
    let rs = rows(&db, "SELECT id, v FROM t WHERE v > (SELECT AVG(v) FROM t) ORDER BY id");
...
fn test_drop_recreate_different_schema() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, x TEXT, y FLOAT)").unwrap();
    db.execute("INSERT INTO t VALUES (1, 'hello', 3.14)").unwrap();

    let r = row(&db, "SELECT id, x, y FROM t");
    assert_eq!(r[0], Value::Integer(1));
    assert_eq!(r[1], Value::Text("hello".into()));
    match &r[2] { Value::Float(f) => assert!((f - 3.14).abs() < 0.01), v => panic!("{:?}", v) }
}
...
fn test_multiple_tables_independent_crud() {
    ...
    db.execute("INSERT INTO products VALUES (10, 99.9)").unwrap();

    db.execute("UPDATE users SET name = 'Robert' WHERE id = 2").unwrap();
...
fn test_arithmetic_type_promotion() {
    ...
    match &r[0] { Value::Float(f) => assert!((f - 5.5).abs() < 0.01), v => panic!("{:?}", v) }
    match &r[1] { Value::Float(f) => assert!((f - 7.5).abs() < 0.01), v => panic!("{:?}", v) }
    match &r[2] { Value::Float(f) => assert!((f - 0.5).abs() < 0.01), v => panic!("{:?}", v) }
}
...
fn test_update_column_expression() {
    ...
    db.execute("UPDATE t SET a = b + c, b = a - 5 WHERE id = 1").unwrap();
...
fn test_wide_table_exact_values() {
    ...
    db.execute("INSERT INTO wide VALUES (1, 10, 'a', 1.1, 20, 'b', 30, 3.3, 40, 'c', 50)").unwrap();
    ...
    match &r[3] { Value::Float(f) => assert!((f - 1.1).abs() < 0.01), v => panic!("{:?}", v) }
    assert_eq!(r[4], Value::Integer(20));
    assert_eq!(r[5], Value::Text("b".into()));
    assert_eq!(r[6], Value::Integer(30));
    match &r[7] { Value::Float(f) => assert!((f - 3.3).abs() < 0.01), v => panic!("{:?}", v) }
...
fn test_wide_table_partial_update() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE wide (id INT PRIMARY KEY, c1 INT, c2 INT, c3 INT, c4 INT, c5 INT)").unwrap();
    db.execute("INSERT INTO wide VALUES (1, 10, 20, 30, 40, 50)").unwrap();

    db.execute("UPDATE wide SET c3 = 999, c5 = 888 WHERE id = 1").unwrap();
...
fn test_bulk_insert_1000_exact() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v INT, name TEXT)").unwrap();

    let n = 1000;
    for i in 0..n {
        db.execute(&format!("INSERT INTO t VALUES ({}, {}, 'row_{}')", i, i * 7, i)).unwrap();
    }
...
fn test_bulk_update_conditional_exact() {
    ...
    db.execute("UPDATE t SET v = v * 2 WHERE v % 2 = 0").unwrap();
...
fn test_prepared_insert_select_exact() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, name TEXT, v INT)").unwrap();

    db.execute_prepared("INSERT INTO t VALUES (?, ?, ?)",
        vec![Value::Integer(1), Value::Text("x".into()), Value::Integer(100)]).unwrap();
    db.execute_prepared("INSERT INTO t VALUES (?, ?, ?)",
        vec![Value::Integer(2), Value::Text("y".into()), Value::Integer(200)]).unwrap();

    let rs = rows(&db, "SELECT id, name, v FROM t ORDER BY id");
    assert_eq!(rs.len(), 2);
    assert_eq!(rs[0], vec![Value::Integer(1), Value::Text("x".into()), Value::Integer(100)]);
    assert_eq!(rs[1], vec![Value::Integer(2), Value::Text("y".into()), Value::Integer(200)]);
}
...
fn test_prepared_where_exact() {
    ...
    let result = db.execute_prepared("SELECT id, v FROM t WHERE v > ? ORDER BY id",
        vec![Value::Integer(100)]).unwrap().materialize().unwrap();
...
fn test_prepared_update_exact() {
    ...
    db.execute_prepared("UPDATE t SET v = ? WHERE id = ?",
        vec![Value::Integer(999), Value::Integer(1)]).unwrap();
...
fn test_nested_arithmetic_exact() {
    ...
    let r = row(&db, "SELECT (a + b) * c, a - b / c, (a + b) / (c + 1) FROM t WHERE id = 1");
...
fn test_nested_where_conditions() {
    ...
    let rs = rows(&db, "SELECT id FROM t WHERE (a = 1 AND b = 1) OR c = 0 ORDER BY id");
    assert_eq!(rs.len(), 3);
    assert_eq!(rs[0][0], Value::Integer(1));
    assert_eq!(rs[1][0], Value::Integer(2));
...
fn test_multiple_close_open_cycles() {
...

    {
        ...
        db.execute("UPDATE t SET v = 'second' WHERE id = 1").unwrap();
...
fn test_if_function_exact() {
    ...
    let rs = rows(&db, "SELECT id, IF(v > 20, 'high', 'low') FROM t ORDER BY id");
...
fn test_full_pipeline_group_where_order() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE sales (id INT PRIMARY KEY, region TEXT, product TEXT, amount INT)").unwrap();
    db.execute("INSERT INTO sales VALUES (1, 'North', 'Laptop', 1000)").unwrap();
    db.execute("INSERT INTO sales VALUES (2, 'North', 'Laptop', 500)").unwrap();
    db.execute("INSERT INTO sales VALUES (3, 'North', 'Phone', 300)").unwrap();
    db.execute("INSERT INTO sales VALUES (4, 'South', 'Laptop', 800)").unwrap();
    db.execute("INSERT INTO sales VALUES (5, 'South', 'Laptop', 200)").unwrap();
    db.execute("INSERT INTO sales VALUES (6, 'South', 'Phone', 600)").unwrap();
    db.execute("INSERT INTO sales VALUES (7, 'South', 'Phone', 400)").unwrap();
    db.execute("INSERT INTO sales VALUES (8, 'East', 'Phone', 700)").unwrap();
...
fn test_join_aggregate_exact() {
    ...
    db.execute("INSERT INTO orders VALUES (10, 1, 100)").unwrap();
    db.execute("INSERT INTO orders VALUES (11, 1, 200)").unwrap();
    ...
    assert_eq!(rs2.len(), 3);
    assert_eq!(rs2[0], vec![Value::Text("Alice".into()), Value::Integer(100)]);
    assert_eq!(rs2[1], vec![Value::Text("Alice".into()), Value::Integer(200)]);
...
fn test_replace_exact() {
    ...
    let r = row(&db, "SELECT REPLACE(s, 'world', 'MoteDB') FROM t WHERE id = 1");
...
fn test_all_types_roundtrip() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, i INT, f FLOAT, s TEXT, b BOOLEAN)").unwrap();
    db.execute("INSERT INTO t VALUES (1, -42, 3.14, 'hello', TRUE)").unwrap();

    let r = row(&db, "SELECT i, f, s, b FROM t WHERE id = 1");
    assert_eq!(r[0], Value::Integer(-42));
    match &r[1] { Value::Float(f) => assert!((f - 3.14).abs() < 0.001), v => panic!("{:?}", v) }
    assert_eq!(r[2], Value::Text("hello".into()));
