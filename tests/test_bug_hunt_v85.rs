...
fn test_group_count_matches_distinct() {
    ...
    db.execute("INSERT INTO t VALUES (1,'a'),(2,'a'),(3,'b'),(4,'c'),(5,'c')").unwrap();
    let groups = q(&db, "SELECT cat, COUNT(*) FROM t GROUP BY cat");
    let distinct = q(&db, "SELECT DISTINCT cat FROM t");
    assert_eq!(groups.len(), distinct.len(), "GROUP BY groups == DISTINCT values");
}
...
fn test_in_subquery_matches_join() {
    let (db, _d) = db();
    db.execute("CREATE TABLE parent(id INT PRIMARY KEY, kind TEXT)").unwrap();
    db.execute("CREATE TABLE child(id INT PRIMARY KEY, pid INT)").unwrap();
    db.execute("INSERT INTO parent VALUES (1,'x'),(2,'y'),(3,'x')").unwrap();
    db.execute("INSERT INTO child VALUES (10,1),(11,3),(12,2),(13,99)").unwrap();
    // parent rows referenced by some child.
    let via_in = sorted_int(&q(&db, "SELECT id FROM parent WHERE id IN (SELECT pid FROM child)"));
    let via_join = sorted_int(&q(&db, "SELECT parent.id FROM parent JOIN child ON parent.id = child.pid"));
...
fn test_and_with_is_null() {
    ...
    db.execute("INSERT INTO t VALUES (1,10,NULL),(2,10,5),(3,NULL,5)").unwrap();
...
fn test_aggregate_after_update() {
    ...
    assert_eq!(q(&db, "SELECT SUM(v) FROM t"), vec![vec![Value::Integer(120)]]);
    assert_eq!(q(&db, "SELECT MAX(v) FROM t"), vec![vec![Value::Integer(60)]]);
}
...
fn test_aggregate_after_delete() {
    ...
    assert_eq!(q(&db, "SELECT SUM(v) FROM t"), vec![vec![Value::Integer(10)]]);
    assert_eq!(q(&db, "SELECT COUNT(*) FROM t"), vec![vec![Value::Integer(1)]]);
}
...
fn test_reinsert_after_delete_aggregate() {
    ...
    assert_eq!(q(&db, "SELECT SUM(v) FROM t"), vec![vec![Value::Integer(40)]]);
...
fn test_text_where_eq_and_in() {
    ...
    db.execute("INSERT INTO t VALUES (1,'foo'),(2,'bar'),(3,'baz')").unwrap();
...
fn test_count_text_where() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, cat TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'a'),(2,'a'),(3,'b'),(4,'a')").unwrap();
    assert_eq!(q(&db, "SELECT COUNT(*) FROM t WHERE cat = 'a'"), vec![vec![Value::Integer(3)]]);
}
...
fn test_count_distinct_vs_subquery() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,20),(3,10),(4,20),(5,30)").unwrap();
    let via_count = q(&db, "SELECT COUNT(DISTINCT v) FROM t");
    let via_subq = q(&db, "SELECT COUNT(*) FROM (SELECT DISTINCT v FROM t) AS sub");
    assert_eq!(via_count, vec![vec![Value::Integer(3)]]);
    assert_eq!(via_subq, via_count, "COUNT(DISTINCT) must equal count over DISTINCT subquery");
}
...
fn test_sum_where_single() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,20),(3,30)").unwrap();
    assert_eq!(q(&db, "SELECT SUM(v) FROM t WHERE id = 2"), vec![vec![Value::Integer(20)]]);
}
...
fn test_min_matches_order_limit() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,30),(2,10),(3,50),(4,20)").unwrap();
...
fn test_in_negative_literals() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,-5),(2,0),(3,-10),(4,5)").unwrap();
...
fn test_between_negative() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,-20),(2,-5),(3,0),(4,5)").unwrap();
...
fn test_case_multi_branch() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1),(2,2),(3,3),(4,4)").unwrap();
...
fn test_where_multiplication() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,3,4),(2,5,2),(3,2,10)").unwrap();
...
fn test_order_by_arithmetic_expr() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10,1),(2,5,5),(3,1,10)").unwrap();
