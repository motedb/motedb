...
fn test_txn_writes_visible_after_commit_lsm() {
    ...
    db.insert_row_with_txn("t", txn, vec![Value::Integer(1), Value::Integer(100)]).unwrap();
    ...
    assert_eq!(rows.len(), 1, "Committed transactional row should be visible");
...
fn test_txn_inserts_survive_crash() {
...

    {
        ...
        let txn = db.begin_transaction().unwrap();
        db.insert_row_with_txn("t", txn, vec![Value::Integer(1), Value::Integer(111)]).unwrap();
        db.insert_row_with_txn("t", txn, vec![Value::Integer(2), Value::Integer(222)]).unwrap();
        db.commit_transaction(txn).unwrap();
...

    {
        ...
        assert_eq!(cnt, 3, "All 3 rows should survive crash (2 txn + 1 auto-commit)");

        // Verify transactional rows
        let r1 = exec(&db, "SELECT val FROM t WHERE id = 1");
        let rows1 = match r1 { QueryResult::Select { rows, .. } => rows, _ => vec![] };
        assert_eq!(rows1[0][0], Value::Integer(111));

        let r2 = exec(&db, "SELECT val FROM t WHERE id = 2");
        let rows2 = match r2 { QueryResult::Select { rows, .. } => rows, _ => vec![] };
...
fn test_write_write_conflict_detection() {
    ...
    db.insert_row_with_txn("t", txn1, vec![Value::Integer(1), Value::Integer(100)]).unwrap();
...
fn test_mixed_autocommit_and_txn_writes() {
...

    {
        ...
        let txn = db.begin_transaction().unwrap();
        db.insert_row_with_txn("t", txn, vec![Value::Integer(2), Value::Integer(20)]).unwrap();
        db.commit_transaction(txn).unwrap();
