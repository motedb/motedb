...
fn test_min_max_text_with_groupby() {
    ...
    db.execute("INSERT INTO t VALUES (1,'banana'),(1,'apple'),(2,'zebra'),(2,'ant')")
        .unwrap();
...
fn test_having_agg_not_in_select() {
    ...
    let r = q(
        &db,
        "SELECT g FROM t GROUP BY g HAVING SUM(v) > 10 ORDER BY g",
    );
...
fn test_having_count_not_in_select() {
    ...
    let r = q(
        &db,
        "SELECT g FROM t GROUP BY g HAVING COUNT(*) >= 3 ORDER BY g",
    );
    assert_eq!(r, vec![vec![Value::Integer(1)]]);
}
...
fn test_having_avg_not_in_select() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(g INT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(1,30),(2,5),(2,5)")
        .unwrap();
    // g1 avg=20, g2 avg=5. HAVING AVG > 10 → g1
    let r = q(
        &db,
        "SELECT g FROM t GROUP BY g HAVING AVG(v) > 10 ORDER BY g",
    );
    assert_eq!(r, vec![vec![Value::Integer(1)]]);
}
...
fn test_subquery_min_in_having() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(g INT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(1,20),(2,5),(2,50)")
        .unwrap();
    // Groups whose SUM > average of all sums... simpler: groups with SUM > 15.
    let r = q(
        &db,
        "SELECT g FROM t GROUP BY g HAVING SUM(v) > 15 ORDER BY g",
    );
    // g1=30>15 ✓, g2=55>15 ✓
    assert_eq!(r, vec![vec![Value::Integer(1)], vec![Value::Integer(2)]]);
}
...
fn test_where_multiple_columns_and() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT, c INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,20,30),(2,10,25,30),(3,15,20,40)")
        .unwrap();
...
fn test_where_between_and_in() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT, cat TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,'a'),(2,20,'b'),(3,30,'a'),(4,15,'c')")
        .unwrap();
    // v BETWEEN 10 AND 25 AND cat IN ('a','c')
    let r = q(
        &db,
        "SELECT id FROM t WHERE v BETWEEN 10 AND 25 AND cat IN ('a','c') ORDER BY id",
    );
