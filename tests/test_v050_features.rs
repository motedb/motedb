...
fn test_integer_columns_not_read_as_float() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, age INTEGER, score INTEGER)").unwrap();
...
fn test_mixed_integer_and_float_columns() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, ival INTEGER, fval FLOAT)").unwrap();
    db.execute("INSERT INTO t VALUES (1, 42, 3.14)").unwrap();

    let rows = select_rows(&db, "SELECT * FROM t");
    assert_eq!(rows[0][0], Value::Integer(1));
    assert_eq!(rows[0][1], Value::Integer(42), "integer column must stay Integer");
    assert!(matches!(rows[0][2], Value::Float(_)), "float column must be Float");
}
...
fn test_update_preserves_other_rows() {
    ...
    assert_eq!(rows[2].iter().find(|v| matches!(v, Value::Integer(999))), Some(&Value::Integer(999)));
...
fn test_repeated_updates_same_row() {
    ...
    for new_v in 2..=20 {
        db.execute(&format!("UPDATE t SET v = {} WHERE id = 1", new_v)).unwrap();
    }
    let rows = select_rows(&db, "SELECT * FROM t");
    assert_eq!(rows.len(), 1, "20 updates must not create duplicates");
    assert_eq!(rows[0][1], Value::Integer(20), "must reflect the last update");
}
...
fn test_update_survives_restart() {
    ...
    let rows = select_rows(&db, "SELECT * FROM t");
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0][1], Value::Integer(200), "updated value must survive restart");
}
...
fn test_delete_survives_restart() {
    ...
    let db = Database::open(&path).unwrap();
    assert_eq!(count(&db, "t"), 1, "deleted row must stay deleted after restart");
    let rows = select_rows(&db, "SELECT * FROM t");
    assert_eq!(rows.len(), 1);
...
fn test_order_by_limit_desc_correct() {
    ...
    for i in 0..100 {
        db.execute(&format!("INSERT INTO t (v) VALUES ({})", i as f64 * 0.5)).unwrap();
    }
    ...
    let first = match &rows[0][0] { Value::Float(f) => *f, _ => 0.0 };
    let last = match &rows[4][0] { Value::Float(f) => *f, _ => 0.0 };
    assert!(first >= last, "DESC order: first ({}) >= last ({})", first, last);
}
...
fn test_order_by_limit_preserves_count() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY AUTO_INCREMENT, v FLOAT)").unwrap();
    for i in 0..50 {
        db.execute(&format!("INSERT INTO t (v) VALUES ({})", i as f64)).unwrap();
    }
...
fn test_distinct_low_cardinality() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY AUTO_INCREMENT, region TEXT)").unwrap();
    for i in 0..100 {
        let region = if i % 3 == 0 { "US" } else { "EU" };
        db.execute(&format!("INSERT INTO t (region) VALUES ('{}')", region)).unwrap();
    }
    let rows = select_rows(&db, "SELECT DISTINCT region FROM t");
    let values: Vec<String> = rows.iter().filter_map(|r| {
        if let Value::Text(t) = &r[0] { Some(t.as_str().to_string()) } else { None }
    }).collect();
...
fn test_indexed_where_equality_correct() {
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY AUTO_INCREMENT, name TEXT, region TEXT)").unwrap();
    for i in 0..100 {
        let region = if i % 3 == 0 { "US" } else { "EU" };
        db.execute(&format!("INSERT INTO t (name, region) VALUES ('n{}', '{}')", i, region)).unwrap();
    }
    ...
    assert_eq!(rows.len(), expected, "indexed WHERE must return correct count");
    // All returned rows must be US
    for row in &rows {
        let region = match &row[2] { Value::Text(t) => t.as_str().to_string(), _ => String::new() };
...
fn test_index_lookup_after_bulk_load_500_entries() {
    ...
    let (_dir, db) = make_db();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY AUTO_INCREMENT, tag TEXT)").unwrap();
    for i in 0..500 {
        db.execute(&format!("INSERT INTO t (tag) VALUES ('val{}')", i)).unwrap();
    }
    db.execute("CREATE INDEX idx_key ON t (tag) USING COLUMN").unwrap();
...
fn test_auto_increment_select_all() {
    ...
    for i in 0..10 {
        db.execute(&format!("INSERT INTO t (v) VALUES ('x{}')", i)).unwrap();
    }
    let rows = select_rows(&db, "SELECT * FROM t");
    assert_eq!(rows.len(), 10);
    // IDs should be 1..=10
    for (i, row) in rows.iter().enumerate() {
        assert_eq!(row[0], Value::Integer((i + 1) as i64), "auto-increment id at {}", i);
    }
}
...
fn test_like_prefix_filter() {
    ...
    db.execute("INSERT INTO t (name) VALUES ('apricot')").unwrap();
    db.execute("INSERT INTO t (name) VALUES ('banana')").unwrap();
