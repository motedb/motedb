...
fn test_distinct_order_by() {
    ...
    db.execute("INSERT INTO t VALUES (1,3),(2,1),(3,3),(4,1),(5,2)").unwrap();
...
fn test_distinct_multiple_columns() {
    ...
    db.execute("INSERT INTO t VALUES (1,1),(1,1),(1,2),(2,1)").unwrap();
...
fn test_distinct_with_null() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,NULL),(2,NULL),(3,5),(4,5)").unwrap();
    let r = q(&db, "SELECT DISTINCT v FROM t ORDER BY v");
    // NULL + 5 (two distinct)
    assert_eq!(r.len(), 2);
}
...
fn test_update_affected_count() {
    ...
    assert_eq!(r, vec![vec![Value::Integer(10)], vec![Value::Integer(0)], vec![Value::Integer(0)]]);
}
...
fn test_avg_with_some_null() {
    ...
    assert!((f_of(&r[0][0]) - 20.0).abs() < 1e-9, "AVG should ignore NULL, got {:?}", r);
}
...
fn test_min_max_ignore_null() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,NULL),(3,30),(4,5)").unwrap();
...
fn test_join_groupby_aggregate() {
    ...
    db.execute("CREATE TABLE cust(id INT PRIMARY KEY, region TEXT)").unwrap();
    db.execute("INSERT INTO orders VALUES (1,1,100),(2,1,200),(3,2,50),(4,2,150)").unwrap();
    db.execute("INSERT INTO cust VALUES (1,'US'),(2,'EU')").unwrap();
    let r = q(
        &db,
    ...
    assert_eq!(
        r,
        vec![vec![Value::text("EU".into()), Value::Integer(200)], vec![Value::text("US".into()), Value::Integer(300)]]
    );
}
...
fn test_groupby_count_star() {
    ...
    db.execute("INSERT INTO t VALUES (1,10),(1,20),(1,30),(2,5)").unwrap();
    let r = q(&db, "SELECT g, COUNT(*) FROM t GROUP BY g ORDER BY g");
    assert_eq!(
        r,
        vec![vec![Value::Integer(1), Value::Integer(3)], vec![Value::Integer(2), Value::Integer(1)]]
    );
}
...
fn test_upper_lower() {
    ...
    assert_eq!(r, vec![vec![Value::text("ABC".into()), Value::text("xyz".into())]]);
}
...
fn test_compare_int_float() {
    ...
    assert_eq!(r, vec![vec![Value::Bool(true), Value::Bool(true), Value::Bool(true), Value::Bool(false)]]);
}
...
fn test_cast_float_to_text() {
    ...
    assert!(s.starts_with("3.14"), "CAST(3.14 AS TEXT) should start with 3.14, got {}", s);
}
...
fn test_between_inclusive() {
    ...
    let r = q(&db, "SELECT id FROM t WHERE v BETWEEN 10 AND 20 ORDER BY id");
...
fn test_not_between() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,5),(2,10),(3,15)").unwrap();
    let r = q(&db, "SELECT id FROM t WHERE v NOT BETWEEN 8 AND 12 ORDER BY id");
...
fn test_between_text() {
    ...
    let r = q(&db, "SELECT s FROM t WHERE s BETWEEN 'b' AND 'd' ORDER BY s");
    assert_eq!(r, vec![vec![Value::text("banana".into())], vec![Value::text("cherry".into())]]);
}
...
fn test_delete_with_in_subquery() {
    let (db, _d) = db();
    db.execute("CREATE TABLE main(id INT PRIMARY KEY, cat TEXT)").unwrap();
    db.execute("CREATE TABLE excl(cat TEXT)").unwrap();
    db.execute("INSERT INTO main VALUES (1,'a'),(2,'b'),(3,'c')").unwrap();
...
fn test_update_with_subquery_where() {
    ...
    let n = affected(&db, "UPDATE t SET v = 0 WHERE id IN (SELECT id FROM marker)");
    assert_eq!(n, 2);
    let r = q(&db, "SELECT v FROM t ORDER BY id");
...
fn test_update_set_from_subquery() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("CREATE TABLE src(id INT PRIMARY KEY, nv INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10)").unwrap();
...
fn test_count_with_is_null() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,NULL),(3,NULL)").unwrap();
...
fn test_and_or_precedence() {
    ...
    db.execute("INSERT INTO t VALUES (1,1,1),(2,1,0),(3,0,1),(4,0,1)").unwrap();
    // WHERE a = 1 OR b = 1 AND id = 4 → a=1 OR (b=1 AND id=4)
    // id1: a=1 T ; id2: a=1 T ; id3: b=1 but id=3 F, a=0 F ; id4: a=0, (b=1 AND id=4) T
    let r = q(&db, "SELECT id FROM t WHERE a = 1 OR b = 1 AND id = 4 ORDER BY id");
    assert_eq!(
...
fn test_and_or_precedence_paren_override() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1,1),(2,1,0),(3,0,1),(4,0,0)").unwrap();
    // WHERE (a = 1 OR b = 1) AND id = 1 → only id1 satisfies both
    let r = q(&db, "SELECT id FROM t WHERE (a = 1 OR b = 1) AND id = 1 ORDER BY id");
    assert_eq!(
        r.iter().map(|row| match &row[0] { Value::Integer(i) => *i, _ => -1 }).collect::<Vec<_>>(),
        vec![1]
    );
