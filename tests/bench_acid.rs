...
fn test_checkpoint_integrity() {
    ...
    // Phase 1: Insert + flush + checkpoint
    {
        ...
        let insert_ms = start.elapsed().as_millis() as u64;
        print_result!("Phase 1: INSERT 10K", n, insert_ms);
    ...
    let mut errors = 0;
    for i in [1, 100, 500, 1000, 5000, 10000] {
    ...
    println!("  -> Spot-check errors: {}/6", errors);
...
fn test_update_delete_consistency() {
    ...
    let insert_ms = start.elapsed().as_millis() as u64;
    print_result!("INSERT 1K", n, insert_ms);
    ...
    print_result!("UPDATE 500", 500, update_ms);
    ...
    let unchanged_score = query_val(&db, "SELECT score FROM crud WHERE id = 800");
    assert_eq!(unchanged_score, 800, "Row 800 score should be unchanged (800)");
    println!("  -> UPDATE verified: row 100 score={} (expect 200), row 800 score={} (expect 800) ✓",
        updated_score, unchanged_score);
    ...
    print_result!("DELETE 200", 200, delete_ms);

    // Verify deletes
    let count_after = query_val(&db, "SELECT COUNT(*) AS cnt FROM crud");
    assert_eq!(count_after, 800, "Should have 800 rows after deleting 200");
    let deleted = query_val(&db, "SELECT COUNT(*) AS cnt FROM crud WHERE id BETWEEN 600 AND 799");
...
fn test_concurrent_consistency() {
    ...
    let ms = start.elapsed().as_millis() as u64;
    print_result!("Concurrent INSERT 10K", n_total, ms);
...
fn test_mixed_oltp() {
    ...
    let update_ms = start.elapsed().as_millis() as u64;
    print_result!("UPDATE 2.5K", 2500, update_ms);
    ...
    println!("  -> completed={}, pending={} (expect 2500/2500)", completed, pending);
    assert_eq!(completed, 2500);
    assert_eq!(pending, 2500);
    ...
    let delete_ms = start.elapsed().as_millis() as u64;
    print_result!("DELETE 1K", 1000, delete_ms);

    // Final verification
    let final_count = query_val(&db, "SELECT COUNT(*) AS cnt FROM oltp");
    assert_eq!(final_count, 4000, "Should have 4000 rows after deleting 1000");
...
fn test_sstable_query_correctness() {
...

    {
        ...
        let insert_ms = start.elapsed().as_millis() as u64;
        print_result!("INSERT 10K (MemTable)", n, insert_ms);
    ...
    // Verify random rows
    let mut errors = 0;
    for i in [1, 50, 100, 500, 1000, 5000, 9999, 10000] {
        let v = query_val(&db, &format!("SELECT id FROM sst WHERE id = {}", i));
        if v != i { errors += 1; }
    }
    println!("  -> Post-recovery verification: {} rows, {} spot-check errors", count, errors);
...
fn test_edge_resource_usage() {
    ...
    print_result!("INSERT 50K (edge config)", n, insert_ms);
