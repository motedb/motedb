...
fn row_count(db: &Database, sql: &str) -> usize {
    let r = db.execute(sql).expect("execute").materialize().expect("materialize");
    r.select_rows().map(|(_, rows)| rows.len()).unwrap_or(r.affected_rows())
}

fn bench<F: FnMut()>(label: &str, n: usize, mut f: F) -> u64 {
    f(); // warmup
    let iters = if n < 10_000 { 200 } else if n < 50_000 { 100 } else { 30 };
    let start = Instant::now();
    for _ in 0..iters { f(); }
    let us = start.elapsed().as_micros() as u64 / iters;
    println!("  {:45} | {:>6} rows | {:>8} us/op | {:>6} rows/ms", label, n, us, n as u64 * 1000 / us.max(1));
    us
}
...
fn test_scalability_latency() {
    ...
    println!("  {}", "=".repeat(80));
    println!("  {:45} | {:>6}       | {:>8}     | {:>8}", "Operation", "Rows", "us/op", "rows/ms");
    println!("  {}", "-".repeat(80));
    ...
    for &n in &sizes {
        ...
        times.push(bench("SELECT * (full scan)", n, || { row_count(&db, "SELECT * FROM sales"); }));
        times.push(bench("WHERE region='US' (filter)", n, || { row_count(&db, "SELECT * FROM sales WHERE region = 'US'"); }));
        times.push(bench("GROUP BY customer + COUNT(*)", n, || { row_count(&db, "SELECT customer, COUNT(*) FROM sales GROUP BY customer"); }));
        times.push(bench("ORDER BY amount DESC LIMIT 10", n, || { row_count(&db, "SELECT * FROM sales ORDER BY amount DESC LIMIT 10"); }));
        times.push(bench("SELECT DISTINCT region", n, || { row_count(&db, "SELECT DISTINCT region FROM sales"); }));
        ...
        times.push(bench("WHERE customer LIKE 'cust_1%'", n, || { row_count(&db, "SELECT * FROM sales WHERE customer LIKE 'cust_1%'"); }));
        times.push(bench("PK SELECT by id", n, || { row_count(&db, &format!("SELECT * FROM sales WHERE id = {}", n / 2)); }));

        // Linearity check
        if let Some((prev_n, prev_times)) = &prev {
            let data_ratio = n as f64 / *prev_n as f64;
            println!("  >>> Scaling from {}K to {}K rows (data {:.1}x):", prev_n / 1000, n / 1000, data_ratio);
            let labels = ["scan", "WHERE", "GROUP BY", "ORDER BY", "DISTINCT", "IN subquery", "LIKE", "PK SELECT"];
            for (i, label) in labels.iter().enumerate() {
                let time_ratio = times[i] as f64 / prev_times[i] as f64;
                let verdict = if time_ratio <= data_ratio * 1.3 { "OK (linear)" }
                              else if time_ratio <= data_ratio * 2.5 { "WARN" }
                              else { "BAD (super-linear)" };
                println!("      {:20} time {:.1}x / data {:.1}x  =>  {}  ({} us -> {} us)",
...
fn test_scalability_memory() {
    ...
    println!("\n  MEMORY vs DATA SIZE");
    println!("  {}", "=".repeat(60));
    println!("  {:>8} | {:>10} | {:>10} | {:>10} | {:>10}", "Rows", "RSS KB", "B/row", "Disk KB", "B/row");
    println!("  {}", "-".repeat(60));
    ...
    for &n in &sizes {
        ...
        let disk_kb = {
            let output = std::process::Command::new("du")
                ...
                .output().ok();
            output.and_then(|o| {
                String::from_utf8_lossy(&o.stdout).split_whitespace().next()?.parse::<u64>().ok()
            }).unwrap_or(0)
        };

        println!("  {:>8} | {:>8} KB | {:>8} B | {:>8} KB | {:>8} B",
            n, rss_kb, rss_kb * 1024 / n as u64, disk_kb, disk_kb * 1024 / n as u64);
        ...
        if let Some((prev_n, prev_rss, prev_disk)) = &prev {
            ...
            let verdict_rss = if rss_ratio <= data_ratio * 1.5 { "linear" } else { "super-linear" };
            let verdict_disk = if disk_ratio <= data_ratio * 1.5 { "linear" } else { "super-linear" };
            println!("    >>> RSS {:.1}x, Disk {:.1}x vs {:.1}x data growth => RSS {}, Disk {}",
