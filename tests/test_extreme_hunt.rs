...
fn test_emoji_in_text() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, msg TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'Hello 🦀 Rust')")
        .unwrap();
    db.flush().unwrap();
...
fn test_text_with_quotes() {
    ...
    db.execute("INSERT INTO t VALUES (1, 'it''s great')")
        .unwrap();
    db.flush().unwrap();
...
fn test_sum_large_integers_no_overflow() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, v BIGINT)")
        .unwrap();
    db.execute(&format!("INSERT INTO t VALUES (1, {})", i64::MAX / 2))
        .unwrap();
    db.execute(&format!("INSERT INTO t VALUES (2, {})", i64::MAX / 2))
        .unwrap();
    db.flush().unwrap();
...
fn test_group_by_order_by_limit_combined() {
    ...
    let r = rows(
        &db,
        "SELECT dept, SUM(salary) FROM t GROUP BY dept ORDER BY dept LIMIT 2",
    );
...
fn test_scalar_subquery_in_select() {
    ...
    let r = rows(
        &db,
        "SELECT id, (SELECT MAX(v) FROM t) AS max_v FROM t WHERE id = 1",
    );
    assert_eq!(r.len(), 1);
...
fn test_where_between() {
    ...
    db.flush().unwrap();
    assert_eq!(
        cnt(&db, "SELECT COUNT(*) FROM t WHERE v BETWEEN 3 AND 7"),
        5
    );
    assert_eq!(
        cnt(&db, "SELECT COUNT(*) FROM t WHERE v NOT BETWEEN 3 AND 7"),
        5
    );
}
...
fn test_recovery_preserves_unicode() {
    ...
    assert_eq!(
        val(&db, "SELECT name FROM t WHERE id = 2"),
        Value::text("한국어".into())
    );
    assert_eq!(
        val(&db, "SELECT name FROM t WHERE id = 3"),
        Value::text("العربية".into())
    );
}
...
fn test_recovery_many_tables() {
    ...
    let path = dir.path().to_path_buf();
    {
        let db = Database::create(&path).unwrap();
        for t in 0..10 {
            db.execute(&format!("CREATE TABLE t{} (id INT PRIMARY KEY, v INT)", t))
                .unwrap();
            for i in 1..=5 {
                db.execute(&format!("INSERT INTO t{} VALUES ({}, {})", t, i, i))
                    .unwrap();
            }
        }
        db.checkpoint().unwrap();
    ...
    for t in 0..10 {
        assert_eq!(
            cnt(&db, &format!("SELECT COUNT(*) FROM t{}", t)),
            5,
            "Table t{} should have 5 rows",
            t
        );
    }
}
...
fn test_sum_where_grouped() {
    let (db, _d) = mk();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, dept TEXT, amt INT)")
        .unwrap();
    ...
    let r = rows(
        &db,
        "SELECT dept, SUM(amt) FROM t WHERE amt > 60 GROUP BY dept",
    );
...
fn test_count_group_by_where_different_col() {
    let (db, _d) = mk();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, cat TEXT, status TEXT)")
        .unwrap();
    for i in 1..=20 {
    ...
    let r = rows(
        &db,
        "SELECT cat, COUNT(*) FROM t WHERE status = 'active' GROUP BY cat",
    );
...
fn test_create_many_tables_query() {
    ...
    for t in 0..5 {
        db.execute(&format!(
            "CREATE TABLE tab{} (id INT PRIMARY KEY, v INT)",
            t
        ))
        .unwrap();
        db.execute(&format!("INSERT INTO tab{} VALUES (1, {})", t, t * 100))
            .unwrap();
    }
    db.flush().unwrap();
    for t in 0..5 {
        assert_eq!(
            val(&db, &format!("SELECT v FROM tab{} WHERE id = 1", t)),
            Value::Integer(t * 100)
        );
    }
}
...
fn test_full_scan_after_mixed_ops() {
    ...
    assert_eq!(
        val(&db, "SELECT v FROM t WHERE id = 25"),
        Value::Integer(25)
    );
}
