...
fn bench_column_index_creation() {
    ...
    let cat_idx_ms = {
        ...
        exec(
            &db,
            "CREATE INDEX idx_cat ON products (category) USING COLUMN",
        );
        db.wait_for_indexes_ready();
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("CREATE COLUMN INDEX on category ({} rows)", n),
        1,
        cat_idx_ms,
    );
    ...
    let price_idx_ms = {
        ...
        exec(
            &db,
            "CREATE INDEX idx_price ON products (price) USING COLUMN",
        );
        db.wait_for_indexes_ready();
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("CREATE COLUMN INDEX on price ({} rows)", n),
        1,
        price_idx_ms,
    );
    ...
    let stock_idx_ms = {
        ...
        exec(
            &db,
            "CREATE INDEX idx_stock ON products (stock) USING COLUMN",
        );
        db.wait_for_indexes_ready();
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("CREATE COLUMN INDEX on stock ({} rows)", n),
        1,
        stock_idx_ms,
    );

    println!(
        "  -> Total index build time: {}ms",
        cat_idx_ms + price_idx_ms + stock_idx_ms
    );
    db.close().ok();
}
...
fn bench_column_index_queries() {
    ...
    exec(
        &db,
        "CREATE TABLE items (id INT PRIMARY KEY, tag TEXT, score FLOAT, qty INT)",
    );
    ...
    let speedup = if no_idx_per > 0.0 {
        no_idx_per / eq_per.max(0.01)
    } else {
        0.0
    };
    println!(
        "  -> Indexed: {:.1}µs, No-index: {:.1}µs, Speedup: {:.1}x",
        eq_per, no_idx_per, speedup
    );
    db.close().ok();
}
...
fn bench_index_scaling() {
    ...
    exec(
        &db,
        "CREATE TABLE scale (id INT PRIMARY KEY, category TEXT, val INT)",
    );
    ...
    for &size in &sizes {
        ...
        for i in 1..=size as i64 {
            ...
            exec(
                &db,
                &format!("INSERT INTO scale VALUES ({}, '{}', {})", i, cat, i % 100),
            );
        }
        ...
        exec(
            &db,
            &format!(
                "CREATE INDEX idx_sc_{} ON scale (category) USING COLUMN",
                size
            ),
        );
...
fn bench_vector_index_build() {
    ...
    exec(
        &db,
        "CREATE TABLE vecs (id INT PRIMARY KEY, embedding VECTOR(8))",
    );
    ...
    print_result(
        &format!("CREATE VECTOR INDEX on {} vectors", n),
        1,
        build_ms,
    );
...
fn bench_text_index_build() {
    ...
    print_result(
        &format!("Text search 'database' top-10 × {}", q),
        q,
        search_ms,
    );
    ...
    println!(
        "  -> Index build: {}ms, Search: {:.1}µs/query",
        build_ms, search_per
    );
    db.close().ok();
}
...
fn bench_multiple_indexes() {
    ...
    exec(
        &db,
        "CREATE TABLE multi (id INT PRIMARY KEY, a TEXT, b FLOAT, c INT, d TEXT)",
    );
    ...
    println!(
        "  -> Total build: {}ms, Per-query avg: {:.1}µs",
...
fn bench_index_write_amplification() {
    ...
    exec(
        &db,
        "CREATE TABLE wa (id INT PRIMARY KEY, category TEXT, val INT)",
    );
    ...
    print_result(
        &format!("INSERT {} rows (with index)", n),
        n,
        with_idx_insert_ms,
    );
    ...
    let with_idx_update_ms = {
        ...
        for i in (1..=n as i64).step_by(3) {
            exec(
                &db,
                &format!("UPDATE wa SET category = 'updated' WHERE id = {}", i),
            );
        }
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("UPDATE {} rows (with index)", upd_count),
        upd_count,
        with_idx_update_ms,
    );
    ...
    let amplification = if no_idx_per > 0.0 {
        with_idx_per / no_idx_per
    } else {
        0.0
    };
    println!(
        "  -> No index: {:.1}µs/insert, With index: {:.1}µs/insert, Amplification: {:.2}x",
