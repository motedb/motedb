...
fn test_show_tables() {
    ...
    db.execute("CREATE TABLE alpha (id INT PRIMARY KEY)")
        .unwrap();
    db.execute("CREATE TABLE beta (id INT PRIMARY KEY)")
        .unwrap();
...
fn test_describe_table() {
    ...
    db.execute("CREATE TABLE users (id INT PRIMARY KEY, name TEXT, age INT, score FLOAT)")
        .unwrap();
...
fn test_alter_table_auto_increment() {
    ...
    db.execute("CREATE TABLE counters (id INT PRIMARY KEY AUTO_INCREMENT, val TEXT)")
        .unwrap();

    // Insert some rows to advance auto_increment
    db.execute("INSERT INTO counters (val) VALUES ('first')")
        .unwrap();
    db.execute("INSERT INTO counters (val) VALUES ('second')")
        .unwrap();

    // Reset auto_increment
    let result = db
        .execute("ALTER TABLE counters AUTO_INCREMENT = 100")
        .unwrap();
    let m = msg(result);
    assert!(
        m.contains("100"),
        "Should confirm AUTO_INCREMENT set to 100"
    );

    // Next insert should get id=100
    db.execute("INSERT INTO counters (val) VALUES ('third')")
        .unwrap();
    let result = db
        .execute("SELECT id FROM counters WHERE val = 'third'")
        .unwrap();
    let r = rows(result);
...
fn test_alter_table_no_auto_increment() {
    ...
    db.execute("CREATE TABLE plain (id INT PRIMARY KEY, val TEXT)")
        .unwrap();

    let result = db.execute("ALTER TABLE plain AUTO_INCREMENT = 10");
    assert!(
        result.is_err(),
        "ALTER AUTO_INCREMENT on non-AUTO_INCREMENT table should error"
    );
}
...
fn test_multi_row_insert() {
    ...
    db.execute("INSERT INTO items VALUES (1, 'apple'), (2, 'banana'), (3, 'cherry')")
        .unwrap();
...
fn test_where_in_subquery() {
    ...
    db.execute("CREATE TABLE orders (id INT PRIMARY KEY, customer_id INT, total FLOAT)")
        .unwrap();
    db.execute("INSERT INTO orders VALUES (1, 10, 100), (2, 20, 200), (3, 10, 150)")
        .unwrap();

    db.execute("CREATE TABLE vip_customers (id INT PRIMARY KEY)")
        .unwrap();
...
fn test_from_subquery() {
    ...
    db.execute("CREATE TABLE sales (id INT PRIMARY KEY, amount FLOAT)")
        .unwrap();
    db.execute("INSERT INTO sales VALUES (1, 100), (2, 200), (3, 300)")
        .unwrap();

    let result = db
        .execute("SELECT SUM(sub.amount) FROM (SELECT amount FROM sales WHERE amount > 100) AS sub")
        .unwrap();
    let r = rows(result);
    ...
    if let Value::Float(sum) = &r[0][0] {
        assert!(
            (sum - 500.0).abs() < 0.01,
            "SUM of amount > 100 should be 500, got {}",
            sum
        );
    }
}
...
fn test_scalar_subquery() {
    ...
    let result = db
        .execute("SELECT id, price FROM products WHERE price > (SELECT AVG(price) FROM products)")
        .unwrap();
    let r = rows(result);
...
fn test_data_type_aliases() {
    ...
    db.execute("CREATE TABLE t1 (id INT PRIMARY KEY, name VARCHAR)")
        .unwrap();
    db.execute("INSERT INTO t1 VALUES (1, 'hello')").unwrap();

    // STRING -> TEXT
    db.execute("CREATE TABLE t2 (id INT PRIMARY KEY, name STRING)")
        .unwrap();
    db.execute("INSERT INTO t2 VALUES (1, 'world')").unwrap();

    // REAL -> FLOAT
    db.execute("CREATE TABLE t3 (id INT PRIMARY KEY, val REAL)")
        .unwrap();
    db.execute("INSERT INTO t3 VALUES (1, 3.14)").unwrap();

    // DOUBLE -> FLOAT
    db.execute("CREATE TABLE t4 (id INT PRIMARY KEY, val DOUBLE)")
        .unwrap();
    db.execute("INSERT INTO t4 VALUES (1, 2.718)").unwrap();

    // BIGINT
    db.execute("CREATE TABLE t5 (id INT PRIMARY KEY, big BIGINT)")
        .unwrap();
...
fn test_boolean_type() {
    ...
    let result = db
        .execute("SELECT id FROM flags WHERE active = TRUE")
        .unwrap();
    let r = rows(result);
    assert_eq!(r.len(), 1);
...
fn test_latest_by() {
    ...
    db.execute("CREATE TABLE readings (id INT PRIMARY KEY, device TEXT, ts INT, value FLOAT)")
        .unwrap();

    // Insert multiple readings for same device
    db.execute("INSERT INTO readings VALUES (1, 'sensor_a', 1000, 10.0)")
        .unwrap();
    db.execute("INSERT INTO readings VALUES (2, 'sensor_a', 2000, 20.0)")
        .unwrap();
    db.execute("INSERT INTO readings VALUES (3, 'sensor_b', 1000, 30.0)")
        .unwrap();
    db.execute("INSERT INTO readings VALUES (4, 'sensor_b', 3000, 40.0)")
        .unwrap();

    let result = db.execute("SELECT device, value FROM readings LATEST BY device ORDER BY device");
    match result {
        Ok(r) => {
            let r = rows(r);
