...
fn stress_pk_lookup() {
    ...
    exec(
        &db,
        "CREATE TABLE t (id INTEGER PRIMARY KEY, val TEXT, score FLOAT, tag TEXT)",
    );
    ...
    print_result(
        &format!("PK SELECT {} queries (SSTable + Cache)", q),
        q,
        sst_ms,
    );
    ...
    print_result(
        &format!(
            "PK SELECT {} (repeated 100 queries × {}, stmt cache)",
            cached_total, cached_reps
        ),
        cached_total,
        cached_ms,
    );
...
fn stress_column_index() {
    ...
    print_result(
        &format!("Seed INSERT {} rows (5 categories)", n),
        n,
        seed_ms,
    );
    ...
    print_result(
        &format!("Column eq scan × {} (category='electronics')", queries),
        queries,
        eq_ms,
    );
    ...
    let range_ms = {
        ...
        for _ in 0..queries {
            exec(
                &db,
                "SELECT * FROM products WHERE price > 500.0 AND price < 600.0",
            );
        }
        start.elapsed().as_millis() as u64
    };
    print_result(
        &format!("Column range scan × {} (500 < price < 600)", queries),
        queries,
        range_ms,
    );
}
...
fn stress_full_scan() {
    ...
    exec(
        &db,
        "CREATE TABLE events (id INTEGER PRIMARY KEY, event_type TEXT, payload TEXT, ts INTEGER)",
    );
    ...
    print_result(
        "SELECT * with WHERE filter (SSTable, 1/20 match)",
        n,
        filter_ms,
    );
    ...
    print_result(
        &format!("COUNT(*) × {} (SSTable)", count_rounds),
        count_rounds,
        count_ms,
    );
}
...
fn stress_mixed_crud() {
    ...
    exec(
        &db,
        "CREATE TABLE orders (id INTEGER PRIMARY KEY, customer TEXT, amount FLOAT, status TEXT)",
    );
    ...
    let total_ms = {
        ...
        for i in (1..=n as i64).step_by(3) {
            exec(
                &db,
                &format!(
                    "UPDATE orders SET status = 'shipped', amount = amount + 10 WHERE id = {}",
                    i
                ),
            );
        }
    ...
    print_result(
        &format!(
            "Mixed CRUD ({} ops: {}ins + {}upd + {}del + {}sel)",
            total_ops,
            n,
            n / 3,
            n / 5,
...
fn stress_batch_insert() {
    ...
    print_result(
        &format!("INSERT {} rows (batch {} chunks)", n, n / batch),
        n,
        ms,
    );
}
...
fn stress_prepared_statement_cache() {
    ...
    exec(
        &db,
        "CREATE TABLE cache_test (id INTEGER PRIMARY KEY, data TEXT)",
    );
    ...
    for i in 1..=n as i64 {
        exec(
            &db,
            &format!("INSERT INTO cache_test VALUES ({}, 'data_{}')", i, i),
        );
    }
    ...
    print_result(
        "PK SELECT 1K queries (cold stmt cache -> warm)",
        1000,
        cold_ms,
    );
    ...
    print_result(
        "PK SELECT 10K (100 unique x 100 repeat, stmt cache hit)",
        10_000,
        hot_ms,
    );
    ...
    println!(
        "  -> Cold: {:.1} us/op, Hot: {:.1} us/op, Speedup: {:.1}x",
