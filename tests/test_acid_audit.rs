...
fn savepoint_rollback_undoes_post_savepoint_only() {
    ...
    assert_eq!(
        count(&db, "t"),
        1,
        "only pre-savepoint row should remain after commit"
    );
}
...
fn execute_prepared_respects_transaction() {
    ...
    db.rollback_transaction(tx).expect("rollback");
    assert_eq!(count(&db, "t"), 0, "rollback must undo prepared insert too");
}
...
fn committed_data_survives_reopen() {
    ...
    assert_eq!(
        count(&db, "t"),
        100,
        "all committed rows must survive reopen"
    );
}
...
fn rollback_then_commit_only_sees_committed() {
    ...
    if let QueryResult::Select { rows, .. } = rs {
        assert_eq!(rows.len(), 1);
        match &rows[0][0] {
            motedb::types::Value::Integer(n) => {
                assert_eq!(*n, 2, "should be the committed row id=2")
            }
            other => panic!("expected Integer, got {:?}", other),
        }
    }
...
fn ryw_insert_visible_in_pk_point_query() {
    let (db, _dir) = setup("CREATE TABLE t (id INT PRIMARY KEY, v INT)");
    let tx = db.begin_transaction().expect("begin");
    db.execute("INSERT INTO t VALUES (42, 99)")
        .expect("insert in txn");
...
fn ryw_count_reflects_uncommitted_writes() {
    ...
    assert_eq!(
        count(&db, "t"),
        3,
        "COUNT(*) must reflect uncommitted inserts"
    );
    db.rollback_transaction(tx).expect("rollback");
...
fn ryw_mixed_insert_delete_update() {
    ...
    db.execute("DELETE FROM t WHERE id = 1")
        .expect("delete in txn");
    db.execute("UPDATE t SET v = 222 WHERE id = 2")
        .expect("update in txn");
    ...
    let rs = db
        .execute("SELECT v FROM t WHERE id = 2")
        .expect("select")
        .materialize()
        .expect("mat");
    use motedb::sql::QueryResult;
    ...
    assert_eq!(
        count(&db, "t"),
        2,
        "after rollback original 2 rows restored"
    );
}
...
fn ryw_join_sees_uncommitted_writes() {
    ...
    db.execute("INSERT INTO a VALUES (2, 20)")
        .expect("insert a");
    db.execute("INSERT INTO b VALUES (2, 2, 'second')")
        .expect("insert b");
    ...
    if let QueryResult::Select { rows, .. } = rs {
        assert_eq!(
            rows.len(),
            2,
            "JOIN must see both committed + uncommitted rows"
        );
    } else {
...
fn ryw_multi_table_counts_independent() {
    ...
    let (db, _dir) = setup_two_tables();
    let tx = db.begin_transaction().expect("begin");
    db.execute("INSERT INTO a VALUES (5, 50)")
        .expect("insert a");
    db.execute("INSERT INTO b VALUES (5, 5, 'fifth')")
        .expect("insert b");
...
fn setup_two_tables() -> (Database, TempDir) {
    ...
    db.execute("CREATE TABLE a (id INT PRIMARY KEY, v INT)")
        .expect("create a");
    db.execute("CREATE TABLE b (id INT PRIMARY KEY, a_id INT, name TEXT)")
        .expect("create b");
    db.execute("INSERT INTO a VALUES (1, 10)").expect("seed a");
    db.execute("INSERT INTO b VALUES (1, 1, 'first')")
        .expect("seed b");
    (db, dir)
}
