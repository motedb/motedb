...
fn test_column_index_returns_results() {
    ...
    db.execute("INSERT INTO users (id, age) VALUES (1, 25)")
        .unwrap();
    db.execute("INSERT INTO users (id, age) VALUES (2, 30)")
        .unwrap();
    db.execute("INSERT INTO users (id, age) VALUES (3, 25)")
        .unwrap();
    ...
    assert!(
        r.len() >= 2,
        "Column index should find at least 2 rows with age=25, got {}",
        r.len()
    );
}
...
fn test_year_function_in_where() {
    ...
    db.execute("INSERT INTO events (id, ts) VALUES (1, 1700000000000000)")
        .unwrap();
    db.execute("INSERT INTO events (id, ts) VALUES (2, 1600000000000000)")
        .unwrap();
...
fn test_vector_distance_in_where_positional() {
    let (db, _dir) = create_db();
    db.execute("CREATE TABLE items (id INT PRIMARY KEY, emb VECTOR(3))")
        .unwrap();
    db.execute("INSERT INTO items (id, emb) VALUES (1, [1.0, 0.0, 0.0])")
        .unwrap();
    db.execute("INSERT INTO items (id, emb) VALUES (2, [0.0, 1.0, 0.0])")
        .unwrap();
    db.execute("INSERT INTO items (id, emb) VALUES (3, [0.0, 0.0, 1.0])")
        .unwrap();

    // L2 distance filter in WHERE — previously returned 0 rows silently
    let r = rows(
        &db,
        "SELECT id FROM items WHERE (emb <-> [1.0, 0.0, 0.0]) < 0.5",
    );
...
fn test_length_unicode() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, name TEXT)")
        .unwrap();
    db.execute("INSERT INTO t (id, name) VALUES (1, '你好世界')")
        .unwrap();
    ...
    if let Value::Integer(len) = r[0][0] {
        assert_eq!(
            len, 4,
            "LENGTH should count characters, not bytes (你好世界 = 4 chars)"
        );
    }
}
...
fn test_concat_null_propagates() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, a TEXT, b TEXT)")
        .unwrap();
    db.execute("INSERT INTO t (id, a, b) VALUES (1, 'hello', 'world')")
        .unwrap();
    db.execute("INSERT INTO t (id, a) VALUES (2, 'hello')")
        .unwrap();
...
fn test_constraints_any_order() {
    ...
    db.execute("CREATE TABLE t (id INT PRIMARY KEY NOT NULL, val INT)")
        .unwrap();
...
fn test_explicit_null_column() {
    let (db, _dir) = create_db();
    db.execute("CREATE TABLE t (id INT, name TEXT NULL)")
        .unwrap();
...
fn test_pk_equality_non_id_column() {
    let (db, _dir) = create_db();
    db.execute("CREATE TABLE items (sku INT PRIMARY KEY, name TEXT)")
        .unwrap();
    db.execute("INSERT INTO items (sku, name) VALUES (100, 'a')")
        .unwrap();
    db.execute("INSERT INTO items (sku, name) VALUES (200, 'b')")
        .unwrap();
    ...
    assert_eq!(
        r.len(),
        1,
        "PK lookup on 'sku' should find 1 row, got {}",
        r.len()
    );
