...
fn test_update_in_subquery() {
    ...
    db.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
    db.execute("UPDATE t SET v = v * 2 WHERE id IN (SELECT id FROM t WHERE v > 15)")
        .unwrap();
...
fn test_update_in_subquery_different_table() {
    ...
    db.execute("UPDATE a SET v = 999 WHERE id IN (SELECT a_id FROM b)")
        .unwrap();
...
fn test_update_in_subquery_empty_result() {
    ...
    db.execute("UPDATE t SET v = 999 WHERE id IN (SELECT id FROM t WHERE v > 100)")
        .unwrap();
    let r = q(&db, "SELECT id, v FROM t ORDER BY id");
    assert_eq!(
        r,
...
fn test_update_scalar_subquery_where() {
    ...
    db.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
    db.execute("UPDATE t SET v = 0 WHERE v > (SELECT AVG(v) FROM t)")
        .unwrap();
    let r = q(&db, "SELECT id, v FROM t ORDER BY id");
...
fn test_update_literal_in() {
    ...
    db.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
    db.execute("UPDATE t SET v = v * 2 WHERE id IN (2, 3)")
        .unwrap();
    let r = q(&db, "SELECT id, v FROM t ORDER BY id");
    assert_eq!(
        r,
...
fn test_update_set_subquery_pk() {
    ...
    db.execute("INSERT INTO t VALUES (1, 10), (2, 20), (3, 30)")
        .unwrap();
    db.execute("UPDATE t SET v = (SELECT MAX(v) FROM t) WHERE id = 2")
        .unwrap();
    let r = q(&db, "SELECT id, v FROM t ORDER BY id");
    assert_eq!(
        r,
...
fn test_update_set_subquery_scan() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER, g INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 10, 1), (2, 20, 1)")
        .unwrap();
    db.execute("UPDATE t SET v = (SELECT MAX(v) FROM t) WHERE g = 1")
        .unwrap();
    let r = q(&db, "SELECT id, v FROM t ORDER BY id");
    assert_eq!(
        r,
