...
fn test_groupby_agg_first_then_key() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
...
fn test_groupby_all_aggs() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'a', 10), (2, 'a', 20), (3, 'b', 5), (4, 'b', 7)")
        .unwrap();
    let r = q(
        &db,
        "SELECT g, COUNT(*), SUM(v), MIN(v), MAX(v), AVG(v) FROM t GROUP BY g",
    );
    assert_rows_eq(
        r,
        vec![
...
fn test_groupby_sum_all_null_is_null() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'a', 10), (2, 'c', NULL)")
        .unwrap();
...
fn test_groupby_avg_ignores_null() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'a', 10), (2, 'a', NULL), (3, 'a', 30)")
        .unwrap();
...
fn test_groupby_avg_all_null_is_null() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'c', NULL), (2, 'c', NULL)")
        .unwrap();
    let r = q(&db, "SELECT g, AVG(v) FROM t GROUP BY g");
...
fn test_groupby_null_group_row() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1, 'a', 10), (2, NULL, 100), (3, NULL, 200)")
        .unwrap();
...
fn test_having_no_groupby_false_condition() {
    ...
    assert!(
        r.is_empty(),
        "HAVING COUNT(*) > 5 with count=3 should return 0 rows, got {:?}",
        r
    );
}
...
fn test_having_no_groupby_with_other_aggregate() {
    ...
    assert!(
        r.is_empty(),
        "HAVING SUM(v) > 100 with sum=60 should return 0 rows"
    );
}
