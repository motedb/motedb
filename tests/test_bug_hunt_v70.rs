...
fn test_not_in_literal_list_with_null_value() {
    ...
    match res {
        Ok(r) => {
            ...
            assert!(got.is_empty(), "NOT IN (.., NULL) should yield no rows, got {:?}", got);
        }
...
fn test_order_limit_with_nulls() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,30),(2,NULL),(3,10),(4,NULL),(5,20)").unwrap();
    // ORDER BY v LIMIT 3 → NULLs first (ASC): NULL, NULL, 10 → ids 2, 4, 3
    let r = q(&db, "SELECT id FROM t ORDER BY v LIMIT 3");
    let ids: Vec<i64> = r.iter().map(|row| match &row[0] { Value::Integer(i) => *i, _ => -1 }).collect();
    assert_eq!(ids.len(), 3);
    // First two should be the NULL rows (ids 2, 4 in some order), third is id 3 (v=10).
    assert!(ids.contains(&3), "third should be id 3 (v=10), got {:?}", ids);
}
...
fn test_float_between() {
    ...
    let r = q(&db, "SELECT id FROM t WHERE v BETWEEN 2.0 AND 3.0 ORDER BY id");
    assert_eq!(r, vec![vec![Value::Integer(2)]]);
}
...
fn test_float_not_between() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v FLOAT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1.5),(2,2.5),(3,3.5)").unwrap();
    let r = q(&db, "SELECT id FROM t WHERE v NOT BETWEEN 2.0 AND 3.0 ORDER BY id");
    assert_eq!(r, vec![vec![Value::Integer(1)], vec![Value::Integer(3)]]);
}
...
fn test_sum_large_no_overflow() {
    ...
    db.execute("INSERT INTO t VALUES (1,1000000000),(2,2000000000),(3,3000000000)").unwrap();
...
fn test_sum_promotes_to_float_on_overflow() {
    ...
    let max = i64::MAX;
    db.execute(&format!("INSERT INTO t VALUES (1,{}),(2,{})", max, max)).unwrap();
    let r = q(&db, "SELECT SUM(v) FROM t");
...
fn test_count_with_groupby_and_having_expression() {
    ...
    db.execute("INSERT INTO t VALUES (1,10),(1,20),(2,5),(2,15)").unwrap();
    let r = q(
        &db,
...
fn test_column_equals_column() {
    ...
    db.execute("INSERT INTO t VALUES (1,5,5),(2,5,10),(3,10,10)").unwrap();
    // a = b → ids 1, 3
...
fn test_column_greater_than_column() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10,5),(2,5,10),(3,7,7)").unwrap();
    // a > b → id 1
...
fn test_min_max_negative() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,-5),(2,-10),(3,-3)").unwrap();
...
fn test_groupby_count_zero_via_where() {
    ...
    let r = q(&db, "SELECT g, COUNT(*) FROM t WHERE v < 30 GROUP BY g ORDER BY g");
