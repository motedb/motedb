...
fn test_unary_minus_precedence() {
    ...
    assert_eq!(r, vec![vec![Value::Integer(6), Value::Integer(1), Value::Integer(15)]]);
}
...
fn test_timestamp_insert_select_roundtrip() {
    ...
    db.execute("INSERT INTO t VALUES (1, '2024-06-15T12:30:45')").unwrap();
...
fn test_timestamp_extract_components() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)").unwrap();
    db.execute("INSERT INTO t VALUES (1, '2024-06-15T12:30:45')").unwrap();
    let r = q(&db, "SELECT YEAR(ts), MONTH(ts), DAY(ts), HOUR(ts), MINUTE(ts), SECOND(ts) FROM t WHERE id = 1");
    assert_eq!(
        r,
        vec![vec![
            Value::Integer(2024),
...
fn test_timestamp_order_chronological() {
    ...
    assert_eq!(r, vec![vec![Value::Integer(2)], vec![Value::Integer(1)], vec![Value::Integer(3)]]);
}
...
fn test_timestamp_comparison_filter() {
    ...
    let r = q(&db, "SELECT id FROM t WHERE ts > '2024-01-01T00:00:00' ORDER BY id");
...
fn test_lag_with_explicit_offset() {
    ...
    let r = q(&db, "SELECT id, LAG(v, 2) OVER (ORDER BY id) AS prev2 FROM t ORDER BY id");
    assert_eq!(
        r,
        vec![
...
fn test_row_number_simple() {
    ...
    db.execute("INSERT INTO s VALUES (1,30),(2,10),(3,20)").unwrap();
    let r = q(&db, "SELECT id, ROW_NUMBER() OVER (ORDER BY score DESC) AS rn FROM s ORDER BY rn");
...
fn test_sum_of_expression() {
    ...
    db.execute("INSERT INTO t VALUES (1,10,2),(2,5,3),(3,20,1)").unwrap();
...
fn test_count_distinct_expression() {
    ...
    db.execute("INSERT INTO t VALUES (1,1,1),(2,1,1),(3,2,2)").unwrap();
...
fn test_where_upper_equality() {
    ...
    db.execute("INSERT INTO t VALUES (1,'Alice'),(2,'ALICE'),(3,'Bob')").unwrap();
...
fn test_where_length_filter() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'hi'),(2,'hello'),(3,'x')").unwrap();
...
fn test_where_substr_filter() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,'alice@x.com'),(2,'bob'),(3,'carol@y.com')").unwrap();
    let r = q(&db, "SELECT id FROM t WHERE SUBSTR(s, 1, 5) = 'alice' ORDER BY id");
