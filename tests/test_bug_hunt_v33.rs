...
fn test_groupby_join_functional_dependency() {
    ...
    db.execute("INSERT INTO a VALUES (1, 'alice'), (2, 'bob')")
        .unwrap();
    db.execute("CREATE TABLE b (id INTEGER PRIMARY KEY, a_id INTEGER, val INTEGER)")
        .unwrap();
    db.execute("INSERT INTO b VALUES (1, 1, 100), (2, 1, 200), (3, 2, 300)")
        .unwrap();
    // Note: ORDER BY a.id may not be respected in the JOIN+GROUP BY path,
    // so compare as a set.
    let r = q(
        &db,
        "SELECT a.name, COUNT(*) FROM a LEFT JOIN b ON a.id = b.a_id GROUP BY a.id",
    );
...
fn test_groupby_join_sum() {
    ...
    let r = q(
        &db,
        "SELECT a.id, SUM(b.val) FROM a LEFT JOIN b ON a.id = b.a_id GROUP BY a.id ORDER BY a.id",
    );
    assert_eq!(
        r,
        vec![
...
fn test_create_table_default_integer() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER DEFAULT 42)")
        .unwrap();
...
fn test_create_table_default_text() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT DEFAULT 'unknown')")
        .unwrap();
    db.execute("INSERT INTO t (id) VALUES (1)").unwrap();
...
fn test_create_table_default_float() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, score FLOAT DEFAULT 1.5)")
        .unwrap();
    db.execute("INSERT INTO t (id) VALUES (1)").unwrap();
...
fn test_create_table_default_not_null() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, status TEXT NOT NULL DEFAULT 'pending')")
        .unwrap();
    db.execute("INSERT INTO t (id) VALUES (1)").unwrap();
...
fn test_insert_partial_uses_default() {
    let (db, _d) = db();
    db.execute(
        "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER DEFAULT 10, name TEXT DEFAULT 'n')",
    )
    .unwrap();
    db.execute("INSERT INTO t (id) VALUES (1)").unwrap();
...
fn test_insert_multi_row_partial_default() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER DEFAULT 5)")
        .unwrap();
