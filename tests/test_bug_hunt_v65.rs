...
fn test_sum_int_returns_int_large() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,1000000),(2,2000000),(3,3000000)").unwrap();
...
fn test_correlated_exists_like() {
    ...
    let (db, _d) = db();
    db.execute("CREATE TABLE customers(id INT PRIMARY KEY, name TEXT)").unwrap();
    db.execute("CREATE TABLE orders(id INT PRIMARY KEY, cust INT)").unwrap();
    db.execute("INSERT INTO customers VALUES (1,'Alice'),(2,'Bob'),(3,'Carol')").unwrap();
    db.execute("INSERT INTO orders VALUES (1,1),(2,1),(3,3)").unwrap();
...
fn test_boolean_column_lifecycle() {
    ...
    db.execute("INSERT INTO t VALUES (1,TRUE),(2,FALSE),(3,NULL)").unwrap();
    let r = q(&db, "SELECT active FROM t ORDER BY id");
    assert_eq!(
        r,
        vec![vec![Value::Bool(true)], vec![Value::Bool(false)], vec![Value::Null]]
    );
    // WHERE active = TRUE
...
fn test_boolean_update() {
    ...
    db.execute("UPDATE t SET active = FALSE WHERE id = 1").unwrap();
