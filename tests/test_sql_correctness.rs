...
fn test_select_single_row_exact_values() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, name TEXT, score FLOAT, active BOOLEAN)").unwrap();
    db.execute("INSERT INTO t VALUES (1, 'Alice', 95.5, TRUE)").unwrap();

    let r = row(&db, "SELECT id, name, score, active FROM t");
    assert_eq!(r[0], Value::Integer(1));
    assert_eq!(r[1], Value::Text("Alice".into()));
    // Float comparison with epsilon
    match &r[2] { Value::Float(f) => assert!((f - 95.5).abs() < 0.001), v => panic!("{:?}", v) }
...
fn test_select_column_order_matches_create_table() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, z TEXT, a INT, m FLOAT)").unwrap();
    db.execute("INSERT INTO t VALUES (1, 'hello', 42, 3.14)").unwrap();
    ...
    match &r[3] { Value::Float(f) => assert!((f - 3.14).abs() < 0.001), v => panic!("{:?}", v) }
}
...
fn test_select_specific_columns_reorder() {
    ...
    db.execute("INSERT INTO t VALUES (1, 10, 'x', 2.5)").unwrap();

    let r = row(&db, "SELECT c, a, id FROM t");
    assert_eq!(r.len(), 3);
    match &r[0] { Value::Float(f) => assert!((f - 2.5).abs() < 0.001), v => panic!("{:?}", v) }
    ...
    for r in &rs {
        ...
        if let Value::Integer(id) = r[0] {
            if let Value::Integer(v) = r[1] {
                ...
                found.insert(id);
            } else { panic!("v not Integer"); }
        } else { panic!("id not Integer"); }
    }
...
fn test_select_expressions_exact() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, price FLOAT, qty INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1, 9.99, 3)").unwrap();

    let r = row(&db, "SELECT price * qty, price + 0.01, qty - 1, qty / 2 FROM t");
    match &r[0] { Value::Float(f) => assert!((f - 29.97).abs() < 0.01), v => panic!("{:?}", v) }
    match &r[1] { Value::Float(f) => assert!((f - 10.0).abs() < 0.01), v => panic!("{:?}", v) }
...
fn test_where_or_independent() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE t (id INT PRIMARY KEY, category TEXT, v INT)").unwrap();
    ...
    let rs = rows(&db, "SELECT id FROM t WHERE category = 'A' OR v = 30 ORDER BY id");
...
fn test_where_complex_boolean() {
    ...
    let rs = rows(&db, "SELECT id FROM t WHERE (a > 5 AND b < 20) OR c = 100 ORDER BY id");
    assert_eq!(rs.len(), 3);
    assert_eq!(rs[0][0], Value::Integer(1));
...
fn test_where_like_patterns() {
    ...
    // Contains 'lic'
    let rs = rows(&db, "SELECT name FROM t WHERE name LIKE '%lic%' ORDER BY id");
...
fn test_order_by_with_nulls() {
    ...
    let non_null: Vec<i64> = rs.iter()
        .filter(|r| !matches!(r[1], Value::Null))
        .map(|r| match r[1] { Value::Integer(v) => v, _ => panic!() })
        .collect();
    assert_eq!(non_null, vec![10, 20, 30], "Non-null values must be in ASC order");
}
...
fn test_avg_exact() {
    ...
    for v in [10, 20, 30, 40, 50] {
        db.execute(&format!("INSERT INTO t VALUES ({}, {})", v, v)).unwrap();
    }
...
fn test_min_max_exact() {
    ...
    for v in &values {
        db.execute(&format!("INSERT INTO t VALUES ({}, {})", v.abs() + v.signum() * 100, v)).unwrap();
    }
...
fn test_group_by_exact_results() {
    ...
    let rs = rows(&db, "SELECT cat, SUM(v), COUNT(*), AVG(v), MIN(v), MAX(v) FROM t GROUP BY cat ORDER BY cat");
    assert_eq!(rs.len(), 3);
    ...
    match &rs[0][3] { Value::Float(f) => assert!((f - 20.0).abs() < 0.001), v => panic!("{:?}", v) } // AVG
...
fn test_update_single_row_exact() {
    ...
    db.execute("UPDATE t SET name = 'Alicia', v = 15 WHERE id = 1").unwrap();
...
fn test_update_multiple_rows() {
    ...
    for i in 0..10 {
        ...
        db.execute(&format!("INSERT INTO t VALUES ({}, '{}', {})", i, cat, i * 10)).unwrap();
    }

    db.execute("UPDATE t SET v = v + 100 WHERE cat = 'A'").unwrap();
...
fn test_delete_specific_rows_remaining_exact() {
    ...
    db.execute("DELETE FROM t WHERE v >= 10 AND v < 15").unwrap();
...
fn test_inner_join_exact_rows() {
    let (db, _dir) = setup();
    db.execute("CREATE TABLE orders (id INT PRIMARY KEY, customer_id INT, amount FLOAT)").unwrap();
    ...
    db.execute("INSERT INTO orders VALUES (101, 1, 50.0)").unwrap();
    db.execute("INSERT INTO orders VALUES (102, 1, 30.0)").unwrap();
    db.execute("INSERT INTO orders VALUES (103, 2, 80.0)").unwrap();
    ...
    match &rs[0][2] { Value::Float(f) => assert!((f - 50.0).abs() < 0.01), v => panic!("{:?}", v) }
...
fn test_left_join_null_fill() {
    ...
    db.execute("CREATE TABLE b (id INT PRIMARY KEY, a_id INT, val TEXT)").unwrap();
    ...
    assert_eq!(rs[1][2], Value::Null, "LEFT JOIN unmatched should fill with NULL");
}
...
fn test_column_index_consistency() {
    ...
    assert_eq!(no_idx.len(), with_idx.len(), "Index should not change result count");
...
fn test_range_query_index_consistency() {
    ...
    let no_idx = rows(&db, "SELECT id FROM t WHERE v >= 200 AND v < 350 ORDER BY id");

    db.execute("CREATE INDEX idx_v ON t (v)").unwrap();
    db.wait_for_indexes_ready();

    let with_idx = rows(&db, "SELECT id FROM t WHERE v >= 200 AND v < 350 ORDER BY id");
...
fn test_flush_preserves_exact_data() {
    ...
    for i in 0..30 {
        db.execute(&format!("INSERT INTO t VALUES ({}, 'user_{}', {:.1})", i, i, i as f64 * 1.5)).unwrap();
    }
    ...
    assert_eq!(before.len(), after.len(), "Row count must be identical after flush");
...
fn test_restart_preserves_exact_data() {
    ...
    // Phase 1: write data
    {
        ...
        for i in 0..50 {
            db.execute(&format!("INSERT INTO t VALUES ({}, {}, 'name_{}')", i, i * 7, i)).unwrap();
        }
        db.flush().unwrap();
        db.close().unwrap();
    }
...
fn test_restart_preserves_deletes_and_updates() {
...

    {
        ...
        for r in &rs[0..5] {
            assert_eq!(r[1], Value::Integer(-1), "id={:?} should have v=-1 but got {:?}", r[0], r[1]);
        }
        // Rows 5-14 unchanged
        for r in &rs[5..15] {
            let id = match r[0] { Value::Integer(i) => i, _ => panic!() };
...
fn test_string_functions_exact() {
    ...
    assert_eq!(text_val(&db, "SELECT LOWER(s) FROM t WHERE id = 1"), "hello world");
    assert_eq!(text_val(&db, "SELECT UPPER(s) FROM t WHERE id = 1"), "HELLO WORLD");
    assert_eq!(int_val(&db, "SELECT LENGTH(s) FROM t WHERE id = 1"), 11);
    assert_eq!(text_val(&db, "SELECT TRIM('  hi  ')"), "hi");
    assert_eq!(text_val(&db, "SELECT CONCAT(s, '!') FROM t WHERE id = 1"), "Hello World!");
}
...
fn test_empty_string_vs_null() {
    ...
    assert_eq!(r1[0], Value::Text("".into()), "Empty string should be Text, not Null");
    ...
    // LENGTH of NULL = NULL
    assert_eq!(val(&db, "SELECT LENGTH(s) FROM t WHERE id = 2"), Value::Null);
}
...
fn test_large_integer_values() {
    ...
    db.execute("INSERT INTO t VALUES (2, -9223372036854775808)").unwrap(); // i64::MIN
...
fn test_unicode_text_exact() {
    ...
    db.execute("INSERT INTO t VALUES (3, 'Héllo Wörld')").unwrap();
...
fn test_multiple_tables_isolation() {
    ...
    for i in 0..10 {
        ...
        db.execute(&format!("INSERT INTO t2 VALUES ({}, {})", i, i * 100)).unwrap();
    }
    ...
    assert_eq!(int_val(&db, "SELECT COUNT(*) FROM t2"), 10, "t2 should be unaffected");
}
...
fn test_float_precision_roundtrip() {
    ...
    for (i, expected) in test_values.iter().enumerate() {
        ...
        assert!((got - expected).abs() < expected.abs() * 1e-6 + 1e-12,
            "Float precision lost for value {}: got {}, expected {}", expected, got, expected);
    }
}
...
fn test_where_with_subquery() {
    ...
    let rs = rows(&db, "SELECT id, amount FROM orders WHERE amount > (SELECT AVG(amount) FROM orders) ORDER BY id");
    assert_eq!(rs.len(), 2);
...
fn test_concurrent_reads_see_consistent_data() {
    ...
    let handle = thread::spawn(move || {
        ...
        for _ in 0..20 {
            let count = match db_clone.execute("SELECT COUNT(*) FROM t").unwrap().materialize().unwrap() {
    ...
    db2.execute("CREATE TABLE t2 (id INT PRIMARY KEY, v INT)").unwrap();
    for i in 0..50 {
        db2.execute(&format!("INSERT INTO t2 VALUES ({}, {})", i, i)).unwrap();
    }
