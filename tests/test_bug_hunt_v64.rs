...
fn test_like_combined_wildcards() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'abc'),(2,'axc'),(3,'abbc'),(4,'ac')")
        .unwrap();
...
fn test_like_percent_underscore_combo() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'hello'),(2,'help'),(3,'heap'),(4,'hi')")
        .unwrap();
...
fn test_like_prefix_and_suffix() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'test.txt'),(2,'test.sql'),(3,'other.txt')")
        .unwrap();
...
fn test_not_like() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'apple'),(2,'banana'),(3,'apricot')")
        .unwrap();
...
fn test_having_case() {
    ...
    db.execute("INSERT INTO t VALUES (1,10),(1,20),(2,5),(2,5)")
        .unwrap();
    let r = q(
        &db,
...
fn test_subquery_in_arithmetic() {
    ...
    let r = q(
        &db,
        "SELECT id, v + (SELECT MAX(v) FROM t) FROM t ORDER BY id",
    );
    assert_eq!(
        r,
        vec![
...
fn test_in_subquery_no_match() {
    ...
    let r = q(
        &db,
        "SELECT id FROM main WHERE id IN (SELECT v FROM s) ORDER BY id",
    );
...
fn test_not_in_subquery() {
    ...
    let r = q(
        &db,
        "SELECT id FROM main WHERE id NOT IN (SELECT v FROM s) ORDER BY id",
    );
...
fn test_timestamp_string_comparison_direct() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, ts TIMESTAMP)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,'2024-01-01T00:00:00')")
        .unwrap();
...
fn test_multiple_distinct_aggs() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, a INT, b INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,1,10),(2,1,20),(3,2,10),(4,2,20)")
        .unwrap();
...
fn test_update_multi_col_expression() {
    ...
    // c = a + b * 2 → 2 + 6 = 8
    db.execute("UPDATE t SET c = a + b * 2 WHERE id = 1")
        .unwrap();
...
fn test_where_abs_function() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,-5),(2,3),(3,-10)")
        .unwrap();
...
fn test_where_mod_function() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,3),(2,4),(3,5),(4,6)")
        .unwrap();
...
fn test_where_then_groupby() {
    let (db, _d) = db();
    db.execute("CREATE TABLE t(g INT, v INT)").unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(1,20),(1,30),(2,5),(2,15)")
        .unwrap();
...
fn test_where_groupby_having_chain() {
    let (db, _d) = db();
    db.execute("CREATE TABLE sales(region TEXT, amt INT)")
        .unwrap();
    db.execute("INSERT INTO sales VALUES ('US',100),('US',50),('EU',200),('EU',30)")
        .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            ...
            vec![Value::text("US".into()), Value::Integer(150)]
        ]
    );
}
