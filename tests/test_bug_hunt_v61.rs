...
fn test_persist_after_update_delete() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            vec![Value::Integer(2), Value::Integer(999)]
        ]
    );
}
...
fn test_self_join_aliased_columns() {
    let dir = TempDir::new().unwrap();
    let db = Database::create(dir.path()).unwrap();
    db.execute("CREATE TABLE emp(id INT PRIMARY KEY, name TEXT, mgr INT)")
        .unwrap();
    db.execute("INSERT INTO emp VALUES (1,'CEO',NULL),(2,'Alice',1),(3,'Bob',1)")
        .unwrap();
...
fn test_join_select_star_collision() {
    ...
    let (db, _d) = db_inner();
    db.execute("CREATE TABLE a(id INT PRIMARY KEY, av INT)")
        .unwrap();
    db.execute("CREATE TABLE b(id INT PRIMARY KEY, bv INT)")
        .unwrap();
...
fn test_avg_returns_float() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, v INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10),(2,20),(3,25)")
        .unwrap();
...
fn test_round_float_decimals() {
    ...
    let r = q(
        &db,
        "SELECT ROUND(2.567, 2), ROUND(2.567, 1), ROUND(2.567, 0)",
    );
    assert_eq!(
        r,
        vec![vec![
...
fn test_update_column_reference_arithmetic() {
    let (db, _d) = db_inner();
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, price INT, qty INT, total INT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,10,3,0)").unwrap();
    db.execute("UPDATE t SET total = price * qty WHERE id = 1")
        .unwrap();
...
fn test_update_string_concat() {
    ...
    db.execute("UPDATE t SET s = s || ' world' WHERE id = 1")
        .unwrap();
...
fn test_groupby_full_combo() {
    let (db, _d) = db_inner();
    db.execute("CREATE TABLE sales(region TEXT, product TEXT, amt INT)")
        .unwrap();
...
fn test_null_vs_empty_in_where() {
    ...
    db.execute("CREATE TABLE t(id INT PRIMARY KEY, s TEXT)")
        .unwrap();
    db.execute("INSERT INTO t VALUES (1,''),(2,NULL),(3,'x')")
        .unwrap();
