...
fn bench_vector_search() {
    ...
    db.execute("CREATE TABLE vecs (id INT PRIMARY KEY AUTO_INCREMENT, cat TEXT, emb VECTOR(128))").unwrap();
    ...
    println!("  INSERT {} vectors ({}-dim): {} ms ({:.0} vecs/s)",
        n, dim, mote_insert_ms, n as f64 / mote_insert_ms as f64 * 1000.0);
    ...
    // Warm up
    let qstr = query.iter().map(|f| format!("{}", f)).collect::<Vec<_>>().join(",");
    ...
    println!("  KNN search (k={}): P50={}μs P99={}μs avg={}μs", k, mote_p50, mote_p99, mote_avg);
    ...
    println!("  暴力搜索 (k={}): P50={}μs P99={}μs avg={}μs", k, bf_p50, bf_p99, bf_avg);
...
fn generate_docs(n: usize) -> Vec<(String, String)> {
    let words = ["database", "machine", "learning", "robot", "sensor", "edge", "compute",
                 "ai", "embedded", "realtime", "spatial", "vector", "search", "index",
                 "query", "performance", "memory", "storage", "crash", "recovery"];
    (0..n).map(|i| {
        let title = format!("doc_{}_{}", i, words[i % words.len()]);
        let body = (0..20).map(|j| words[(i + j) % words.len()]).collect::<Vec<_>>().join(" ");
        (title, body)
    }).collect()
}
...
fn bench_text_search() {
    ...
    config.max_result_rows = None;
    let db = Database::create_with_config(dir.path(), config).unwrap();
    db.execute("CREATE TABLE docs (id INT PRIMARY KEY AUTO_INCREMENT, title TEXT, body TEXT, cat TEXT)").unwrap();
    ...
    let mote_insert_ms = t.elapsed().as_millis();
    println!("  MoteDB INSERT {} docs: {} ms ({:.0} docs/s)",
        n, mote_insert_ms, n as f64 / mote_insert_ms as f64 * 1000.0);
    ...
    // Warm up
    let _ = db.execute(&format!("SELECT id FROM docs WHERE MATCH(body, '{}') LIMIT 10", query));
    ...
    println!("  MoteDB FTS top-10: P50={}μs P99={}μs avg={}μs", mote_p50, mote_p99, mote_avg);

    // ── SQLite FTS5 ──
    let sqlite_dir = TempDir::new().unwrap();
    let conn = Connection::open(sqlite_dir.path().join("test.db")).unwrap();
    conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL; PRAGMA cache_size=-32000;").unwrap();
    conn.execute("CREATE TABLE docs (id INTEGER PRIMARY KEY, title TEXT, body TEXT, cat TEXT)", []).unwrap();
    conn.execute_batch("CREATE VIRTUAL TABLE docs_fts USING fts5(body, content='docs', content_rowid='id');").unwrap();

    // INSERT
    let t = Instant::now();
    conn.execute_batch("BEGIN TRANSACTION").unwrap();
    {
        let mut stmt = conn.prepare("INSERT INTO docs (title, body, cat) VALUES (?, ?, ?)").unwrap();
    ...
    // Build FTS index
    conn.execute_batch("INSERT INTO docs_fts(docs_fts) VALUES('rebuild');").unwrap();
    let sqlite_insert_ms = t.elapsed().as_millis();
    println!("  SQLite  INSERT {} docs + FTS5: {} ms ({:.0} docs/s)",
        n, sqlite_insert_ms, n as f64 / sqlite_insert_ms as f64 * 1000.0);

    // Warm up
    let _ = conn.execute("SELECT id FROM docs_fts WHERE docs_fts MATCH ? LIMIT 10", params![query]);
    ...
    println!("  SQLite  FTS5 top-10: P50={}μs P99={}μs avg={}μs", sql_p50, sql_p99, sql_avg);

    if mote_avg > 0 && sql_avg > 0 {
        println!("  搜索加速比: {:.1}x ({})", mote_avg as f64 / sql_avg as f64,
            if mote_avg < sql_avg { "MoteDB 更快" } else { "SQLite 更快" });
    }
    ...
    println!("  LIKE prefix P50: MoteDB={}μs SQLite={}μs", mote_like_p50, sql_like_p50);
...
fn bench_spatial_search() {
    ...
    for i in 0..n {
        ...
        sql.push_str(&format!("(POINT({:.1},{:.1}), {:.1}, '{}')", x, y, i as f64, if i%3==0{"US"}else{"EU"}));
    }
    db.execute(&sql).unwrap();
    let mote_insert_ms = t.elapsed().as_millis();
    println!("  MoteDB INSERT {} points: {} ms ({:.0} pts/s)",
        n, mote_insert_ms, n as f64 / mote_insert_ms as f64 * 1000.0);

    // CREATE SPATIAL INDEX
    let t = Instant::now();
    db.execute("CREATE SPATIAL INDEX idx_loc ON points (loc)").unwrap();
    let mote_index_ms = t.elapsed().as_millis();
    ...
    println!("  MoteDB spatial KNN top-10: P50={}μs P99={}μs avg={}μs", mote_p50, mote_p99, mote_avg);

    // ── SQLite (brute force — no RTree extension available) ──
    let sqlite_dir = TempDir::new().unwrap();
    let conn = Connection::open(sqlite_dir.path().join("test.db")).unwrap();
    conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL; PRAGMA cache_size=-32000;").unwrap();
    conn.execute("CREATE TABLE points (id INTEGER PRIMARY KEY, x REAL, y REAL, val REAL, region TEXT)", []).unwrap();
    conn.execute("CREATE INDEX idx_xy ON points(x, y)", []).unwrap();

    // INSERT
    let t = Instant::now();
    conn.execute_batch("BEGIN TRANSACTION").unwrap();
    {
        let mut stmt = conn.prepare("INSERT INTO points (x, y, val, region) VALUES (?, ?, ?, ?)").unwrap();
        for i in 0..n {
            let x = (i as f64 * 0.37) % 1000.0;
            let y = (i as f64 * 0.59) % 1000.0;
            stmt.execute(params![x, y, i as f64, if i%3==0{"US"}else{"EU"}]).unwrap();
        }
    }
    conn.execute_batch("COMMIT").unwrap();
    let sqlite_insert_ms = t.elapsed().as_millis();
    println!("  SQLite  INSERT {} points + INDEX: {} ms ({:.0} pts/s)",
        n, sqlite_insert_ms, n as f64 / sqlite_insert_ms as f64 * 1000.0);
    ...
    println!("  SQLite  spatial KNN top-10: P50={}μs P99={}μs avg={}μs", sql_p50, sql_p99, sql_avg);
...
fn bench_multimodal_write() {
    ...
    let words = ["database", "robot", "sensor", "edge", "ai", "embedded", "realtime", "query"];
    ...
    let rss = get_rss_mb();
    println!("  INSERT {} multimodal rows (VEC+GEO+TEXT): {} ms ({:.0} rows/s, RSS {:.0}MB)",
        n, insert_ms, n as f64 / insert_ms as f64 * 1000.0, rss);

    // CREATE all indexes
    let t = Instant::now();
    db.execute("CREATE VECTOR INDEX idx_emb ON items (emb)").unwrap();
    let vec_idx = t.elapsed().as_millis();
    println!("  CREATE VECTOR INDEX: {} ms", vec_idx);

    let t = Instant::now();
    db.execute("CREATE SPATIAL INDEX idx_loc ON items (loc)").unwrap();
    let spa_idx = t.elapsed().as_millis();
    println!("  CREATE SPATIAL INDEX: {} ms", spa_idx);

    let t = Instant::now();
    db.execute("CREATE TEXT INDEX idx_body ON items (body)").unwrap();
    let txt_idx = t.elapsed().as_millis();
    println!("  CREATE TEXT INDEX: {} ms", txt_idx);

    let t = Instant::now();
    db.execute("CREATE INDEX idx_region ON items (region) USING COLUMN").unwrap();
    ...
    // Mixed queries
    let qstr = vectors[0].iter().map(|f| format!("{}", f)).collect::<Vec<_>>().join(",");
    let _ = db.execute(&format!("SELECT id FROM items ORDER BY emb <-> [{}] LIMIT 10", qstr));
    ...
    println!("    Spatial KNN:     P50={}μs  P99={}μs", spatial_p50, spatial_p99);
