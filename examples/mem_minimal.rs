...
fn main() {
    ...
    // Just 10K rows (small dataset)
    for s in (0..10000).step_by(5000) {
        let e = (s+5000).min(10000);
        let mut b = String::new();
        for i in s..e {
            if !b.is_empty() { b.push(','); }
            b.push_str(&format!("('c_{}',{},'{}')",i%1000,(i as f64*1.7)%1000.0,if i%3==0{"US"}else{"EU"}));
        }
        db.execute(&format!("INSERT INTO t (c,a,r) VALUES {}", b)).unwrap();
    }
    eprintln!("[mem] 10K INSERT: {} KB", rss());
    let _ = db.execute("SELECT COUNT(*) FROM t").unwrap();
    eprintln!("[mem] after COUNT: {} KB", rss());
    let _ = db.execute("SELECT * FROM t WHERE r = 'US' LIMIT 10").unwrap();
    eprintln!("[mem] after WHERE: {} KB", rss());
    println!("DONE");
}
